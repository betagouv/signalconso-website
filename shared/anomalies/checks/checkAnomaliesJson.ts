import {
  allowedInputTypesInYaml,
  companyKinds,
  DetailInputType,
  reportTagsAllowedInYaml,
  specialCategories,
} from "../Anomaly";
...
const anomalySpec: ObjectSpec = {
  ...
  specialCategory: (_) =>
    _.ifDefined()?.assertIsAllowedString(specialCategories),
...
const baseSubcategorySpec: ObjectSpec = {
  tags: (_) =>
    _.ifDefined()?.assertIsArrayOfAllowedStrings(reportTagsAllowedInYaml),
  ...
  // triggers the recursion
  subcategories: (_) => _.ifDefined()?.assertIsArrayWith(assertIsSubcategory),
  companyKindQuestion: (_) =>
    _.ifDefined()?.assertIsObjectWith(companyKindQuestionSpec),
...
const companyKindQuestionSpec: ObjectSpec = {
  ...
  options: (_) =>
    _.assertIsArrayWith((_) =>
      _.assertIsObjectWith(companyKindQuestionOptionSpec)
    ),
};
...
function assertIsSubcategory(subcategory: AnomalyTreeWalker) {
  ...
  if (
    subcategory.value &&
    Object.keys(subcategory.value).includes("blockingInfo")
  ) {
  ...
  function getRestOfSpec(): ObjectSpec {
    switch (detailInput.into("type").value) {
      ...
      case DetailInputType.DATE_NOT_IN_FUTURE:
        return {
          defaultValue: (_) =>
            _.ifDefined()?.assertIsAllowedString(["SYSDATE"]),
        };
