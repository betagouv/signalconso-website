...
export const Acknowledgement = () => {
  const {createReport: {entity: report}} = useReportFlowContext()
  ...
  return fnSwitch(reportCase, {
    ...
    [AcknowledgmentCases.EmployeeReport]: () => (
      <AcknowledgementLayout title="Que va-t-il se passer pour l'entreprise ?">
        ...
        <p>
          Afin de garantir la sécurité de votre emploi, votre signalement ne sera pas transmis à l'entreprise.
          Par contre, il a bien été enregistré dans la base de données de la répression des fraudes
          (<abbr title="Direction Générale de la Concurrence, Consommation et Répression des Fraudes">DGCCRF</abbr>).
        </p>
      </AcknowledgementLayout>
    ),
    [AcknowledgmentCases.ForeignCompany]: () => (
      ...
      >
        <p>
          Vous avez indiqué que l’entreprise est une entreprise étrangère ({country?.name}).
        </p>
        <p>
        ...
        {country?.european && (
          <p>
            Nous vous invitons à faire votre signalement directement auprès du Centre Européen des Consommateurs.
            Il vous apportera une assistance pour régler votre problème.<br/>
            <a
        ...
        {country?.transfer && country.code === 'AD' && (
          <p>
            Nous vous invitons à faire votre signalement directement auprès du service du commerce et de la consommation d’Andorre :<br/>
            <a
              href={externalLinks.consoAndorre}
              rel="noreferrer"
              target="_blank"
              title="comerc.ad (nouvelle fenêtre)"
            >
        ...
        {!country?.european && !country?.transfer && (
          <p>
            Nous vous invitons à faire votre signalement auprès de econsumer.gov afin d’aider les autorités internationales à lutter contre la fraude.
    ...
    [AcknowledgmentCases.NotTransmittable]: () => (
      <AcknowledgementLayout title="Que va-t-il se passer maintenant ?">
        ...
        <Row icon={<Icon aria-hidden="true" sx={{color: t => t.palette.success.light}}>check_circle</Icon>}>
          Les enquêteurs pourront être amenés à vous contacter afin de vérifier votre
          identité ou de vous demander des éléments complémentaires à votre signalement.
        </Row>
      </AcknowledgementLayout>
    ),
    ...
    [AcknowledgmentCases.ContractualDisputeWithSIRET]: () => (
      <AcknowledgementLayout
        ...
        showChargeBack={createdReport.tags.includes(ReportTag.LitigeContractuel) && !!createdReport.websiteURL}
      >
        <Row icon={<Icon aria-hidden="true" sx={{color: t => t.palette.warning.main}}>warning</Icon>}>
        ...
        {createdReport.contactAgreement ? (
          <>
            <p>
              ...
              Elle aura la possibilité de corriger directement le problème grâce à vos informations. Votre nom et vos coordonnées lui seront communiqués s’il souhaite vous
              répondre.
            </p>
            <p>
    ...
    <>
      ...
      <Panel title={
        ...
        <PanelBody className="blog">
          {title && (
            <Txt size="big" bold block sx={{mb: 2}}>{title}</Txt>
          )}
          {children}
          ...
          {showChargeBack && (
            <>
              ...
              <p>
                Grâce à la procédure de charge-back vous pouvez être remboursé gratuitement suite à un achat effectué en ligne :<br/>
