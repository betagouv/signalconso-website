...
import {CompanyDraft, CompanySearchResult, ReportDraft} from '@signal-conso/signalconso-api-sdk-js'
...
import {IsForeignCompany} from './CompanyAskIsForeign'
...
export const _Company = ({
  ...
  return (
    <>
      ...
      <hr/>
      <CompanyByWebsite>
        {(websiteUrl, result) => result && result.length > 0 ? (
          <CompanySearchResultComponent companies={result}>
            {selected => <>{selected.name} {websiteUrl}</>}
          </CompanySearchResultComponent>
        ) : (
          <CompanyIdentifyBy companyKind={draft.companyKind!}>
            {identifyBy => (
              <>{identifyBy} {websiteUrl}</>
            )}
          </CompanyIdentifyBy>
        )}
      </CompanyByWebsite>
...
// const x = () => (
//   <Website>
//     {(website, searchResult) => searchResult ? (
//       <SearchResult/>
//     ): (
//       <IdentifyBy>
//         {identityBy => fnSwitch(identityBy, {
//           NAME: () => (
//             <Name>
//               {searchResult => (
//                 <SearchResult/>
...
//                      <End onComplete={() => onChange({name, postalCode, })}/>
//                     )
//                   </ForeignDetails>
//                 )
//               })}
