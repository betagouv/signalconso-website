// ----------------------------------------------
// ---- Generated file, do not edit manually ---
// ----------------------------------------------
export const frLandingsData = [
  {
    url: 'achat-magasin',
    lang: 'fr',
    isSemiAutomatic: true,
    seoTitle: 'Signalez un achat en magasin à la répression des fraudes',
    seoDescription:
      "Vous avez eu un problème de prix, sur une promotion ou de qualité, lors d'un achat dans un magasin. Vous avez repéré une infraction dans sa publicité ou son fonctionnement...",
    title: 'Achat en Magasin',
    targetedCategory: ['AchatMagasin'],
    catchPhrase:
      'Signalez un problème lors de vos achats en magasin, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi faire un signalement si vous rencontrez des problèmes lors de vos achats en magasin ?',
    secondaryTitle2: 'Que va devenir votre signalement concernant un achat dans un magasin ?',
    sampleReports: [
      {
        text: "J'ai acheté un téléphone en date du 22/11/2022. Au bout de 4 mois, le téléphone tombe en panne, je l'amène au service après-vente qui refuse de faire marcher la garantie au motif qu'il y a un petit impact sur la coque du téléphone. Sachant que j'ai fait cet impact assez rapidement et que mon téléphone a toujours bien fonctionné, le refus de faire marcher la garantie est pour moi aberrant. Je demande donc que mon téléphone soit réparé gratuitement car j'ai une garantie magasin et l'impact sur la coque ne peut pas être la cause du dysfonctionnement.",
        author: 'Léo C.',
      },
      {
        text: "Différence de prix : le prix en caisse était à 2,01 euros. Les caissiers ont simplement dit \"ce n'est pas nous qui changeons le prix\" et ne connaissaient visiblement pas la règle qui impose de facturer au prix affiché. Ils voulaient juste annuler l'article. Ce n'est pas la première fois que je constate cela dans ce magasin qui se trouve dans un quartier populaire. J'ai envoyé un mail au service client.",
        author: 'Nathan S.',
      },
      {
        text: "Affiche trompeuse chez un commerçant : il dit que c'est fait maison, mais rien n'est noté. De plus, il a écrit en gros qu'il vend de la viande de Kobe alors qu'il n'est pas référencé sur le site officiel.",
        author: 'Timothée A.',
      },
      {
        text: 'Bonjour, l’hypermarché XXX récupère les promotions qu’il effectue sur des produits en fidélité négative. Je m’explique en effet suite à une campagne de pub sur les fruits et légumes trop généreuse selon eux la direction a prélevé sur toutes les scanettes fidélités une somme et ainsi nous nous retrouvons avec des sommes négatives - 4 euros qui seront récupérés sur de prochaines promos !!! j’hallucine !!! de plus ce magasin vous a été signalé à plusieurs reprises pour des problèmes liés au dlv sur des produits frais (poulet qui pourrissent avant la fin de la dlv).',
        author: 'Ahmed T.',
      },
      {
        text: "Commerçant derrière la caisse persiste en disant qu'il n'est pas obligé d'indiquer le montant minimum de paiement en CB par un affichage car c’est lui qui décide et qu'il peut même la refuser s’il veut (comportement limite dans sa réponse).",
        author: 'Salma N.',
      },
      {
        text: "Le vendeur m'a vendu un matelas pour 300 € qui ne correspond pas du tout à la description soi-disant matelas spécial dos mais le matelas est de très mauvaise qualité, il ne veut pas reprendre le produit car c'est un marchand ambulant et il vient de XXX et la vente s'est fait à mon domicile à XXX, j'ai le droit à un retour avec un avoir mais le vendeur ne veut plus répondre au téléphone.",
        author: 'Hassan Z.',
      },
      {
        text: "Le problème du sweat est qu'à l'achat il semblait parfaitement conforme, mais une fois lavé à froid comme recommandé sur l'étiquette il a commencé à se décomposer d'une manière fulgurante. Le vendeur dit qu'il ne l'a pas vendu comme ça et refuse donc de le reprendre. Mais un vêtement se lave forcément avant d'être porté. Effectivement dans le magasin il ne semble pas présenter de défaut mais une fois mouillé, celui-ci se décompose. La décomposition a été constatée par le vendeur et celui-ci n'a rien voulu savoir.",
        author: 'Louis S.',
      },
      {
        text: "J’ai acheté un tapis, en magasin, qui n'était pas présenté en magasin, mais sur le catalogue internet. Le prix catalogue, que j'ai réglé est de 359 euros, et l'étiquette sur le tapis, qui venait directement de l'entrepôt mentionnait 299 euros. J'ai fait une réclamation et l'entreprise répond que le prix à payer est celui du catalogue, en vigueur au moment de l'achat, et non de l'étiquette, qui n'est plus valable. Merci de m'indiquer quelle est la règle en la matière.",
        author: 'Soukaina A.',
      },
      {
        text: "Bonjour, j’ai récupéré en magasin un œuf de pâques dans la pâtisserie de XXX à la suite d'une commande sur internet. L'œuf était annoncé comme faisant 320g. Un peu étonnée lorsque je l'ai récupéré, je l'ai pesé : - Poids avec emballage : 305 grammes - Poids de l'œuf avec support en carton : 237 grammes. J'ai, le jour même, contacté le service client pour leur faire part de mon constat et essayer de trouver une solution (après m'être assurée que ma balance était fonctionnelle en pesant un paquet de farine neuf). Ils m'ont justifié cette différence de poids par le fait que la pâtisserie était artisanale et que lors d'une prochaine visite, je pourrais récupérer un petit sachet de fritures, en les prévenant suffisamment en avance (plus de réponses du service client depuis notamment lorsque je leur demande un échange...) Concrètement, l'œuf fait quasi 30% de moins que le poids indiqué sur l'annonce : l'artisanat ne peut justifier un tel écart.",
        author: 'Leila S.',
      },
      {
        text: "Je me permets de vous écrire pour déposer une plainte concernant une infraction à la réglementation en matière de protection des consommateurs dans la chaîne de magasin de bricolage XXX. De plus, je n'ai pas été informé avant l'achat de la possibilité d'une variation de couleur, ce qui m'a empêché de faire un choix éclairé. Il est donc recommandé que XXX informe le consommateur des variations de teinte possibles, que ce soit par le biais d'un panneau explicatif en magasin ou d'une information clairement indiquée sur le site internet. De cette manière, le client pourra prendre une décision éclairée et être mieux préparé aux différences de couleur qui pourraient apparaître.",
        author: 'Alexandre M.',
      },
      {
        text: "Bonjour, J'ai récemment acheté des nouvelles lunettes de vue dans une boutique de centre-ville, et le vendeur m'a informé que j'aurais deux verres simples au prix de 160€ chacun, qui correspond au plafond de remboursement de ma mutuelle. Cependant, après avoir consulté la prise en charge sur le site de ma mutuelle, j'ai remarqué que le vendeur m'a facturé un verre simple à 160€ et un verre complexe à 215€, alors que les deux verres sont unifocaux (simples).",
        author: 'Hassan S.',
      },
      {
        text: "Lors du passage en caisse, j'ai constaté un écart entre le prix affiché en rayon (offre promotionnelle) et le prix en caisse. J'ai signalé cet écart et j'ai eu comme réponse \"il faut que j'aille changer le prix en rayon\" et refus de remboursement du magasin.",
        author: 'Stella T.',
      },
      {
        text: "Le magasin XXX a refusé d'appliquer les conditions générales de vente (CGV) lors d'une demande d'échange d'article suite à l'achat d'un article ne convenant pas. Le magasin a indiqué qu'il pouvait échanger seulement de taille mais pas d'article. Nous avons donc demandé un remboursement comme le prévoit les CGV. Le magasin n'a pas accepté. Nous n'avons pu qu'obtenir un avoir. Nous souhaitons que le magasin se mette en conformité avec les CGV.",
        author: 'Jacqueline R.',
      },
      {
        text: 'Dans les magasins de l’enseigne XXX, la même chaussure Beige/Argent est proposée à 100€ prix normal Homme et Femme. Les problèmes 1/ Le modèle Homme est mis en avant avec une remise de 25€ non appliquée au modèle femme. Il s’agit du même produit avec exactement les mêmes caractéristiques. 2/ Le modèle Homme ne commence qu’à la pointure 41,5. Donc un homme de pointure inférieure à 41,5 doit prendre le modèle femme et ne peut bénéficier de la remise. - Par ces deux faits cette promotion est discriminatoire. - D’une manière générale est-ce règlementaire de proposer deux prix pour un même produit sans informer le client de cette différentiation sexe et taille ?',
        author: 'Stella C.',
      },
      {
        text: "Le commerçant ne veut pas reprendre le produit défectueux malgré nous avoir informés que d'autres clients avaient eu des problèmes similaires avec le même produit. Il ne vend plus de bois exotique suite à de multiples problèmes, mais refuse de rembourser ses clients. Ces lames de bois présentent des trous de termites (non conforme à la vente) et se sont défibrées au bout de deux mois de pose, ce qui est anormal pour un bois exotique. Malgré plusieurs relances, le commerçant ne souhaite rien entendre. Cependant les articles L.217-4 à L. 217-14 du Code de la consommation définissent la garantie légale de conformité. En effet, la loi stipule que la garantie légale de conformité oblige le vendeur professionnel à rembourser ou réparer les produits ayant révélé un défaut dans les deux ans à compter de leur achat. Nous vous remercions par avance de l'attention portée à notre requête.",
        author: 'Mehdi E.',
      },
      {
        text: "Après avoir déposé mon équipement au SAV de la boutique pour réparation, cela fait plus de 60 jours que je n'ai toujours pas récupéré mon matériel, pire, XXX n'a même pas de retour du réparateur agréé pour avoir un état d'avancement. Je ne sais pas ce qu'il en est de mon matériel et XXX me fait patienter sans informations. Je souhaite récupérer mon bien dans l'état où je l'ai déposé.",
        author: 'Jules L.',
      },
      {
        text: "J'ai pu constater que les oursons en guimauve vendu par l'enseigne XXX était fait avec de la gélatine de porc ce qui n'est pas signifié sur l'emballage dans les ingrédients il y a indiqué juste \"gélatine \" il n'y a pas écrit si c'est de la gélatine de porc ou autre de plus les employés en point de vente ne le signalent pas je trouve ça trompeur !",
        author: 'Stella B.',
      },
      {
        text: 'Le commerçant ne veut pas me reprendre le produit car il conteste le fait que les chaussures présentent un défaut de fabrication.',
        author: 'Jules R.',
      },
      {
        text: "Le magasin refuse de traiter les garanties des achats de plus d'un mois. Le SAV renvoie le dossier vers un autre site internet, apparemment celui de son fournisseur. Le magasin doit respecter ses obligations en matière de garantie.",
        author: 'Yasmina Z.',
      },
      {
        text: "Une promotion est affichée avec -50% sur le deuxième article. Le prix de ce même article a augmenté de 40% par rapport à la semaine dernière, revenant plus cher après promotions qu'avec les prix normaux. Ce n'est pas la première fois dans ce magasin que des produits sont affichés en promotions alors qu'ils ont très nettement augmenté par rapport à la semaine précédente.",
        author: 'Enzo A.',
      },
      {
        text: 'Les prix des produits vendus dans cette boutique ne sont pas affichés. Étant donné que les prix sont très élevés, j’ai été très surpris de constater la somme demandée par le commerçant. D’autres clients avec qui j’en ai discuté, m’ont dit qu’ils étaient également étonnés de ne pas voir d’affichage de prix, comme cela doit être le cas. Le commerçant perd sa patience, lorsqu’on lui fait constater ce qu’il n’est ni professionnel, ni tolérable. Il conviendrait de l’obliger à afficher ses prix, soit au gramme, soit au kilo soit à l’unité (barquette) ce qui éviterait des abus et des variations de prix d’un client à l’autre… Merci.',
        author: 'Salma E.',
      },
      {
        text: "Produit alimentaire en date courte. Date dépassée acheté sans vérification. Achat 26/03 date 25/03. Pourtant il y a du personnel le dimanche matin qui aurait dû retirer l'article. Ce n'est pas la 1ere fois que ça m'arrive dans ce magasin avec un produit date courte avec une date de péremption dépassée.",
        author: 'Stella A.',
      },
      {
        text: 'Grosse étiquette bio devant des oignons que ne le sont pas. D’autres produits, fruits et légumes, qui ne sont pas Bio sont sur le même étalage que les produits bio avec de grosses étiquettes en hauteur.',
        author: 'Dylan A.',
      },
      {
        text: "Nous avons passé une commande de 9 articles dans un magasin de meubles : bahut, meuble télé, meuble vitrine, table basse et table salle à manger, 4 chaises pour un montant HT de 9429 euros. Le délai de livraison était prévu pour fin novembre 2022, les articles ont été livrés avec trois mois de retard soit le 24/02/23 sans les chaises. Nous n'arrivons pas à obtenir un délai de livraison précis pour ces 4 chaises.",
        author: 'Zoé A.',
      },
      {
        text: 'La viande n’est pas bonne, mauvais entretien et hygiène déplorable. Après avoir acheté de la viande chez ce boucher nous sommes 3 à être tomber malade, maux de ventre toute la nuit. Le commerçant n’a rien voulu savoir. Aucun prix affiché sur les produits et les employés sont très sales.',
        author: 'Zahra É.',
      },
      {
        text: "Refus de reprise du produit et demande d'avoir pour nouvel achat refusé. Le commerçant évoque une politique de retour produit inchangée depuis plus de 30 ans et l'application d'un protocole interne de revendeur pour se détourner de ses responsabilités. Résultats : la non-reconnaissance de vice caché pourtant indiscutable dû à un problème d'assemblage et de matière rendant le produit inutilisable. Ceci équivaut à un refus d'application des garanties destinées à protéger les consommateurs des procédures multi-contextuelles des relations entre revendeur et fabricant. Finalement c'est le client qui est pénalisé. Par ailleurs une communication en magasin indique la non reprise de produits à prix réduits. Ceux-ci bénéficient pourtant des mêmes garanties. Cette communication est trompeuse et non régulière.",
        author: 'Rayan S.',
      },
    ],
  },
  {
    url: 'achat-site',
    lang: 'fr',
    isSemiAutomatic: true,
    seoTitle: "Signalez un site d'achat à la répression des fraudes",
    seoDescription:
      "Vous avez eu un problème de prix, sur une promotion ou de qualité lors d'un achat sur Internet. Vous n'avez pas reçu votre commande ou celle-ci est abîmée, le service client n'est pas joignable ou insatisfaisant...",
    title: 'Achat sur internet',
    targetedCategory: ['AchatInternet'],
    catchPhrase:
      'Signalez un problème lors de vos achats sur internet, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi faire un signalement si vous rencontrez des problèmes lors de vos achats en ligne ?',
    secondaryTitle2: 'Que va devenir votre signalement concernant un achat sur internet ?',
    otherText1: '\n',
    sampleReports: [
      {
        text: "Commandé sur internet le 4 avril une paire de chaussure sur le site xxx france pour la somme de 59,99 euros. A ce jour aucune nouvelle, aucune réponse aux messages laissés et pas de livraison. Aux dernières nouvelles plusieurs messages indiquent que c'est une arnaque !!! Quels sont les recours contre ce genre de voleurs ?",
        author: 'Zoé N.',
      },
      {
        text: 'Bonjour, le 12/12/2022 nous avons effectué un achat en ligne sur le site XXX. La livraison devait être faite par le transporteur XXX. A deux reprises nous avons trouvé dans la boîte aux lettres 2 avis de passage non datés non signés et sans appel reçu. Après avoir contacté le service de livraison par téléphone et internet ils avaient reprogrammé une livraison. Le jour prévu de la livraison nous étions chez nous et n’avons eu ni appel ni trace de livreur. Depuis nous avons essayé de contacter à plusieurs reprises le transporteur et le site (chat, téléphone, Messenger, Twitter) mais nous n’avons à ce jour toujours pas été remboursés. Il est très difficile de joindre XXX car ils ne fonctionnent que par chat et tous se renvoient la balle. 4 mois après notre commande nous souhaiterions enfin être remboursés. Nous avons tout essayé et nous souhaiterions un peu d’aide… Sur le suivi colis est affiché livré le 15/12 alors que le 30/12 est écrit en transit… Merci',
        author: 'Hugo M.',
      },
      {
        text: "Bonjour, après maintes tentatives pour être remboursé d'une commande non reçue et passée le 12 décembre sur internet (attestation envoyée fin décembre, plus d'une vingtaine d'appels et de relances), la société de service ne traite pas mon dossier pour clore ce dernier. Je suis ainsi contraint de passer par vos services. En espérant que cela puisse m'aider, je vous remercie par avance de votre aide",
        author: 'Stella D.',
      },
    ],
  },
  {
    url: 'animaux',
    lang: 'fr',
    isSemiAutomatic: true,
    seoTitle: 'Signalez un problème d’animaux, garde, vétérinaire, élevage, toilettage à la DGCCRF',
    seoDescription:
      'Vous avez un problème avec un(e) pet sitter, pension, assurance santé animaux, animalerie, pedigree LOOF, éleveur, toiletteur, croquettes, chien, chat, NAC, cheval',
    title: 'Animaux',
    targetedCategory: ['Animaux'],
    catchPhrase: 'Signalez un problème relatif à vos animaux, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi signaler les difficultés relatives à vos animaux, leur alimentation ou leur garde ?',
    secondaryTitle2: 'Que va devenir votre signalement relatif aux animaux ?',
    sampleReports: [
      {
        text: "J'ai acheté des croquettes XXX. Mon chien a été malade. Il s'avère que nous sommes plus de 5000 personnes à avoir eu ces mème problèmes. J'ai demandé le remboursement des paquets non entamés, ce qui m'a été refusé. J'aimerai qu'une enquête soit ouverte pour analyser ces croquettes et arrêter que nos animaux soient malades ou en meurent.",
        author: 'Louis F.',
      },
      {
        text: 'Sauf erreur de compréhension de ma part à la lecture d’un encart affiché au rayon animalerie des nacs, j’ai la nette impression que le magasin XXX détourne la loi en ce qui concerne la nouvelle réglementation sur l’acquisition d’un animal domestique. Détournement de loi concernant l’acquisition d’un lapin sans nécessité d’établir le certificat d’engagement des 7 jours si l’acquéreur a déjà possédé un lapin précédemment. Pardon mais d’une part la loi ne concerne pas que les lapins mais tous les animaux nacs du rayon mais en plus comment les gens vont prouver qu’ils ont déjà détenu cet animal et possèdent les connaissances requises … ? Les animaleries auraient elles obtenu des dérogations ? Merci pour votre éclairement et si besoin que va-t-il se passer suite à ma demande s’il vous plaît ? Avec mes remerciements anticipés. Sincères salutations.',
        author: 'Jacqueline A.',
      },
      {
        text: "Bonjour, nous avons acheté un collier pour notre chien, quelques temps après le collier s'est mis à rouiller. Nous avons contacté XXX et pas de solution. Un jour, mon chien a gardé son collier toute la journée et son cou était tout gris, mon éducateur canin qui nous avait conseillé d'acheter ce collier nous a dit de lui enlever et qu'il avait certainement un problème de fabrication j'ai recontacté la société XXX et pas de réponse C'est pour cela que je demande votre aide. Merci",
        author: 'Zoé Z.',
      },
    ],
  },
  {
    url: 'banque-assurance-mutuelle',
    lang: 'fr',
    isSemiAutomatic: true,
    seoTitle: 'Signalez un problème de banque, assurance, mutuelle à la DGCCRF',
    seoDescription:
      'Vous avez un problème de frais bancaires, carte, CB, remboursement, déclaration de sinistre, prise en charge optique/dentaire, indemnisation, contrat d’assurance, prévoyance',
    title: 'Banque / Assurance / Mutuelle',
    targetedCategory: ['BanqueAssuranceMutuelle'],
    catchPhrase:
      'Signalez un problème à votre banque ou assurance, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi signaler les difficultés rencontrées avec votre banque, votre mutuelle ou votre assurance ?',
    secondaryTitle2: 'Que va devenir votre signalement à l’encontre de votre banque, de votre mutuelle ou  de votre assurance ?',
    sampleReports: [
      {
        text: "Depuis le mois de décembre j'ai déclaré un sinistre dû aux intempéries d'octobre et novembre. Les différents interlocuteurs me disaient que j'allais être remboursé des frais avancés et que l'expert allait venir constater l'étendue des dégâts. Au mois de mars, l'expert vient après avoir appelé à plusieurs reprises pour constater les dégâts et savoir quoi faire. Le 24 mars alors que je n'arrive pas à vous joindre, je contourne la voie normale et réussi à vous avoir, pour vous entendre me dire que je ne serais finalement pas remboursé et que les dégâts ne sont pas à cause des intempéries, alors que depuis janvier 2022 je suis dans la maison et je n'ai pas eu d'infiltration. L'experte qui est venue a bien vu qu'il y a eu des infiltrations le jour de son passage et a dit que ça allait être pris en charges (témoins à l'appui). Au vu de la réponse du 24 mars je voudrais obtenir le rapport de l’expert pour pouvoir faire une contre-expertise. Malgré plusieurs demandes, je n’ai à ce jour toujours rien reçu et les infiltrations continue. Merci de votre aide.",
        author: 'Stella J.',
      },
      {
        text: "Bonjour, je possède un compte au sein de votre Banque XXX. Il s’agit de mon compte bancaire principal, compte largement alimenté. Je possède également un compte auprès de la banque XXX. Depuis quelque temps, je ne peux plus faire de virements vers cette banque à partir de mon compte en ligne. Je dois me rendre au guichet et payer des frais bancaires. La banque invoque à chaque fois un problème technique ou un service indisponible. Or je sais pertinemment que c'est faux car les virements vers les autres comptes épargne sont effectués sans le moindre problème. La banque XXX a tout simplement blacklisté la banque XXX sans le dire. Et ce n'est probablement pas légal. La banque XXX est en outre située dans la zone SEPA. Ce n'est pas la première. J'ai contacté la banque via la messagerie électronique et n'ai reçu aucune réponse. Merci de ce que vous pourrez faire pour résoudre ce problème irritant. Vous trouverez en pièce jointe un exemple de message sur le website de la Banque XXX lorsque l'on tente un virement vers la banque XXX.",
        author: 'Émile L.',
      },
      {
        text: "Le 10 mars, ma maman de 82 ans souhaitait changer de mutuelle et résilier son contrat signé le 31/12/2019 auprès de la mutuelle XXX. Elle signe un contrat avec la mutuelle XXX qui envoie la demande de résiliation auprès de sa mutuelle actuelle. Il lui est précisé le délai d'un mois. Date prévue de début de contrat 15 avril 2023. Je vérifie que la demande de résiliation soit bien enregistrée sur son compte client. Chose faite le 15/03 (suivi demande joint). Je ne m'inquiète pas. Ça suit son cours. Le 6/04, message dans son espace client : Refus de résilier avant la date anniversaire, soit le 31/12. Je téléphone au service client le 7/04 : « Ne vous inquiétez pas, c'est une lettre type ». Le 11/04, l'échéance d'avril est entièrement prélevée sur son compte. Je rappelle le service client parce je m'en étonne. Réponse : « On a reçu le 1er courrier trop tard pour une résiliation le 15 avril, il faut refaire un courrier. » Je contacte le service client sur twitter (réponse jointe). Je capitule. Bilan, à partir du 15 avril, Maman aura et paiera 2 mutuelles. Quelle solution avez-vous à me proposer ?",
        author: 'Enzo N.',
      },
    ],
  },
  {
    url: 'bloctel',
    lang: 'fr',
    isSemiAutomatic: true,
    seoTitle:
      'Vous avez un problème de démarchage abusif malgré Bloctel ? Vous avez un problème de démarchage à domicile ? Signalez-le à la DGCCRF',
    seoDescription:
      'Vous avez un problème de démarchage à domicile ou d’appels indésirables, inconnus, incessants, de spams, de sms, de télé vente, d’arnaque au CPF, à la formation professionnelle',
    title: 'Démarchage abusif',
    targetedCategory: ['DemarchageAbusif'],
    catchPhrase: 'Signalez un problème de démarchage abusif, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi faire un signalement si vous êtes victime de démarchage abusif ?',
    secondaryTitle2: 'Que devient votre signalement pour démarchage abusif ?',
    sampleReports: [
      {
        text: "A plusieurs reprises au mois d'octobre, sur mon lieu de travail, j'ai été contacté par téléphone (harcèlement) par votre commerciale XXX, pour me vendre une parution sur les pages jaunes pour l'entreprise de mon mari. Non seulement, cet appel était non sollicité, mais il était également très intrusif et insistant. L'interlocuteur m'a proposé une offre commerciale que j'ai refusée à plusieurs reprises, mais il a continué à insister de manière agressive et désagréable. Je trouve ce comportement inacceptable et contraire à la législation en vigueur en matière de démarchage téléphonique.",
        author: 'Ali H.',
      },
      {
        text: "XXX veut me vendre la fibre depuis plusieurs mois alors que je leur ai gentiment expliqué que mon habitation ne pouvait pas être raccordée. Un technicien envoyé par XXX en 2021 a confirmé l’impossibilité de raccordement. Systématiquement, j'explique au conseiller XXX qui appelle pendant les heures de repas que techniquement, on ne peut pas... Réponse à chaque fois : \"oui monsieur, c'est bien noté. Vous ne serez plus dérangé !\" En fait rien n'est fait et environ toutes les trois semaines ils m’appellent. J'en ai marre de répéter la même chose. Pour information, je suis inscrit sur Bloctel et ai refusé sur le site XXX d'être contacté pour tout démarchage. Merci d’en tenir compte.",
        author: 'Louis J.',
      },
      {
        text: "Je souhaite porter à votre attention un cas de démarchage téléphonique abusif dont j'ai été victime récemment. Le 23/01/2023, j'ai reçu un appel de la part d'un de vos représentants commerciaux qui m'a proposé une offre promotionnelle pour vos services. Malgré mon refus poli et répété, l'interlocuteur a continué à me harceler avec insistance pour que j'accepte l'offre. J'ai finalement dû raccrocher pour mettre fin à cette conversation agressive et malveillante. Je suis très mécontent de cette expérience et je trouve inacceptable que votre entreprise utilise ce type de méthodes de vente. Je tiens à souligner que je suis inscrit sur la liste Bloctel pour éviter ce type d'appels. Je vous demande donc de prendre les mesures nécessaires pour que cela ne se reproduise pas à l'avenir et de me tenir informé des actions que vous allez entreprendre.",
        author: 'Emma C.',
      },
    ],
  },
  {
    url: 'cafe-restaurant',
    lang: 'fr',
    isSemiAutomatic: true,
    seoTitle: 'Signalez un problème avec un café / restaurant à la répression des fraudes',
    seoDescription:
      "Les descriptions sont incorrectes (origine, fait maison, allergènes, halal). L'établissement est sale, insalubre, problème de fraîcheur, d'hygiène, intoxication ou problème à la commande / livraison",
    title: 'Café / Restaurant',
    targetedCategory: ['CafeRestaurant'],
    catchPhrase:
      'Signalez un problème à votre café ou restaurant, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi signaler un café ou un restaurant qui ne vous a pas donné satisfaction ?',
    secondaryTitle2: 'Que va devenir votre signalement à l’encontre de votre café ou restaurant ?',
    sampleReports: [
      {
        text: "Cet établissement ne respecte pas les règles d'hygiènes que cela soit au niveau du matériel que de la nourriture et également du logement du personnel. J'y ai travaillé 3 jours avant de décider de ne pas y rester car je suis moi-même tombé malade suite à un problème bactériologique. Au niveau de la cuisine, les casseroles et poêles ne sont pas propres et des résidus datant des jours d’avant se voient à l'œil nu, le patron se permet de mettre un vieux seau sale sur son plan de travail où il prépare les plats et trouve cela normal de plus il ne se lave pas les mains entre chaque préparation. Il range le poisson (saumon sec) dans frigo en le couvrant juste avec du papier de poissonnier et le ressort tel quel pour le mettre en préparation si un client en demande. Il ne filme pas les aliments du frigo, se contente de mettre un sopalin humide et le range ainsi. Les murs et plans de travail sont sales, les livraisons se font à même le sol en extérieur sur le trottoir, les poubelles ne sont jamais désinfectées.",
        author: 'Yasmina A.',
      },
      {
        text: 'Ce petit restaurant situé à XXX où je suis allé diner par hasard en famille est horrible. Ce que j’ai pu y voir m’a choqué. L’endroit est très sale les toilettes, qui sont d’ailleurs mixtes, aussi. Il n’y a même pas de produits d’hygiène ni de papier toilettes. De plus la tenancière de l’établissement ne donne pas les prix et encaisse à « la tête du client ». Elle nous a servis un couscous réchauffé de la veille d’après un autre client qui était venu justement se plaindre du manque d’hygiène et n’a pas voulu nous encaisser en carte bancaire prétextant que la machine était en panne ce qui est faux puisque mon épouse l’avait aperçue encaisser un autre client auparavant… De plus au fond de la salle plusieurs groupes de jeunes fumaient et même du cannabis et une bagarre a même éclaté… Certains riverains parleraient même de trafic du sein même de ce restaurant mais je n’en ai pas vu. Il faut faire quelque chose car il est intolérable qu’un tel lieu puisse exister dans un endroit aussi emblématique.',
        author: 'Émile E.',
      },
      {
        text: "Je vous écris pour exprimer ma profonde insatisfaction à propos de la commande que j'ai reçue de votre part. Lorsque j'ai reçu ma commande, j'ai été choqué de constater que la nourriture était dans un état complètement inadmissible. De plus, les emballages étaient déchirés et les contenants de nourriture étaient renversés. Cela a rendu la nourriture immangeable. Je suis très déçu de la qualité de service que j'ai reçu de votre part. J'ai dépensé de l'argent pour une commande qui n'était pas à la hauteur de mes attentes. Je m'attendais à une nourriture fraîche et savoureuse, mais ce que j'ai reçu était tout le contraire. Je suis vraiment déçu de la façon dont ma commande a été traitée. Je demande donc un remboursement INTEGRAL pour ma commande. Je pense que cela est justifié car j'ai reçu un service de très mauvaise qualité. Je vous remercie de votre compréhension et j'attends votre réponse avec impatience.",
        author: 'Amira N.',
      },
    ],
  },
  {
    url: 'demarches-administratives',
    lang: 'fr',
    isSemiAutomatic: true,
    seoTitle: 'Signalez un problème de médiation ou de faux courrier administratif à la DGCCRF',
    seoDescription:
      'Vous avez un problème d’arnaques, services payants pour papiers d’identité, actes d’état civil, certificat d’immatriculation, Crit’Air, carte Vitale, KBis, frais d’enregistrement',
    title: 'Démarches administratives',
    targetedCategory: ['DemarchesAdministratives'],
    catchPhrase:
      'Signalez un problème lors de démarches administratives, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi faire un signalement si vous rencontrez des difficultés lors de vos démarches administratives ?',
    secondaryTitle2: 'Que devient votre signalement relatif à des démarches administratives ?',
    sampleReports: [
      {
        text: "Le 31/03/2022, j'ai fait installer des panneaux photovoltaïques via la société XXX dont le siège est à XXX. L'installation s'est bien déroulée ainsi que les échanges. À la suite de l'installation, j'ai été démarché par cette adresse mail : XXX pour récupérer la TVA. Or, il s'est avéré que j'ai reçu une lettre de relance des impôts indiquant que je n'avais pas déclarer mes panneaux. La personne des impôts en charge de mon dossier, comprenant ma situation, m'a transmis les \"bons\" contacts pour faire ma demande d'assujettissement à la TVA. Au final, l'adresse mail a reçu toutes mes informations personnelles sur l'installation des panneaux photovoltaïques et peut les utiliser à son souhait et non à usage professionnel comme je le pensais. Je vous joins les documents envoyés par XXX (4 premiers documents) et les documents officiels.",
        author: 'Zoé A.',
      },
      {
        text: "Je suis arrivé sur le site XXX à la place de celui de l'ANTS et pensais traiter directement avec les autorités françaises. Cependant, un peu pressé de conclure la transaction avec mon acheteur, j'ai rempli ces formulaires, etc. et effectué à contrecœur un paiement de 59,90 € pour transférer la propriété d'une caravane. Mon acheteur m'a contacté hier soir pour me dire qu'il ne pouvait pas enregistrer le véhicule à son nom de toute façon car ils n'avaient pas encore terminé le transfert. Donc, aujourd'hui, je suis allé sur le bon site Web ANTS et j'ai effectué le transfert moi-même sans aucun frais. Il est très décevant que des entreprises tentent de tirer profit du consommateur de cette manière en utilisant Internet pour apparaître comme l'organisme officiel de l'État capable d'effectuer ce type de transfert de véhicule.",
        author: 'Noémie S.',
      },
      {
        text: "Je me connecte sur le site de la mairie de XXX. Je suis dirigé sans le savoir, sur le site XXX. Ne me méfiant pas, je remplis le formulaire de renouvellement de mon passeport et règle le montant du timbre fiscal de 126 euros. Je reçois alors le récapitulatif de ma demande, le timbre fiscal, l'adresse des mairies. Je vérifie et me rends compte que le prix du timbre est en réalité de 86 euros et qu'il y a donc un écart de 30 euros. Je contacte la société www.advercity.fr qui m'affirme que le coût de 30 euros est bien annoncé sur le site. Je demande l'annulation et le remboursement de mon dossier : on me dit que cela sera fait sous 2 à 4 jours. Inquiète de cette réponse, je fais opposition sur ma carte bancaire. Je vérifie la validité du timbre fiscal sur le site des impôts qui ne retrouve pas l'achat du timbre ! Une seconde fois, je contacte XXX pour obtenir le numéro de la transaction d'achat du timbre fiscal : cela m'est refusé.",
        author: 'Sarah A.',
      },
    ],
  },
  {
    url: 'eau-gaz-electricite',
    lang: 'fr',
    isSemiAutomatic: true,
    seoTitle: "Signalez un problème d'eau, gaz ou électricité à la répression des fraudes",
    seoDescription:
      'Vous avez un problème de contrat d’énergie, de prélèvement, une facture excessive, un dépassement de consommation ou vous avez eu une fuite d’eau, de canalisation.',
    title: 'Eau / Gaz / Electricité',
    targetedCategory: ['EauGazElectricite'],
    catchPhrase:
      'Signalez un problème à votre fournisseur d’énergie, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi faire un signalement à l’encontre de votre fournisseur d’énergie ?',
    secondaryTitle2: 'Que va devenir votre signalement à l’encontre de votre fournisseur d’énergie ?',
    sampleReports: [
      {
        text: "Suite à la résiliation de mon contrat avec la société XXX le 09/12/2022, XXX doit me verser 43.06€. Après vous avoir transmis les justificatifs demandés, vos services auraient viré le montant sur le compte fermé de mon père décédé, puis envoyé un chèque à mon domicile, jamais parvenu. Après vous avoir fourni les copies de mes relevés de compte et une attestation de ma banque jointe démontrant qu'aucun versement XXX n'est intervenu, vous ne cessez d'ouvrir des réclamations arrivant à échéance puis ouvertes à nouveau, sans me régler et malgré les justificatifs fournis. Le dernier événement en date étant de m'informer de la nomination d'un \"conseiller spécialisé\" pour prendre ma réclamation en charge. La question se pose si votre dysfonctionnement renvoie à de l'incompétence ou de la malhonnêteté, ayant tous les justificatifs en votre possession pour régler le montant qui m'est dû depuis le 09/12. Vous remerciant de votre compréhension.",
        author: 'Chloé N.',
      },
      {
        text: "Suite à mes nombreuses sollicitations envers votre entreprise XXX dont je suis client pour corriger mes index de consommations d'électricité, je n'obtiens pas de correction de votre part. En effet, vous m'attribuez un index de début et actuel erroné de (19.10.22 = 97793 KWh) alors que l'index était est 95524KWh. De plus l'index actuel est de 95881KWh. Vous avez donc procédé à la correction de l'indice actuel à la suite de nombreux échanges et l'envoi de preuves. Néanmoins, vous avez attribué un index de début bien en deçà de la réalité et ont donc remplacé l'index du 19.10.22 de 97793KWh par un index de 94950KWh a postériori. J'ai essayé de vous expliquer que ceci est une pratique non raisonnable vu que vous n'avez aucun moyen de faire une relève 5 mois après. Je vous signale donc vos pratiques pour faire payer vos clients plus que leur consommation sont douteuses !",
        author: 'Ahmed C.',
      },
      {
        text: "Nous avons reçu un démarcheur de la société XXX le mercredi 11 janvier 2023 en fin de matinée, ce dernier n'a pas voulu rentrer dans la maison et la démarche s'est tenue sur la pelouse au moyen de son téléphone sur lequel nous avons dû signer. Suite à cette démarche nous avons changé de fournisseur de gaz. Les arguments évoqués étaient si avantageux que nous nous sommes laissé convaincre. Dans cette situation instable, nous n'avons reçu aucuns documents de la part du démarcheur et encore moins de documents concernant une possible rétractation, seulement deux factures de consommation d'un montant de 236,53 et 157,99 euros. Je n'ai pu interrompre ce contrat XXX qu'en redemandant à mon ancien fournisseur une nouvelle admission qui a pris effet le 26 mars 2023. Si j'avais eu un document de rétractation la période de fourniture XXX aurait été beaucoup plus courte. Compte tenu du préjudice je vous demande de bien vouloir examiner ma demande et d’envisager un remboursement.",
        author: 'Maria S.',
      },
    ],
  },
  {
    url: 'immobilier',
    lang: 'fr',
    isSemiAutomatic: true,
    seoTitle: 'Signalez un problème de loyer, syndic, copropriété, DPE, charges à la DGCCRF',
    seoDescription:
      'Vous avez un problème avec une copropriété, agence immobilière, syndic, notaire en tant que locataire ou propriétaire, un souci de loyer, bail, DPE, location, vente ou mandat',
    title: 'Immobilier',
    targetedCategory: ['Immobilier'],
    catchPhrase: "Signalez un problème d'immobilier, de loyer, renseignez-vous sur vos droits avec la répression des fraudes !",
    secondaryTitle1: "Pourquoi signaler des problèmes d'immobilier ou de loyer ?",
    secondaryTitle2: "Que va devenir votre signalement concernant vos problèmes d'immobilier ou de loyer ?",
    sampleReports: [
      {
        text: "On m'a établi un bail avec des documents non conformes ou manquants. Quand j'ai demandé de régulariser le bail et les paiements, on m'a répondu qu'il en était hors de question et qu'aucun geste ne serait fait. Diagnostic Gaz pour les logements de plus de 15ans non fourni et DPE plus valable à la date de signature du bail. Le nouveau DPE fourni plus d'un mois après la date de début du bail et à ma demande est moins qualitatif (note en D au lieu de C qu'on m'a annoncé). Là encore, l'agence refuse de revoir les frais d'agence alors que le service réel est clairement pas à la hauteur du professionnalisme attendu dans le coût de la prestation. Je poste ici car après plusieurs échanges avec l'agence et notamment la gestionnaire du bien qui transmet aux responsables. J'obtiens systématiquement le même retour. Nous maintenons notre position. Nous n'avons fait aucune erreur. Aucun geste ne sera fait. Je trouve cette position très hautaine et laisse aucune possibilité d'agir à l'amiable.",
        author: 'Leila S.',
      },
      {
        text: "Madame, Monsieur, je souhaite signaler une négligence dans mon dossier pour la signature d'un acte de vente qui a eu lieu via l'Office XXX. Un des diagnostics obligatoires était manquant. Nous venons de nous en rendre compte. J'ai voulu m'arranger à l'amiable avec l'étude notariale en proposant de rembourser la facture du diagnostic manquant. Mais ces derniers refusent car le notaire des vendeurs ne souhaite pas payer. J'ai demandé un remboursement du diagnostic qui s'élèvent à 124€... Ils ne sont pas du tout coopérants, je suis dans l'obligation de signaler ces manquements au devoir d'un notaire. Il me semble important de respecter les documents obligatoires dans le cadre d'une vente. Les erreurs peuvent arriver, même chez les notaires, mais la moindre des choses est tout de même de rattraper ces manquements. Je vous remercie pour votre aide et votre compréhension.",
        author: 'Ahmed M.',
      },
      {
        text: "Bonjour, je suis locataire d'un appartement dont la gestion locative a été confiée à une agence immobilière. Depuis le 1er juin 2019, je reçois des quittances de loyer qui font apparaitre un montant inférieur à celui je leur verse chaque mois. Aucun justificatif de charges non plus. Je souhaite porter à votre connaissance ces manquements graves et vous ajoute en pièce jointe, le courrier que je leur adresse ce jour. Je reste à votre disposition pour toute information complémentaire que vous jugeriez utile. Bien cordialement,",
        author: 'Élias S.',
      },
    ],
  },
  {
    url: 'internet',
    lang: 'fr',
    isSemiAutomatic: true,
    seoTitle: "Signalez un problème d'arnaque en ligne à la répression des fraudes",
    seoDescription:
      "Vous avez été victime d'une publicité trompeuse sur internet, vous souhaitez signaler un influenceur sponsorisé qui n'indique pas sa rémunération par une marque...",
    title: 'Internet (hors achats)',
    targetedCategory: ['Internet'],
    catchPhrase:
      'Signalez un problème aux plateformes de médias sociaux, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi faire un signalement si vous constatez les manquements d’un influenceur ?',
    secondaryTitle2: 'Que va devenir votre signalement concernant un influenceur ?',
    sampleReports: [
      {
        text: "En mars 2023 j'ai participé à un concours proposé par la société XXX. Pour participer il fallait liker la page de la société, partager la publication, et taguer en commentaire un ami susceptible d’être intéressé. Le tirage au sort devait avoir lieu quelques jours plus tard. J'étais le seul participant, donc je pensais évidemment être le gagnant. N'ayant pas de nouvelle du tirage au sort, j'ai contacté ce matin ladite société pour demander si j'avais bien gagné, et on m'a répondu que non car il fallait un minimum 10 participants. Cette clause n'apparaissait nulle part sur leur publication. J'ai donc demandé le règlement du concours mais je n’ai pas eu de réponse de leur part. Quelques instants plus tard, les publications relatives au concours ont été effacées de la page. Vous trouverez en pièces jointes des captures d'écran prouvant l’existence de ce concours ainsi que ma participation.",
        author: 'Leila J.',
      },
      {
        text: "Une personne se cachant derrière une fausse identité et revendiquant à tort le titre de médecin a créé des comptes sur les réseaux sociaux pour faire de la publicité mensongère. Elle fait notamment la promotion d’un produit de type bain de bouche dont elle affirme qu'il peut traiter l'intégralité des problèmes dentaires. Elle mentionne expressément dans ses publicités les effets sur les caries, la parodontite, l’halitose et les aphtes. Toutes ces publicités sont totalement mensongères car des produits avec de telles propriétés n'existent évidemment pas. Il semblerait que la personne situe son activité en Californie. Mais elle parle français et sa cible de vente est située en France ou est du moins francophone. Il est urgent d'interdire cette page et ses publicités ayant pour objectif de tromper les consommateurs.",
        author: 'Axel A.',
      },
      {
        text: "J’ai fait l’achat d'un fond de teint et d'une poudre avec un code promo proposé sur les réseaux par une influenceuse mais je n'ai jamais reçu ma commande. Après plusieurs semaines, j'ai contacté l'influenceuse pour savoir où en était ma commande. Elle m’a dit que j'allais recevoir ma commande à plusieurs reprises et que les retards et problèmes de livraison étaient dû à la guerre en Ukraine. Au début je l’ai crue et j’ai pris mon mal en patience. Mais je n’ai aucune nouvelle du site. J’ai donc décidé de passer un appel à la poste avec les références de mon colis mais il aurait été envoyé à une adresse qui n’est pas la mienne et il semblerait en plus que le colis ne soit pas à mon nom. Mais je n’habite pas là-bas et j’ai bien passé la commande à mon nom !!! Mais après avoir fait des recherches sur internet et sur différents forums je me suis aperçu que je n'étais pas seul, loin de là. Je regrette d’avoir bêtement fait confiance à cette influenceuse.",
        author: 'Adam M.',
      },
    ],
  },
  {
    url: 'intoxication-alimentaire',
    lang: 'fr',
    isSemiAutomatic: true,
    seoTitle: 'Vous êtes malade, diarrhée, nausée, fièvre suite à un repas, signalez-le à la DGCCRF',
    seoDescription:
      'Vous êtes souffrant, avez des problèmes de digestion après avoir mangé un aliment suspect, goût, odeur de la nourriture ? Mal au ventre, estomac, vomissement ?',
    title: 'Intoxication alimentaire',
    targetedCategory: ['IntoxicationAlimentaire'],
    catchPhrase:
      "Signalez un problème d'intoxication alimentaire, renseignez-vous sur vos droits avec la répression des fraudes !",
    secondaryTitle1: "Pourquoi faire un signalement si vous êtes victime d'une intoxication alimentaire ?",
    secondaryTitle2: 'Que devient votre signalement relatif à une intoxication alimentaire ?',
    sampleReports: [
      {
        text: "Achat d'un sandwich au saumon dans une boulangerie. Diarrhées aiguës et fièvre pendant 1 semaine : contamination 2 bactéries suite à analyses de selles, j'ai du consulter un médecin.",
        author: 'Louis S.',
      },
      {
        text: "Les pâtisseries n'avaient pas l'air fraiches lorsqu'on les a ouvertes à la maison. Nous les avons prises au goûter, et très rapidement nous avons été malades (vomissements, diarrhée). Nous avons évoqué notre incident avec des amis de la commune qui nous ont indiqué ne plus rien acheter dans cette pâtisserie, que des problèmes d'hygiène y sont connus... Nous ne le savions pas. La société est en redressement judiciaire. Le lendemain matin, nous sommes passés devant la porte du laboratoire, il y avait des crottes de chiens à l'intérieur et une forte odeur d'urine. L'immeuble est en vente, connaissant l'identité du propriétaire, nous l'avons contacté et nous a indiqué que le gérant de la pâtisserie délabrait tout, qu'il y avait des crottes partout... Nous ne retournerons plus acheter de produits là-bas, mais nous souhaitons vraiment que cela n'arrive à personne d'autre.",
        author: 'Zahra N.',
      },
      {
        text: "Hier à partir de 20H30 ma fille a vomi, puis moi vers 22H, ensuite mon fils vers minuit et mon mari ce matin à 6H30. Tous les 4 avons eu des vomissements, nausées, maux de ventre, maux de tête et frissons. Et ce plusieurs fois depuis l'heure de départ, et cela continue. Grosse fatigue également. Nous avons tous déjeuné vers 13H30, des lasagnes faites maison le dimanche matin, à partir de la viande steak hachée achetée chez XXX.",
        author: 'Leila J.',
      },
    ],
  },
  {
    url: 'pathdelacategory',
    lang: 'fr',
    isSemiAutomatic: true,
    seoTitle: '(seoTitle) Le titre SEO de la category',
    seoDescription: '(seoDescription) Description SEO de la category',
    title: '(title) Une category de démo',
    targetedCategory: ['DemoCategory'],
    catchPhrase: "Phrase d'accroche blabla, bla bla répression des fraudes",
    secondaryTitle1: 'Titre bla bla pourquoi faire signalement',
    secondaryTitle2: 'Titre bla bla que va devenir le signalement',
    sampleReports: [],
  },
  {
    url: 'sante',
    lang: 'fr',
    isSemiAutomatic: true,
    seoTitle: 'Signalez un problème avec un professionnel de santé à la répression des fraudes',
    seoDescription:
      "Vous avez eu un reste à charge trop élevé, un non-respect du 100% santé, n'avez pas eu de devis par votre clinique, hôpital, opticien ou les effets mis en avant en parapharmacie sont trompeurs...",
    title: 'Secteur de la santé',
    targetedCategory: ['Sante'],
    catchPhrase:
      'Signalez un problème dans le secteur de la santé, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi faire un signalement si vous rencontrez des problèmes dans le secteur de la santé ?',
    secondaryTitle2: 'Que va devenir votre signalement concernant le secteur de la santé ?',
    sampleReports: [
      {
        text: "Bonjour, je suis allé aujourd'hui à la Grande Pharmacie XXX. Je voulais acheter un flacon de vitamines. Le prix affiché était de 12,90 euros. Arrivé à la caisse, la \"charmante\" dame me demande 16,90 euros. Je lui signale l'erreur d'affichage de prix et elle me soutient que les prix ont augmenté et que c'est maintenant celui-là. Je proteste et insiste sur le fait qu'elle doit me faire payer le prix indiqué sur l'étiquette, que l'erreur vient d'eux et que ce n’est pas à moi, client, de payer leur erreur. Elle finit par m'envoyer \"ch…\". J'ai quitté cette pharmacie sans prendre le produit vu la mauvaise foi de cette dame (Responsable de la pharmacie ? je ne sais pas) ..., je suis parti aussitôt signaler ce litige à la police. Je pense ne pas être la seule personne à qui cela arrive. Mais personne n'ose rien dire. J'espère que mon signalement servira à quelque chose et que vous pourrez vérifier leur prix sinon c'est du vol. Merci de votre attention.",
        author: 'Fatima L.',
      },
      {
        text: "Le docteur XXX, médecin généraliste de secteur 1, affiche sur Doctolib une consultation à distance à 25€. Il ne m'a pas notifié pendant notre séance (qui a duré environ 10 minutes), un dépassement d'honoraires de 15€. Doctolib précise que cette notification doit être pratiquée pour pouvoir appliquer un complément de tarification non prévu initialement. Je précise que je n'avais jamais eu de consultation vidéo avec ce praticien auparavant.",
        author: 'Soukaina A.',
      },
    ],
  },
  {
    url: 'services-aux-particuliers',
    lang: 'fr',
    isSemiAutomatic: true,
    seoTitle: 'Signalez un problème avec un service / prestation à la répression des fraudes',
    seoDescription:
      "Vous avez un problème avec un organisme de formation, coiffeur, esthéticien, tatoueur, ménage, déménageur, aide à domicile, garde ou accompagnement d'enfants...",
    title: 'Services aux particuliers',
    targetedCategory: ['ServicesAuxParticuliers'],
    catchPhrase:
      'Signalez un problème à votre prestataire de service, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi faire un signalement si vous rencontrez des problèmes avec un prestataire de service ?',
    secondaryTitle2: 'Que va devenir votre signalement concernant un prestataire de service ?',
    sampleReports: [
      {
        text: "Suite à une course réservée auprès de votre société XXX en véhicule tarif éco pour faire un trajet XXX, le choix du paiement directement dans le véhicule est choisi pour 385€. Pour cause de prolongation d'hospitalisation, la course ne s'est pas faite. Je procède à l’annulation plus de 48h avant, la société nous prélève quand même 385€ de caution malgré le délai d’annulation respecté. Impossible de nous les rembourser car c’est soi-disant inscrit dans les CGV. Un avoir nous est proposé en nous demandant notre carte d'identité. 1ERE ESCROQUERIE Quelques jours plus tard, nous nous voyons encore prélevé par cette société 245€ sans notre accord. Soi-disant correspondant à la réservation d'un VAN spacieux, et donc un tarif plus cher. Or, nous avions choisi une berline tarif ECO (voir PJ). Votre société reconnait son erreur, confirme donc le remboursement à venir. Mais �� ce jour, impossible de vous joindre par mail, téléphone ou sms et le remboursement n'est pas effectué. 2EME ESCROQUERIE",
        author: 'Salma L.',
      },
      {
        text: "J'ai demandé le 24 février dernier, en respectant le délai légal de 14 Jours prévu à par l'article L221-18 le remboursement intégral d'un montant de 109e au vu de la formation de conduite supervisée. Pour rappel ce délai de rétractation est stipulé dans l'article 1 de vos conditions générales de vente. Pour rappel cette rétractation intervient suite à votre incompétence répétée mais également à la vente limite mensongère d’un service dont je n'ai point pu faire usage. Or, depuis le 14 mars je suis sans nouvelle de ce remboursement, qui de plus n'est pas effectué dans les règles. Vous affirmez envoyer un chèque alors que le remboursement intervient lors d'une rétractation sur un paiement réalisé par Carte Bancaire. Ce n'est pas la première fois que je fais face à un litige en lien avec votre entreprise. (cf- Litige du 21 Juillet concernant l'examen à la conduite) J'aimerais obtenir dédommagement de ces multiples problématique qui n'aurait jamais dû avoir lieu.",
        author: 'Stella S.',
      },
      {
        text: 'J’ai sollicité la plateforme XXX pour des cours de soutien pour ma fille. Sur leur site il est indiqué que le site est là pour vous mettre en contact avec des professeurs. J’ai effectivement été mis en contact avec un professeur, qui propose des cours en visio. Cela ne me convenait pas, nous souhaitions des cours en privé et à domicile. Ce professeur m’a donc remboursé les 90€ que je lui avais versé pour 3h de cours prévues. Mais ce qui n’apparaît nulle part, c’est que vous vous engagez à verser une cotisation de 29 € mensuelle à la plateforme pour vous mettre en relation avec des professeurs. Ce site est une véritable bande organisée, qui escroque un nombre considérable de parents ou d’étudiants. J’ai pour ma part été débité de 5x 29€ à mon insu soit 145€. J’ai contacté cette entreprise qui m’indique que cela était précisé et que j’ai validé cet achat. Par conséquent je me vois spolié de 145€. Sur des forums des centaines de personnes se sont faites arnaquer par cette entreprise. Quelle est la procédure pour obtenir l’intégralité de ce qui m’a été volé, oui je dis bien volé sur mon compte bancaire. Malgré mais différentes demandes et mes échanges par écrits, je n’ai aucun retour !',
        author: 'Adam S.',
      },
    ],
  },
  {
    url: 'tel-internet-media',
    lang: 'fr',
    isSemiAutomatic: true,
    seoTitle: 'Signalez un problème de forfait à la répression des fraudes',
    seoDescription:
      'Votre opérateur vous a facturé des options non souscrites, votre abonnement mobile ou box est en augmentation ou a été reconduit abusivement, vous avez un doute sur les frais de résiliation.',
    title: "Téléphonie / Fournisseur d'accès internet / médias",
    targetedCategory: ['TelephonieFaiMedias'],
    catchPhrase:
      "Signalez un problème avec votre opérateur mobile ou fournisseur d'accès internet, renseignez-vous sur vos droits avec la répression des fraudes !",
    secondaryTitle1:
      "Pourquoi faire un signalement si vous rencontrez des problèmes avec votre opérateur mobile ou votre fournisseur d'accès internet ?",
    secondaryTitle2: 'Que va devenir votre signalement concernant vos problèmes de mobile ou internet ?',
    sampleReports: [
      {
        text: "Bonjour, un abonnement intitulé XXX est associé à mon forfait de téléphone depuis le mois de février. Cet abonnement a été ajouté à mon forfait sans mon accord. Cet abonnement est d'un montant de 3 euros hebdomadaire par tacite reconduction. Je n'ai jamais adhéré à ce service ni donner quelconque autorisation de prélèvement. Je souhaite l'arrêt immédiat de ce prélèvement frauduleux et exige un remboursement intégral des montant prélevés. Je joins les factures détaillées.",
        author: 'Noémie M.',
      },
      {
        text: "Bonjour, je viens me plaindre de l’opérateur XXX qui m'a envoyé une facture de 116.96€ qui fait suite à mon abonnement. En revanche, nulle part lors de la souscription / échange de box, j'ai été informé de la facturation de 49€ de renouvellement de la box et de 20€ pour son envoi. Facture augmentée de 69€. Cela est d'autant plus étrange qu'en théorie, les clients de plus de 5 années sont, à priori, exonérés de payer ces deux sommes comme signalé dans leurs pubs. Est-ce volontaire ? Malgré mon appel la semaine dernière, je n'ai pas de nouvelle. Veuillez m'aider dans cette démarche afin d'être remboursé de cette somme de 69€. L'information donnée est que seule la Box XXX n'est pas concernée par cette facturation supplémentaire. Box qui est indisponible pour quelques temps et l'était lors de ma demande de renouvellement. Ancienneté de 20 ans… Je vous joins la facture concernée. Cordialement,",
        author: 'Stella C.',
      },
      {
        text: "Je suis abonné à la fibre chez l’opérateur XXX depuis le 12/08/2022 et engagé sous 12 mois. L'opérateur a augmenté le forfait de base sans m'en informer et je le constate aujourd'hui sur ma facture. Pouvez m'indiquer si je reste engagé ou si je peux résilier sans frais ce contrat ?",
        author: 'Inès M.',
      },
    ],
  },
  {
    url: 'travaux-renovation',
    lang: 'fr',
    isSemiAutomatic: true,
    seoTitle: 'Signalez un problème de travaux ou rénovation à la répression des fraudes',
    seoDescription:
      "Vous avez fait intervenir un artisan (serrurier, plombier, isolation, pompe à chaleur, chaudière, prime rénov, fenêtre, ...) et vous avez un souci de délais d'exécution, de devis, malfaçon, ...",
    title: 'Travaux / Rénovation',
    targetedCategory: ['TravauxRenovations'],
    catchPhrase: 'Signalez un problème à votre artisan, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi faire un signalement si vous rencontrez des problèmes lors de travaux ou rénovation ?',
    secondaryTitle2: 'Que va devenir votre signalement concernant vos travaux ?',
    sampleReports: [
      {
        text: 'A ce jour vous avez réalisé en partie des travaux de plomberie dans mon appartement. Durant le chantier nous avons rencontré de nombreuses fois des problèmes liés à votre installation : une fuite d’eau, une fixation des raccords défectueuse, un écoulement difficile à cause d’une pente insuffisante, un mauvais choix des diamètres de tuyaux, l’installation d’un coude à la place d’un siphon sur un évier, de nombreux raccords pour relier les tuyaux, une arrivée d’eau impossible depuis l’alimentation générale et enfin un chauffe-eau endommagé. Cette mauvaise installation a généré de nombreux dégâts : des dégâts des eaux, des trous dans les murs et les plafonds, un sol détérioré… Je tiens à souligner que ces dommages ont été intégralement à ma charge, moi, le propriétaire. A la suite de tout cela je suis dans l’obligation de mettre un terme à notre collaboration. J’ai fait appel à un autre plombier pour terminer et corriger toutes les erreurs de l’installation. Je vous demande de régler la situation à l’amiable en prenant en charge l’intégralité des coûts des travaux.',
        author: 'Stella E.',
      },
      {
        text: "J'ai contacté votre entreprise par téléphone afin de me faire faire un devis pour une potentielle installation d'un poêle à bois. Une personne est venue pour un AUDIT mais en aucun cas ne m'a informé que je devrais régler quoique ce soit... ou que cela engendrerait un engagement financier ou autre de ma part… Si à la suite de ce rendez-vous j’avais été intéressé, il aurait fallu que je donne suite or je ne l'ai pas fait... Malgré cela j'ai reçu dernièrement un courrier de votre part qui m'indique de vous faire parvenir sans délai mon avis d'imposition 2022 ainsi que ma taxe foncière 2022 sans quoi je vous serai redevable de la somme de 500 euros... J'ai essayé à plusieurs reprises de vous contacter afin d'avoir des explications mais votre entreprise est injoignable.",
        author: 'Idriss C.',
      },
      {
        text: 'Votre devis est évasif et imprécis. En effet, les informations que vous m’avez donnés oralement ne sont pas retranscrites dessus. Je constate ensuite une usurpation de ma signature sur un second devis et sur la facture. Par ailleurs, les tarifs que vous proposez sont prohibitifs, la facturation est très exagérée en ce qui concerne les services. Vous mentionnez notamment une recherche de panne à 550 euros la minute ! Vous êtes de mauvaise foi, vous usez de menaces à peine voilées. Vous vous permettez l’enregistrement de nos conversations téléphoniques sans mon accord préalable !',
        author: 'Yasmina M.',
      },
    ],
  },
  {
    url: 'voiture-vehicule',
    lang: 'fr',
    isSemiAutomatic: true,
    seoTitle: 'Signalez un problème lié à la réparation, entretien, contrôle technique d’une voiture',
    seoDescription:
      'Vous avez un problème avec un garagiste, agence de location suite à la réparation d’un pneu, d’une panne, la vente d’un véhicule neuf ou d’occasion, la non application de la garantie constructeur',
    title: 'Voiture / Véhicule / Vélo',
    targetedCategory: ['VoitureVehiculeVelo'],
    catchPhrase:
      'Signalez un problème de voiture ou de véhicule, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi faire un signalement si vous rencontrez des problèmes avec une voiture ou un véhicule ?',
    secondaryTitle2: 'Que va devenir votre signalement concernant une voiture ou un véhicule ?',
    sampleReports: [
      {
        text: "Achat d'un véhicule de marque XXX, au garage XXX , avec lors du bon de commande une garantie de 12 mois, à la livraison pas de trace sur la facture de cette garantie ( le vendeur me dit qu'elle est en cours auprès d'un organisme et qu'elle sera du haut de gamme au regard de la valeur de cette voiture, des problèmes sont arrivés sur ce véhicule apres 350 kms parcourus et la facture a été réglée volontairement par le garage, mais à ce jour aucune trace de cette garantie, qui selon le patron du garage commencera au moment de sa mise en place de cette garantie de 12 mois même après 4 mois. Pour info les problèmes liés au véhicule auraient du être notifiés au contrôle technique. Je pense que nous sommes face à une vente en abus de confiance, un véhicule garanti n'est pas au même prix que non garanti. En l'attente d'une intervention de la part de vos services. Cordialement.",
        author: 'Axel L.',
      },
      {
        text: "Suite à panne moteur sur autoroute à Sanary 83.Le dépanneur d'autoroute m'a facturé 96 euros de frais de gardiennage pour un week-end férié de Pâques (celui ci ne réparant pas la voiture) et ceci le temps que l'assistance envoie une autre dépanneuse pour l'emmener dans un autre garage de réparation agrée par l'assurance. Ce  garage ne m a pas informé des frais au préalable et sans me donner un contrat ou autre. Ps : j' avais du payé à distance (CB ) sous menace de bloquer mon véhicule chez eux (verbalement)",
        author: 'Jacqueline S.',
      },
      {
        text: "Bonjour, suite à une défaillance avérée du réservoir adblue de mon véhicule et après 3 mois d'échanges avec XXX, j'ai obtenu la prise en charge financière des pièces liées au changement de réservoir mais la main d'œuvre est restée à ma charge, soit 289.86 €. Or s'agissant d'un problème avéré, non lié à l'usure du véhicule, en témoigne la prise en charge des pièces à 100%, et compte tenu du faible kilométrage de ma voiture, je sollicite également le remboursement de la main d'œuvre liée à cette intervention. Vous trouverez la facture en PJ. J'ai déjà effectué cette demande auprès de XXX par anticipation sans retour positif. Étant dorénavant en possession de la facture, je réitère cette demande. Merci, cordialement.",
        author: 'Inès I.',
      },
    ],
  },
  {
    url: 'voyage-loisirs',
    lang: 'fr',
    isSemiAutomatic: true,
    seoTitle: 'Signalez un problème de séjour, vol, voyage à la répression des fraudes',
    seoDescription:
      'Vous avez un problème d’annulation, retard, remboursement concernant une réservation d’un billet d’avion, train, bateau, taxi, VTC, hôtel, camping ou un soucis de bagage',
    title: 'Voyage / Loisirs',
    targetedCategory: ['VoyageLoisirs'],
    catchPhrase: 'Signalez un problème relatif à un voyage, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi signaler les difficultés relatives à un voyage, loisir ou divertissement ?',
    secondaryTitle2: 'Que va devenir votre signalement relatif à un voyage ?',
    sampleReports: [
      {
        text: "Bonjour, J'ai réservé un billet d'avion sur le site internet XXX . J'ai voulu consulter les détails et vérifier ma réservation après l'achat de celui-ci. Suite à une mauvaise manipulation, ma réservation a été annulée accidentellement. Lorsque l'annulation a été faite, il était bien stipulé que \"Si vous annulez votre voyage, nous vous rembourserons selon le tarif du billet que vous avez réservé\"(voir pièces jointes). Le site parle bien de \"tarif du billet\" et non de \"type de billet\" ? En français lorsqu'on parle de \"tarif du billet\", c'est qu'il s'agit du montant du billet réservé. Au final le site rembourse sous forme de crédit de 10 euros à utiliser sur leur site internet ! Le parcours de ce site internet est fait pour que vous vous trompiez. C'est la première fois que je fais face à une si grande arnaque.",
        author: 'Emma A.',
      },
      {
        text: "Bonjour, j'ai effectué une réservation d'une semaine en pension complète pour 5 personnes entre juillet et aout, j'ai payé pour cela mais le mail de confirmation que j'ai reçu indique qu'il s'agit d'une demi-pension. J'ai pourtant bien sélectionné pension complète lors de ma recherche puis j'ai vérifié que c'était encore bien écrit lors lors de la sélection des dates. J'ai donc enregistré une vidéo pour montrer toutes les étapes de la réservation, nous n'y voyons jamais apparaitre la demi-pension. Après quelques recherches il s'avère qu'il faut cliquer sur un lien qui ouvre un popup pour voir que la pension complète est subitement passé en demi-pension. A cause de ce changement camouflé je n'ai pas vu que la pension avait changée, j'aurais pourtant du être avertie clairement. J'ai donc contacté le service client pour leur demander de bien vouloir me passer en pension complète, mais il a refusé. ",
        author: 'Amira N.',
      },
      {
        text: "Bonjour je me permet de vous contacter car nous avons réservé le 02/02/2023 un séjour en Tunisie avec l'agence XXX avec seulement nos cartes d'identité du 27 mai 2023 au 3 juin 2023. Nous apprenons autour de nous la semaine dernièrere que la compagnie aérienne avec laquelle nous partons refuse les cartes d'identité. Je me rend à cl'agence XXX pour annuler notre voyage on m'annonce 375€,24 de frais d'annulation. On me dit que le mieux c'est de se rendre à l'aéroport et quand l'embarquement nous sera refusé de les appeler et que \"peut-être \" il y aura remboursement ! J'appelle la compagnie aérienne qui me confirme qu'il ne font plus de séjour en Tunisie sans passeport. Comprenez que je ne vais pas engager des frais pour me rendre à un voyage pour lequel je suis sure de ne pas embarquer ! Nous sommes victimes d'une escroquerie ! Nos connaissances qui partent avec une autre agence en Tunisie on eu l'annulation et le remboursement de leur séjour.",
        author: 'Amira L.',
      },
    ],
  },
  {
    url: 'allegation-neutralite-compensation-carbone',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Fausse allégation compensation ou neutralité carbone',
    seoDescription: 'Signalez une fausse allégation relative à la neutralité ou la compensation carbone',
    title: 'Usage d’une fausse allégation carbone',
    targetedCategory: ['AchatMagasin'],
    catchPhrase:
      'Signalez l’usage d’une fausse allégation relative à la neutralité ou la compensation carbone pour éviter qu’il y ait d’autres victimes ! ',
    secondaryTitle1:
      'Pourquoi signaler l’usage frauduleux d’une allégation relative à la neutralité ou la compensation carbone ?',
    secondaryTitle2:
      'Que devient votre signalement de l’usage d’une fausse allégation relative à la neutralité ou compensation carbone ?',
    sampleReports: [
      {
        text: "Dans plusieurs capsules vidéos sponsorisées, l'influenceur minimise abusivement la réalité de la pollution du secteur aéronautique. En effet, l'impression générale de respect de l'environnement est en contradiction avec la réalité factuelle en termes de pollution. Concernant la promesse de neutralité carbone, l'ADEME a l'an passé publié un document déconseillant fortement l'utilisation de ce terme par son caractère trompeur. De nombreuses études pointent l'incapacité du secteur à atteindre le net zéro en 2050 sans réduction substantielle du trafic aérien. Des affirmations telles que 0g de CO2 pour les moteurs électriques manquent de précision sur l'ensemble de la chaîne de production. L'utilisation de publicités comparatives, couplées au manque de connaissance du public, induit en erreur le récepteur quant à la pollution réelle. Il convient de tenir compte du contexte de visionnage utilisé qui renforce la capacité à induire le public en erreur.",
        author: 'Louis Z.',
      },
      {
        text: "Bonjour, Je vous sollicite pour vous demander d'étudier le marquage de la compensation carbone sur les stations-service de votre société. Comment pouvez-vous faire croire au consommateur qu'en achetant de l'essence et du gasoil dans votre station-service il réduit les émissions de CO2. Je trouve honteux ce genre de greenwashing qui donne un message erroné. A vous lire on pourrait penser que plus on roule plus on réduit les émissions carbones.\n",
        author: 'Jules É.',
      },
      {
        text: "En visitant le rayon des cafés, j'ai été attirée par l'emballage de votre café biologique qui annonçait fièrement sa neutralité carbone. Toutefois, en cherchant à obtenir des informations supplémentaires sur cette affirmation, je n'ai trouvé aucun rapport ni aucune preuve tangible soutenant cette déclaration. Cette absence de transparence me laisse perplexe et soulève des doutes quant à l'engagement réel de cette marque envers la durabilité environnementale. Je vous remercie de m'apporter un éclairage sur cette question et de me garantir l'intégrité des informations fournies à vos consommateurs.",
        author: 'Fatima M.',
      },
    ],
  },
  {
    url: 'arnaque-escroquerie',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Arnaque Escroquerie',
    seoDescription: 'Votre problème concerne une arnaque ou une escroquerie ?',
    title: 'Arnaque ou escroquerie',
    targetedCategory: [],
    catchPhrase: 'Signalez une arnaque ou une escroquerie',
    secondaryTitle1: 'Pourquoi signaler une arnaque ou une escroquerie ?',
    secondaryTitle2:
      "Vous avez été victime d'une arnaque ou d'une escroquerie en magasin ou en ligne ? Faux site de vente, phishing, ... ?",
    sampleReports: [
      {
        text: "J'ai commandé un robot de piscine que j'ai payé par carte bancaire sur ce site et je n'ai toujours pas de nouvelles à ce jour . Je leur ai envoyé plusieurs e-mails mais aucun retour de leur part. Ce site est toujours actif. ATTENTION c'est une arnaque. Merci de faire fermer ce site avant qu'il y ait trop de personnes dans le même cas que moi.",
        author: 'Mathis M.',
      },
      {
        text: "Commande faite en ligne le 09/05/23, je ne l'ai jamais reçue. J'ai contacté l'adresse mail service client à deux reprises sans retour. Sur leur site, il n'y a pas d'adresse, ni de numéro pour les contacter. Le site est encore en ligne alors que c'est une arnaque !",
        author: 'Amira L.',
      },
      {
        text: "J'ai fait appel à la société en ligne https://xxxxxx.com/ pour la création d'une carte grise, ils m'ont prélevé plus de 450 € pour un service qui n'a jamais eu lieu. Les avis de ce site sont à 90‰ négatif. Je réclame le rembourse de la totalité de la prestation. Je suis toujours dans l'attente d'une réponse. L'activité se poursuit de mois en moi set les gens se font arnaquer comme moi, ce site devrait être interdit ou fermer.",
        author: 'Emma S.',
      },
      {
        text: "Je subis une arnaque venant du site xxx concernant l'achat de statues en résine, en effet j'ai fait une commande sur ce site pour une somme de 179 euros, à ce jour je n'ai toujours rien reçu presque un après, j'ai essayé de les contacter à maintes reprises par mail et téléphone, et leurs réponses restées toujours la même:\" nous passons votre commande en priorité\" .. etc... Nous avons même demandé le remboursement mais ils nous répondent que c'est impossible. En octobre nous avons été informé que la commande aller être livré oufff!!!, elle l'a été oui!!! avec plein de défauts, donc nous n'avons pas accepté le colis et l'avons renvoyé. Depuis plus de nouvelle. Svp nous avons besoin d'aide pour le remboursement, ou autre. Ci joint vous trouverez ma commande, et quelques réponses de mails parmi tant d'autre.",
        author: 'Timothée S.',
      },
      {
        text: "Je me suis fait arnaqué par phishing. Je recevais des mails de xxx sur ma messagerie qui me disait que j'avais été tiré au sort pour obtenir gratuitement une cafetière électrique de la marque xxx. J'ai répondu à l'offre. Ensuite j'ai reçu un message sur ma messagerie qui me demandait de payer 1,95 € de frais d'envoi. J'ai accepté et payé avec ma carte bancaire. J'ai reçu un mail de qui me confirmait la prise en compte de mon paiement, avec des liens que je n'ai pas lu. En fait je venais de payer 5 jours d'essai à un service professionnel. Le 26/05 j'ai été débité sur mon compte bancaire par prélèvement de 49,80€ pour leur service de base . Ils ont utilisé mes données bancaires du paiement de 1,95€ pour faire ce prélèvement J'ai utilisé un lien du site marchand arnaqueur pour me désabonner : j'ai reçu un message pour me dire que je n'aurais pas d'autres prélèvements. J'ai fait une contestation à ma banque. Il s'agit vraiment d'une arnaque !!\n",
        author: 'Léa L.',
      },
      {
        text: "Bonjour, je souhaite faire un signalement concernant la plateforme d'investissement xxx. J'ai effectué un investissement mais la levée de fonds n'a pas eu lieu et ils refusent de me rembourser. Ils gardent la somme et font tout pour ne pas me rembourser. Ils ont même été dire au porteur de projet qu'aucun investisseur ne s'était présenté alors que c'est faux. J'ai retrouvé plusieurs personnes sur un forum qui indiquent avoir été victimes de cette arnaque. Il apparait que cette entreprise est coutumière de ce genre de pratique et détourne l'argent des investisseurs. Je peux vous transmettre des copies d'échanges de mail si nécessaire. Cordialement",
        author: 'Yasmina D.',
      },
      {
        text: 'Signalement pour escroquerie et vol de la part de la société xxx. Plus de 1800 euros de prélèvements abusifs en trois mois et impossible de se faire rembourser. J’ai souscrit un contrat pour des avantages divers auprès de cette société. Les contrats comprenaient deux prélèvements par mois. Or, j’ai sur mon compte cinq prélèvements au moins tous les mois de ces sommes pour chaque société. Également un prélèvement très récurent pour une offre à laquelle je n’ai pas souscris. J’ai déjà épuisé tous les recours possible auprès des sociétés. Impossible d’obtenir réparation. Ils reconnaissent les fautes mais ne me rembourse pas et continue de prélever abusivement sur mon compte sans limites. Le groupe agit ainsi depuis des années en toute impunité et à la connaissance de tous. Des milliers de personnes sont dans mon cas et rien ne change, on se fait tous escroquer.',
        author: 'Noémie I.',
      },
      {
        text: 'Vous pouvez constater qu’il s’agit bien d’une escroquerie réalisée de façon continue dans ce magasin. Comme l’atteste la pièce jointe le ticket de caisse indique 6,50€ pour 1 kg de pain alors qu’en vérité il n’y a que 885 gr de pain. Je vous ai fait plusieurs signalements relevant le mauvais comportement de ce groupe comptant 17 magasins. Le responsable de la Boulangerie de xxx m’a informé que son patron n’installera jamais de balance en caisse alors que la loi lui en fait l’obligation.  Il faut mettre un terme à cette vaste escroquerie qui perdure depuis plus d’une année. J’agis uniquement par intérêt général.',
        author: 'Maria É.',
      },
    ],
  },
  {
    url: 'arnaques-assurance-sante',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: "Arnaques à l'assurance santé",
    seoDescription:
      "Vous êtes victime d'arnaques à l'assurance santé ? Signalez l'incident pour aider à prévenir d'autres victimes.",
    title: "Arnaques à l'Assurance Santé",
    targetedCategory: ['BanqueAssuranceMutuelle'],
    catchPhrase:
      "Signalez un problème d'arnaques à l'assurance santé, renseignez-vous sur vos droits avec la répression des fraudes !",
    secondaryTitle1:
      'Pourquoi signaler une fausse politique, une surcharge non justifiée ou une fausse représentation de la couverture ?',
    secondaryTitle2: "Que devient votre signalement relatif à une arnaque à l'assurance santé ?",
    sampleReports: [
      {
        text: "En février 2023, j'ai été contacté par une société, *****, qui prétendait offrir une couverture d'assurance santé à un prix très bas. Après avoir souscrit, j'ai découvert que la couverture était loin d'être aussi complète qu'ils l'avaient présenté et j'ai été laissé avec des factures médicales élevées.",
        author: 'Noémie Z.',
      },
      {
        text: "En mars 2023, j'ai été approché par ***** qui prétendait offrir une assurance santé abordable. Cependant, après avoir payé la première cotisation, ils sont devenus injoignables. J'ai réalisé que c'était une arnaque après avoir été laissé avec une facture d'hôpital importante et aucune assurance pour la couvrir.",
        author: 'Myriam M.',
      },
      {
        text: "J'ai été victime d'une arnaque à l'assurance santé par *****. Ils ont prétendu que j'étais éligible à une couverture complète à un taux réduit. Cependant, lorsque j'ai eu besoin de soins médicaux, ils ont refusé de couvrir les coûts. J'ai été laissé avec une facture importante et aucune aide.",
        author: 'Salma A.',
      },
      {
        text: "J'ai souscrit à une assurance santé proposée par ***** après qu'ils m'aient assuré qu'elle couvrirait tous mes besoins médicaux. Cependant, après avoir payé les primes, j'ai découvert que la plupart de mes dépenses de santé ne sont pas couvertes. Je me sens trompé et j'ai été laissé avec des factures médicales élevées.",
        author: 'Soukaina Z.',
      },
      {
        text: "J'ai été victime d'une arnaque à l'assurance santé par *****. Ils m'ont vendu une police d'assurance avec la promesse de couvrir toutes mes dépenses de santé. Cependant, quand j'ai eu besoin de soins médicaux, ils ont refusé de payer, me laissant avec des factures énormes à payer.",
        author: 'Jules Y.',
      },
    ],
  },
  {
    url: 'arnaques-garanties-artisans',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Arnaques\n  et problèmes liés aux garanties des artisans',
    seoDescription:
      'Signalez\n  une arnaque ou un problème avec une garantie décennale ou une garantie de\n  parfait achèvement.',
    title: 'Arnaques\n  et problèmes liés aux garanties des artisans',
    targetedCategory: ['TravauxRenovations'],
    catchPhrase:
      'Signalez\n  une difficulté avec une garantie offerte par un artisan, que ce soit une\n  garantie décennale ou une garantie de bon fonctionnement !',
    secondaryTitle1:
      'Pourquoi\n  signaler des problèmes de garanties et des arnaques liées à une couverture\n  inadéquate des artisans ?',
    secondaryTitle2: "Que\n  devient votre signalement d'un problème de garantie ou d'arnaque d'un artisan\n  ?",
    sampleReports: [
      {
        text: "J'ai fait appel à un artisan de la société ***** pour des travaux de rénovation dans ma maison. Malgré la promesse d'une garantie, plusieurs problèmes sont apparus par la suite. J'ai essayé de contacter l'artisan plusieurs fois, mais sans succès. Je suis profondément déçu et je me sens arnaqué.",
        author: 'Jules H.',
      },
      {
        text: "J'ai contracté les services de l'entreprise ***** pour la rénovation de ma salle de bains. Les travaux se sont terminés il y a trois mois et depuis, plusieurs problèmes sont apparus. Malgré une garantie en théorie, l'entreprise fait la sourde oreille à mes appels. Je suis désemparé face à cette situation.",
        author: 'Émile D.',
      },
      {
        text: "J'ai engagé ***** pour des travaux de plomberie dans ma maison. L'artisan a fait preuve d'un manque de professionnalisme évident, et malgré une garantie de deux ans, je ne parviens pas à le contacter pour régler les nombreux problèmes qui sont apparus. Je me sens totalement arnaqué.",
        author: 'Ali R.',
      },
      {
        text: "J'ai eu recours à un artisan de chez ***** pour des travaux de peinture dans mon appartement. Malgré la garantie promise, de nombreux défauts sont apparus sur le travail réalisé. J'ai tenté de contacter l'artisan sans succès. Je suis vraiment insatisfait et je me sens arnaqué.",
        author: 'Yasmina G.',
      },
      {
        text: "J'ai fait appel à un artisan de l'entreprise ***** pour des travaux d'électricité. Malgré une garantie de deux ans, plusieurs problèmes sont survenus. L'artisan ne répond plus à mes appels. C'est une véritable arnaque et je ne sais pas quoi faire.",
        author: 'Ahmed S.',
      },
      {
        text: "J'ai engagé ***** pour des travaux de carrelage dans ma cuisine. Malgré une garantie de cinq ans, des défauts importants sont apparus. J'ai contacté l'entreprise à plusieurs reprises sans réponse. Je me sens arnaqué et déçu par cette situation.",
        author: 'Stella A.',
      },
      {
        text: "J'ai engagé un artisan de l'entreprise ***** pour des travaux de rénovation. Malgré une garantie de trois ans, des problèmes sont apparus. L'artisan ne répond plus à mes appels. Je suis frustré et je me sens victime d'une arnaque.",
        author: 'Fatima N.',
      },
      {
        text: "J'ai fait appel à un artisan de chez ***** pour des travaux de maçonnerie. Malgré une garantie en théorie, de nombreux problèmes sont apparus par la suite. J'ai essayé de contacter l'artisan sans succès. Je me sens arnaqué et je ne sais pas comment résoudre ce problème.",
        author: 'Axel S.',
      },
    ],
  },
  {
    url: 'arnaques-pose-panneaux-photovoltaiques',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Arnaques\n  et problèmes liés à la pose de panneaux photovoltaïques',
    seoDescription:
      "Signalez\n  un problème d'arnaque, de malfaçon ou de performance insatisfaisante lors de\n  la pose de panneaux photovoltaïques.",
    title: 'Arnaques\n  et problèmes liés à la pose de panneaux photovoltaïques',
    targetedCategory: ['TravauxRenovations'],
    catchPhrase:
      "Signalez\n  un problème d'arnaque, de malfaçon ou de performance insatisfaisante lors de\n  la pose de panneaux photovoltaïques, renseignez-vous sur vos droits avec la\n  répression des fraudes !",
    secondaryTitle1:
      'Pourquoi\n  signaler des arnaques, malfaçons ou problèmes de performance liés à la pose\n  de panneaux photovoltaïques ?',
    secondaryTitle2:
      'Que devient votre signalement relatif\n  aux arnaques, malfaçons ou problèmes de performance liés à la pose de\n  panneaux photovoltaïques ?',
    sampleReports: [
      {
        text: "J'ai fait appel à ***** pour l'installation de panneaux photovoltaïques sur le toit de ma maison. Ils m'ont promis des économies importantes sur mes factures d'énergie. Malheureusement, je n'ai constaté aucune amélioration après plusieurs mois. De plus, ils ont endommagé le toit pendant l'installation et refusent de prendre en charge les réparations. Je me sens complètement trompé.",
        author: 'Soukaina H.',
      },
      {
        text: "J'ai contacté ***** pour l'installation de panneaux photovoltaïques. Malheureusement, depuis l'installation, je fais face à des problèmes continus avec le système. De plus, ils ne répondent pas à mes demandes de service après-vente. Je me sens abandonné et déçu par leur manque de professionnalisme.",
        author: 'Amira J.',
      },
      {
        text: "J'ai fait appel à ***** pour installer des panneaux photovoltaïques sur ma maison. Cependant, le rendement promis n'a pas été atteint et ils refusent d'assumer la responsabilité. De plus, ils ont laissé ma propriété dans un état déplorable après l'installation. Je me sens trompé et négligé.",
        author: 'Leila T.',
      },
      {
        text: "J'ai embauché ***** pour l'installation de panneaux photovoltaïques. Malheureusement, ils ont effectué un travail de mauvaise qualité et le système ne fonctionne pas correctement. De plus, ils ont refusé de rectifier les problèmes. Je suis très déçu et je me sens arnaqué.",
        author: 'Sarah C.',
      },
      {
        text: "J'ai sollicité ***** pour l'installation de panneaux photovoltaïques. Malheureusement, les panneaux installés ne produisent pas la quantité d'énergie promise. De plus, ils ont endommagé le toit pendant l'installation et ne veulent pas réparer les dégâts. Je me sens délaissé et trompé.",
        author: 'Nadia M.',
      },
      {
        text: "J'ai fait appel à ***** pour l'installation de panneaux photovoltaïques. Cependant, ils ont mal installé les panneaux, ce qui a entraîné des problèmes d'infiltration d'eau. Malgré mes multiples demandes, ils n'ont pas résolu le problème. Je me sens complètement arnaqué.",
        author: 'Mohamed A.',
      },
      {
        text: "J'ai contacté ***** pour l'installation de panneaux photovoltaïques. Ils ont effectué un travail de mauvaise qualité et ont laissé des débris sur mon terrain. De plus, ils refusent de revenir pour nettoyer ou corriger leur travail. Je me sens complètement négligé.",
        author: 'Théo S.',
      },
      {
        text: "J'ai engagé ***** pour installer des panneaux photovoltaïques sur mon toit. Depuis l'installation, je rencontre de nombreux problèmes avec le système et ils ne répondent pas à mes demandes de réparation. De plus, l'installation a causé des dommages à mon toit. Je suis extrêmement déçu et me sens complètement trompé.",
        author: 'Noémie A.',
      },
    ],
  },
  {
    url: 'arnaques-primes-energie-renovation',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: "Arnaques\n  et problèmes liés aux primes d'énergie et aux aides à la rénovation",
    seoDescription:
      "Signalez\n  un problème d'arnaque, de non-attribution ou de détournement lié aux primes\n  d'énergie ou aux aides à la rénovation.",
    title: "Arnaques\n  et problèmes liés aux primes d'énergie et aux aides à la rénovation",
    targetedCategory: ['TravauxRenovations'],
    catchPhrase:
      "Signalez\n  un problème d'arnaque, de non-attribution ou de détournement lié aux primes\n  d'énergie ou aux aides à la rénovation, renseignez-vous sur vos droits avec\n  la répression des fraudes !",
    secondaryTitle1:
      "Pourquoi\n  signaler des arnaques, problèmes de non-attribution ou détournements liés aux\n  primes d'énergie et aux aides à la rénovation ?",
    secondaryTitle2:
      "Que\n  devient votre signalement relatif aux arnaques, problèmes de non-attribution\n  ou détournements liés aux primes d'énergie et aux aides à la rénovation ?",
    sampleReports: [
      {
        text: "J'ai fait une demande de prime d'énergie auprès de *****. Cependant, malgré plusieurs relances, je n'ai toujours pas reçu le montant promis. Je me sens trompé et délaissé.",
        author: 'Emma A.',
      },
      {
        text: "J'ai contacté ***** pour bénéficier d'une aide à la rénovation. Malheureusement, malgré de multiples promesses, je n'ai toujours pas reçu de prime. Je me sens ignoré et arnaqué.",
        author: 'Hassan M.',
      },
      {
        text: "J'ai demandé une prime d'énergie auprès de *****. Malgré plusieurs engagements de leur part, je n'ai toujours pas reçu le montant promis. C'est un véritable scandale.",
        author: 'Mohamed M.',
      },
      {
        text: "J'ai sollicité ***** pour une subvention d'aide à la rénovation de ma maison. Malheureusement, malgré plusieurs promesses, aucune aide n'a été accordée. Je me sens complètement lésé.",
        author: 'Mehdi J.',
      },
      {
        text: "J'ai demandé une prime d'énergie auprès de *****. Malheureusement, malgré de multiples engagements, je n'ai toujours pas reçu le montant promis. Je me sens délaissé et trompé.",
        author: 'Maria A.',
      },
      {
        text: "J'ai contacté ***** pour bénéficier d'une aide à la rénovation. Cependant, malgré plusieurs promesses, je n'ai reçu aucune aide. Je me sens totalement ignoré.",
        author: 'Michel N.',
      },
      {
        text: "J'ai fait une demande de prime d'énergie auprès de *****. Cependant, malgré plusieurs engagements de leur part, je n'ai toujours pas reçu le montant promis. C'est extrêmement frustrant.",
        author: 'Rachid A.',
      },
      {
        text: "J'ai sollicité ***** pour une aide à la rénovation de ma maison. Malheureusement, malgré plusieurs promesses, aucune aide n'a été accordée. Je me sens complètement lésé.",
        author: 'Rayan L.',
      },
    ],
  },
  {
    url: 'arnaques-reparation-appareils-electromenagers',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: "Arnaques\n  et problèmes liés aux services de réparation d'appareils électroménagers",
    seoDescription: "Signalez\n  un problème d'arnaque, de surcoût ou de réparation inefficace d'appareils\n  électroménagers.",
    title: "Arnaques\n  et problèmes liés aux services de réparation d'appareils électroménagers",
    targetedCategory: ['TravauxRenovations'],
    catchPhrase:
      "Signalez\n  un problème d'arnaque, de surcoût ou de réparation inefficace d'appareils\n  électroménagers, renseignez-vous sur vos droits avec la répression des\n  fraudes !",
    secondaryTitle1:
      "Pourquoi\n  signaler des arnaques, surcoûts ou problèmes de réparation inefficace\n  d'appareils électroménagers ?",
    secondaryTitle2:
      "Que\n  devient votre signalement relatif aux arnaques, surcoûts ou problèmes de\n  réparation d'appareils électroménagers ?",
    sampleReports: [
      {
        text: "J'ai fait appel à ***** pour la réparation de mon lave-vaisselle. Non seulement l'appareil n'est pas réparé, mais le service après-vente se montre désormais injoignable. Je me sens complètement abandonné avec un appareil toujours en panne.",
        author: 'Salma Z.',
      },
      {
        text: "J'ai confié mon réfrigérateur à l'entreprise ***** pour une réparation. Cependant, le frigo est revenu dans un état pire qu'avant et l'entreprise refuse maintenant de prendre en compte mes plaintes. C'est un véritable scandale.",
        author: 'Timothée I.',
      },
      {
        text: "J'ai fait appel à ***** pour la réparation de mon four. Malheureusement, le four ne fonctionne toujours pas correctement et l'entreprise ne répond plus à mes appels. Je me sens délaissé et arnaqué.",
        author: 'Michel S.',
      },
      {
        text: "J'ai engagé ***** pour réparer mon lave-linge. Non seulement l'appareil n'est pas réparé, mais l'entreprise ne prend pas la peine de répondre à mes plaintes. C'est extrêmement frustrant.",
        author: 'Noémie G.',
      },
      {
        text: "J'ai fait confiance à ***** pour réparer mon aspirateur. Cependant, non seulement l'appareil n'est pas réparé, mais l'entreprise ne répond plus à mes appels. Je me sens complètement arnaqué.",
        author: 'Myriam H.',
      },
      {
        text: "J'ai engagé l'entreprise ***** pour la réparation de mon micro-ondes. Malheureusement, l'appareil est revenu dans un état pire qu'avant et l'entreprise refuse de prendre ses responsabilités. C'est un véritable cauchemar.",
        author: 'Clara É.',
      },
      {
        text: "J'ai confié mon grille-pain à ***** pour une réparation. Cependant, non seulement l'appareil n'est pas réparé, mais l'entreprise refuse de répondre à mes plaintes. Je me sens délaissé et trahi.",
        author: 'Alexandre É.',
      },
      {
        text: "J'ai engagé ***** pour la réparation de mon robot de cuisine. Cependant, l'appareil n'a pas été réparé et l'entreprise ne répond plus à mes appels. Je me sens complètement lésé.",
        author: 'Élias L.',
      },
    ],
  },
  {
    url: 'blocage-carte-bancaire-sans-motif',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Blocage de votre carte bancaire sans motif',
    seoDescription: 'Votre carte bancaire a-t-elle été bloquée sans motif ? Signalez le problème pour obtenir une résolution.',
    title: 'Blocage de Carte Bancaire sans Motif',
    targetedCategory: ['BanqueAssuranceMutuelle'],
    catchPhrase:
      'Signalez un problème de blocage de votre carte bancaire sans motif, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi signaler un blocage de carte non justifié, un accès refusé ou une restriction injustifiée ?',
    secondaryTitle2: 'Que devient votre signalement relatif à un blocage de carte bancaire sans motif ?',
    sampleReports: [
      {
        text: "J'ai été surpris d'apprendre que ma carte bancaire avec ***** a été bloquée sans aucune raison. J'ai contacté le service client, mais je n'ai reçu aucune explication satisfaisante.",
        author: 'Mohamed S.',
      },
      {
        text: "J'ai essayé d'utiliser ma carte bancaire avec *****, mais elle a été refusée. Lorsque j'ai contacté la banque, ils ont dit qu'elle avait été bloquée, mais n'ont pas pu fournir de raison pour ce blocage.",
        author: 'Léa L.',
      },
      {
        text: "Récemment, ma carte bancaire ***** a été bloquée sans aucun motif valable. C'est très frustrant car je suis incapable de faire des achats ou de retirer de l'argent.",
        author: 'Adam M.',
      },
      {
        text: "Ma carte bancaire avec ***** a été bloquée sans aucune explication. C'est très gênant et perturbe grandement mes transactions quotidiennes.",
        author: 'Timothée A.',
      },
      {
        text: "J'ai tenté d'effectuer une transaction avec ma carte bancaire chez *****, mais elle a été refusée. Après avoir contacté la banque, j'ai découvert qu'elle avait été bloquée sans raison apparente.",
        author: 'Dylan A.',
      },
    ],
  },
  {
    url: 'blocage-compte-bancaire-injustifie',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Blocage de compte bancaire injustifié',
    seoDescription:
      'Votre compte bancaire a-t-il été bloqué injustement ? Signalez le problème pour obtenir une résolution rapide.',
    title: 'Blocage de Compte Bancaire Injustifié',
    targetedCategory: ['BanqueAssuranceMutuelle'],
    catchPhrase:
      'Signalez un problème de blocage de compte bancaire injustifié, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi signaler un blocage de compte, un accès refusé ou une restriction injustifiée ?',
    secondaryTitle2: 'Que devient votre signalement relatif à un blocage injustifié de compte bancaire ?',
    sampleReports: [
      {
        text: "En avril 2023, ma banque, *****, a bloqué mon compte sans explication. Malgré mes nombreuses tentatives pour les contacter et comprendre la raison de ce blocage, je n'ai reçu aucune réponse satisfaisante. Je suis incapable d'accéder à mes fonds et cela m'a causé beaucoup de stress et de difficultés financières.",
        author: 'Élias S.',
      },
      {
        text: "En juin 2023, sans avertissement, ***** a bloqué mon compte bancaire. Je suis resté sans accès à mes fonds pendant des semaines. Malgré de nombreux appels et courriers électroniques, je n'ai reçu aucune explication claire sur les raisons de ce blocage.",
        author: 'Amine N.',
      },
      {
        text: "Mon compte chez ***** a été bloqué sans motif en mai 2023. Toutes mes tentatives pour comprendre la raison de ce blocage sont restées vaines. Cela a eu un impact sérieux sur ma vie, car je suis incapable de payer mes factures et d'accéder à mes économies.",
        author: 'Juliette A.',
      },
      {
        text: "En mars 2023, j'ai reçu un message de ***** m'informant que mon compte a été bloqué. Malgré de multiples appels et messages, je n'ai pas pu obtenir une explication claire. Je suis dans une situation financière précaire à cause de ce blocage injustifié.",
        author: 'Sofia M.',
      },
      {
        text: "Mon compte chez ***** a été soudainement bloqué en juin 2023 sans aucune explication. J'ai contacté à plusieurs reprises la banque, mais aucune raison valable n'a été fournie. Ce blocage a causé de graves problèmes financiers et a perturbé mon quotidien.",
        author: 'Ali L.',
      },
    ],
  },
  {
    url: 'complications-livraison-nourriture',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Complications\n  lors de la livraison de nourriture',
    seoDescription: "Signalez\n  un problème de livraison de nourriture d'un restaurant",
    title: 'Complications\n  lors de la livraison de nourriture',
    targetedCategory: ['CafeRestaurant'],
    catchPhrase: "Signalez\n  un problème de livraison de nourriture d'un restaurant, votre expérience\n  compte !",
    secondaryTitle1: 'Pourquoi\n  signaler une complication lors de la livraison de nourriture ?',
    secondaryTitle2: "Que devient votre signalement d'une\n  complication lors de la livraison de nourriture ?",
    sampleReports: [
      {
        text: "J'ai commandé de la nourriture chez ***** et la livraison a été un véritable cauchemar. Non seulement la commande est arrivée en retard, mais la nourriture était froide et les articles manquaient. C'est la dernière fois que je commande chez eux.",
        author: 'Fatima M.',
      },
      {
        text: "J'ai commandé un repas chez ***** et j'ai été déçu par leur service de livraison. La commande est arrivée une heure en retard et plusieurs articles étaient incorrects. Je suis vraiment déçu par leur manque de professionnalisme.",
        author: 'Chloé L.',
      },
      {
        text: "J'ai eu une expérience décevante avec le service de livraison de *****. Mon repas est arrivé avec une heure de retard et plusieurs plats étaient erronés. C'est vraiment frustrant.",
        author: 'Ahmed R.',
      },
      {
        text: "J'ai vécu une expérience négative avec le service de livraison de *****. Non seulement la livraison a été très retardée, mais en plus plusieurs articles étaient manquants dans ma commande. C'est inacceptable.",
        author: 'Maria G.',
      },
      {
        text: "La livraison de nourriture de chez ***** a été une véritable calamité. Non seulement mon repas est arrivé en retard, mais de plus, il manquait des articles. Leur service client n'a pas été à la hauteur.",
        author: 'Théo M.',
      },
      {
        text: "J'ai eu une expérience très frustrante avec la livraison de nourriture chez *****. La commande était en retard et plusieurs articles étaient manquants. Leur service client n'a pas été réactif.",
        author: 'Noémie A.',
      },
    ],
  },
  {
    url: 'cpf-formation',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Arnaque CPF',
    seoDescription:
      'Votre problème concerne un démarchage, une arnaque CPF Compte Personnel de Formation par téléphone, mail ou SMS',
    title: 'Arnaque ou escroquerie CPF',
    targetedCategory: ['ServicesAuxParticuliers'],
    catchPhrase: 'Signalez une arnaque au CPF',
    secondaryTitle1: 'Pourquoi signaler une arnaque au CPF ?',
    sampleReports: [
      {
        text: "Je suis harcelée par un numéro commençant par 02 xx xx xx xx (6 appels aujourd'hui, sans compter les jours précédents). Objet : le CPF. Je bloque tous les numéros commençant par ces chiffres. Ils continuent de m'appeler. Je leur ai déjà dit que je n'étais pas intéressée à deux reprises et ils continuent !\n",
        author: 'Émile A.',
      },
      {
        text: 'Bonsoir juste aujourd’hui j’ai reçu 16 appels à propos du compte CPF cela devient très gênant, 16 appels en une journée alors que je suis inscrit sur bloctel, ceci est mon numéro de téléphone professionnel cela devient vraiment gênant de recevoir autant d’appel d’arnaque au CPF.',
        author: 'Emma S.',
      },
      {
        text: "Appel téléphonique du +33 x xx xx xx xx concernant mon compte CPF. Se présente comme étant conseiller centre formation que je n'ai jamais sollicité. Voulait me poser des questions sur mon CPF mais m'a immédiatement raccroché au nez dès que je lui ai dit que je gérais mes affaires seule directement sur mon compte en ligne.",
        author: 'Margaux L.',
      },
      {
        text: "Spam reçu par SMS, le 31/05/2023 à 9h46, du numéro +33x xx xx xx xx, que je ne connais pas. Contenu de l'escroquerie au CPF, dont je ne suis pas concerné : ACTUALISATION 2023 ATTENTION: C'est bientôt la fin des formations 100% financées par l'Etat. Convertissez vos droits et bénéficiez d’une formation intégralement prise en charge avant l’entrée en vigueur d’un reste à payer. ",
        author: 'Dylan M.',
      },
      {
        text: "Appels incessants toute la journée concernant mon CPF pour lequel mon solde est à zéro Malgré le fait que je leur demande d'arrêter et que je leur indique que je suis inscrit sur bloctel, cela continue. Ci-joint une capture d'écran de mon historique d'appels. Merci",
        author: 'Ayoub S.',
      },
      {
        text: "Bonjour, je reçois des appels du numéro suivant +33 x xx xx xx xx alors que mon numéro est inscrit sur Bloctel. J'ai demandé plusieurs fois à mon interlocutrice de me préciser son employeur, ce qu'elle a refusé de faire. Elle prétend être un conseil professionnel lié au CPF (www.moncompteformation.gouv.fr), ce qu'elle n'est pas évidemment puisqu'elle a refusé de s'identifier. Son argumentaire semble très bien rodé, et les victimes pourraient être nombreuses.",
        author: 'Chloé C.',
      },
    ],
  },
  {
    url: 'declarations-inexactes-produits-alimentaires',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Déclarations\n  inexactes sur les produits alimentaires',
    seoDescription: 'Signalez\n  une déclaration inexacte sur un produit alimentaire',
    title: 'Déclarations\n  inexactes sur les produits alimentaires',
    targetedCategory: ['CafeRestaurant'],
    catchPhrase: 'Signalez\n  une déclaration inexacte sur un produit alimentaire, exigez la transparence !',
    secondaryTitle1: 'Pourquoi\n  signaler une déclaration inexacte sur un produit alimentaire ?',
    secondaryTitle2: "Que\n  devient votre signalement d'une déclaration inexacte sur un produit\n  alimentaire ?",
    sampleReports: [
      {
        text: "J'ai acheté un produit chez ***** qui indiquait qu'il était sans gluten. Après avoir consommé le produit, j'ai eu une réaction allergique, car le produit contenait en réalité du gluten. Leur déclaration inexacte a mis ma santé en danger.",
        author: 'Clara M.',
      },
      {
        text: "J'ai récemment acheté un produit chez ***** qui était étiqueté comme étant bio. Après avoir fait des recherches sur l'entreprise, j'ai découvert que leurs produits ne sont pas vraiment bio. C'est trompeur et totalement inacceptable.",
        author: 'Margaux A.',
      },
      {
        text: "J'ai récemment acheté un produit chez ***** qui prétendait être bio. Après des recherches, j'ai découvert que leurs produits ne sont pas vraiment bio. C'est décevant et franchement malhonnête.",
        author: 'Émile J.',
      },
      {
        text: 'Chez *****, j\'ai acheté un produit étiqueté comme "naturel", seulement pour découvrir plus tard que le produit contenait des additifs artificiels. C\'est un cas clair de déclarations inexactes sur les produits alimentaires.',
        author: 'Chloé A.',
      },
      {
        text: 'Chez *****, un produit était commercialisé comme étant "bio" alors qu\'il contenait plusieurs ingrédients non biologiques. Il s\'agit là de déclarations inexactes sur les produits alimentaires.',
        author: 'Sofia L.',
      },
      {
        text: 'Le produit que j\'ai acheté chez ***** était étiqueté comme "100% naturel", alors que la liste des ingrédients indiquait plusieurs composants artificiels. C\'est une déclaration inexacte sur les produits alimentaires.',
        author: 'Alexandre S.',
      },
    ],
  },
  {
    url: 'defaillance-chaine-froid-restaurants',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Défaillance\n  de la chaîne du froid dans les restaurants',
    seoDescription: 'Signalez\n  un problème de défaillance de la chaîne du froid dans un restaurant',
    title: 'Défaillance\n  de la chaîne du froid dans les restaurants',
    targetedCategory: ['CafeRestaurant'],
    catchPhrase: 'Signalez\n  un problème de défaillance de la chaîne du froid dans un restaurant,\n  préservez votre santé !',
    secondaryTitle1: 'Pourquoi\n  signaler une défaillance de la chaîne du froid dans un restaurant ?',
    secondaryTitle2: "Que\n  devient votre signalement d'une défaillance de la chaîne du froid dans un\n  restaurant ?",
    sampleReports: [
      {
        text: "J'ai commandé un repas chez ***** et j'ai reçu un plat dont la viande n'était pas correctement réfrigérée. Cela m'a non seulement rendu malade, mais cela montre aussi un manque flagrant de respect pour les normes de sécurité alimentaire. Ils doivent être tenus responsables de leurs actions.",
        author: 'Chloé I.',
      },
      {
        text: "Lors de ma dernière visite chez *****, la salade que j'ai commandée avait une drôle de couleur et une odeur désagréable. Il est évident que la chaîne du froid a été rompue à un moment donné, ce qui représente un risque sérieux pour la santé.",
        author: 'Mathis R.',
      },
      {
        text: "La dernière fois que j'ai dîné chez *****, la salade que j'ai commandée avait une couleur et une odeur suspectes. Il est clair qu'une rupture de la chaîne du froid a eu lieu, ce qui pose un sérieux risque sanitaire.",
        author: 'Léa N.',
      },
      {
        text: "Chez *****, j'ai commandé un plat de fruits de mer qui n'avait clairement pas été conservé à la bonne température. Il est impératif qu'ils revoient leur gestion de la chaîne du froid pour prévenir toute intoxication alimentaire.",
        author: 'Zahra T.',
      },
      {
        text: "En mangeant chez *****, j'ai reçu un plat de poisson qui était tiède, signe d'une rupture de la chaîne du froid. Un tel manque de respect des règles de sécurité alimentaire est alarmant.",
        author: 'Soukaina C.',
      },
      {
        text: "Chez *****, j'ai commandé un plat de fruits de mer qui m'est arrivé tiède, une indication flagrante d'une défaillance dans la chaîne du froid. C'est très préoccupant.",
        author: 'Timothée T.',
      },
    ],
  },
  {
    url: 'demarchage-telephonique',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Démarchage téléphonique',
    seoDescription: 'Signalez un problème de démarchage téléphonique',
    title: 'Démarchage téléphonique',
    targetedCategory: ['DemarchageAbusif'],
    catchPhrase:
      'Signalez un problème de démarchage téléphonique, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi faire un signalement si vous êtes victime de démarchage téléphonique ?',
    secondaryTitle2: 'Que devient votre signalement relatif à du démarchage téléphonique abusif ?',
    sampleReports: [
      {
        text: 'Date du premier appel\n02/06/2023\nDate du second appel\n31/05/2023\nDate du troisième appel\n30/05/2023\nDate du quatrième appel\n27/05/2023\nDate du cinquième appel\n20/05/2023\nAppels incessants jusqu’à 2 à 3 fois par jour, avec plusieurs numéros, appels même en masqué, envoi des courriers à ma compagne alors que nous ne vivons pas à la même adresse (2 courriers reçu le même jour)',
        author: 'Émile S.',
      },
      {
        text: "Je reçois 2 à 5 appels par jour venant du bureau d'études ... J'arrête pas de les bloquer ils me rappellent avec d'autres numéros commençant part 06 ou 07. Je leur dis que je suis pas intéressé mais ils insistent. J'ai prévenu mon opérateur téléphonique mais ils ne peuvent rien faire de plus.",
        author: 'Clara A.',
      },
      {
        text: "Ce numéro me téléphone tous les jours, je ne réponds plus, c'est du harcèlement\n",
        author: 'Chloé N.',
      },
      {
        text: 'L’entreprise xxx chez laquelle j’ai un contrat d’électricité me harcèle. C’est jusqu’à 4 appels par jour depuis plus de 15 jours. Je bloque les numéros et tous les jours de nouveaux numéros me téléphonent et c’est encore eux. Malgré mes nombreux refus de démarchage, on continue de m’appeler en faisant du forcing. C’est invivable.',
        author: 'Louis M.',
      },
      {
        text: "Depuis le 25 mai, je fais l'objet d'un démarchage commercial pour le CPF par 4 lignes téléphoniques différentes, dont le 09 xx xx xx xx. 12 appels en 1 semaine par cette \"entreprise\". J'ai demandé d'être rayée de la liste d'appels. La personne m'a répondu que ce n'était pas possible. Je lui ai dit que s'il continuait de m'appeler, je portais plainte.",
        author: 'Rachid M.',
      },
    ],
  },
  {
    url: 'distribution-gratuite-bouteilles-plastique',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Distribution gratuite de bouteilles en plastique contenant des boissons',
    seoDescription: 'Signalez une distribution gratuite de bouteilles en plastique dans un café ou un restaurant',
    title:
      'Distribution gratuite de bouteilles en plastique contenant des boissons dans les établissements recevant du public et dans les locaux à usage professionnel',
    targetedCategory: ['CafeRestaurant'],
    catchPhrase: 'Signalez un problème de distribution gratuite de bouteilles en plastique contenant des boissons',
    secondaryTitle1: 'Pourquoi signaler une distribution gratuite de bouteilles en plastique dans un café ou un restaurant ?',
    secondaryTitle2: 'Que devient votre signalement relatif à une distribution gratuite de bouteilles en plastique ?',
    sampleReports: [
      {
        text: "L'établissement nous a posé 3 bouteilles d'eau en plastique alors qu'on avait rien demandé !",
        author: 'Juliette H.',
      },
      {
        text: "Le serveur nous a spontanément fourni une bouteilles d'eau en plastique sans que nous l'ayons sollicitée et gratuite en plus !",
        author: 'Nour N.',
      },
      {
        text: "Le serveur nous a offert une bouteille d'eau en plastique de sa propre initiative, sans que nous en ayons fait la demande, et en plus, gratuitement !",
        author: 'Rachid L.',
      },
    ],
  },
  {
    url: 'duree-de-vie-produit-obsolescence-programmee-internet',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Duree-de-vie-produit-obsolescence-programmee-internet',
    seoDescription:
      'Signalez un appareil acheté sur internet dont la durée de vie vous semble trop courte et qui pourrait s’apparenter à de l’obsolescence programmée. ',
    title: 'Obsolescence programmée : votre appareil acheté sur interne a une durée de vie qui vous semble trop courte ',
    targetedCategory: ['AchatInternet'],
    catchPhrase:
      'Signalez un appareil acheté sur interne dont la durée de vie vous semble trop courte et pourrait s’apparenter à de l’obsolescence programmée et contribuez ainsi à préserver notre environnement pour les générations futures.',
    secondaryTitle1:
      'Pourquoi signaler un appareil acheté sur interne dont la durée de vie vous semble trop courte et pourrait s’apparenter à de l’obsolescence programmée ?',
    secondaryTitle2:
      "Que devient votre signalement relatif à l'obsolescence programmée d'un appareil acheté sur interne dont la durée de vie vous semble trop courte ?",
    sampleReports: [
      {
        text: "Je suis déçu par l'achat récent d'un téléphone portable qui semble être sujet à une obsolescence programmée. Malgré les promesses de durabilité et de qualité de votre entreprise, le téléphone que j'ai acheté présente des signes évidents de défaillance prématurée, notamment des problèmes de batterie et de performance, à peine quelques mois après l'achat.\n\nCette situation est non seulement frustrante en tant que consommateur, mais elle soulève également des préoccupations sérieuses quant à la politique de votre entreprise en matière de durabilité et de respect des droits des consommateurs. L'obsolescence programmée est une pratique déloyale et anti-éthique qui nuit à la fois aux clients et à l'environnement, en encourageant la surconsommation et en générant une quantité excessive de déchets électroniques.",
        author: 'Noémie A.',
      },
      {
        text: "Je souhaiterais exprimer ma déception et ma préoccupation concernant l'achat récent d'un ordinateur portable sur votre boutique en ligne. Malgré les affirmations selon lesquelles vos produits sont conçus pour être durables et respectueux de l'environnement, à peine la fin de la garantie passée, mon ordinateur dysfonctionne. \nImpossible d'obtenir de l'aide de la part de votre service après vente. Pire encore... la pièce défectueuse ne peut pas être changée. non seulement elle est dans un boitier scellé mais en plus le composant n'est plus fabriqué par la société. Merci l'obsolescence programmée ! \nL'ordinateur rencontre des problèmes de performances et de fonctionnement. Cette situation est frustrante mais elle remet également en question l'engagement de votre entreprise envers l'éthique des produits électroniques.",
        author: 'Jade H.',
      },
    ],
  },
  {
    url: 'dysfonctionnements-commandes-nourriture',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Dysfonctionnements\n  des commandes en ligne de nourriture',
    seoDescription: 'Signalez\n  un dysfonctionnement avec une commande en ligne de nourriture',
    title: 'Dysfonctionnements\n  des commandes en ligne de nourriture',
    targetedCategory: ['CafeRestaurant'],
    catchPhrase: 'Signalez\n  un dysfonctionnement avec une commande en ligne de nourriture, assurez un\n  service de qualité !',
    secondaryTitle1: 'Pourquoi\n  signaler un dysfonctionnement de commande en ligne de nourriture ?',
    secondaryTitle2: "Que\n  devient votre signalement d'un dysfonctionnement de commande en ligne de\n  nourriture ?",
    sampleReports: [
      {
        text: "J'ai passé une commande en ligne chez ***** et ma commande n'est jamais arrivée. J'ai contacté le service client à plusieurs reprises, mais ils n'ont pas réussi à résoudre le problème. C'est très frustrant et inacceptable.",
        author: 'Théo G.',
      },
      {
        text: "J'ai passé une commande en ligne chez ***** et non seulement ma commande est arrivée en retard, mais plusieurs articles manquaient également. J'ai contacté leur service clientèle, mais le problème n'a pas été résolu.",
        author: 'Salma L.',
      },
      {
        text: "J'ai commandé en ligne chez ***** et ma commande est arrivée tard et incomplète. J'ai contacté le service client, mais le problème n'est toujours pas résolu.",
        author: 'Nathan S.',
      },
      {
        text: "Ma dernière commande en ligne chez ***** a été une expérience frustrante. Le repas est arrivé tard et plusieurs plats manquaient. Le service client n'a pas été en mesure de résoudre le problème.",
        author: 'Michel H.',
      },
      {
        text: "J'ai passé une commande en ligne avec *****, et l'expérience a été très décevante. Mon repas a été livré en retard et des plats étaient manquants. Leur service client était inutile.",
        author: 'Hassan R.',
      },
      {
        text: 'Ma récente commande en ligne de repas chez ***** a été un désastre total. La livraison était en retard et de plus, certains plats étaient manquants. Leur service à la clientèle a été insuffisant.',
        author: 'Axel N.',
      },
    ],
  },
  {
    url: 'emballage-plastique-trop-grand-internet',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Utilisation d’un emballage plastique trop grand comparé à la quantité de produit emballée acheté sur internet',
    seoDescription:
      'Signalez l’utilisation d’un emballage plastique trop grand par rapport à la quantité de produit emballée acheté sur internet',
    title: 'Utilisation d’un emballage plastique trop grand par rapport à la quantité de produit emballée acheté sur internet',
    targetedCategory: ['AchatInternet'],
    catchPhrase:
      'Signalez l’utilisation d’un emballage plastique trop grand par rapport à la quantité de produit emballée acheté sur internet et contribuez ainsi à préserver notre environnement pour les générations futures.',
    secondaryTitle1:
      'Pourquoi signaler l’utilisation d’un emballage plastique trop grand par rapport à la quantité de produit emballée ?',
    secondaryTitle2:
      'Que devient votre signalement relatif à l’utilisation d’un emballage plastique trop grand par rapport à la quantité de produit emballée ?',
    sampleReports: [
      {
        text: "Je tiens à exprimer mon mécontentement concernant un récent achat effectué sur votre boutique en ligne. J'ai commandé un ensemble de bouteilles réutilisables dans le cadre de mon engagement envers un mode de vie sans plastique, cependant, à ma grande déception, ces bouteilles ont été livrées dans un emballage plastique aussi facultatif que démesuré par rapport à leur taille.\nL'utilisation d'un emballage plastique inutile et disproportionné pour emballer un produit qui, par définition, vise à réduire la consommation de plastique, est non seulement ironique mais également préjudiciable pour l'environnement. Je suis profondément déçu de constater un tel gaspillage de ressources et une contribution inutile à la pollution plastique.\nMerci d'être cohérent et d'adopter des solutions plus respectueuses de l'environnement, telles que l'utilisation de matériaux d'emballage recyclables, compostables ou réutilisables. ",
        author: 'Mehdi J.',
      },
      {
        text: "Je souhaite porter à votre attention une préoccupation concernant un récent achat que j'ai effectué sur votre boutique en ligne. J'ai commandé un ensemble d'ustensiles de cuisine en acier inoxydable, mais à ma grande déception, ces articles étaient enveloppés dans un emballage plastique excessif qui ne correspondait pas du tout à la taille des produits. Chaque ustensile était emballé individuellement et tous étaiuent ensuite emballés dans un sac plastique. Le tout, pour une protection optimale rangé dans une boîte.... roulement de tambour en plastique !\nL'utilisation d'un emballage plastique disproportionné et inutile est non seulement écologiquement irresponsable, mais également contraire aux valeurs environnementales que votre boutique en ligne prétend promouvoir. Je suis profondément déçue de constater un tel gaspillage de plastique.\nJe vous encourage vivement à revoir vos pratiques d'emballage et à opter pour des solutions plus respectueuses de l'environnement, telles que l'utilisation de matériaux d'emballage recyclables ou réutilisables. En réduisant votre utilisation de plastique superflu, vous contribuerez non seulement à la préservation de l'environnement, mais vous renforcerez également la confiance des consommateurs dans votre engagement en faveur d'une consommation plus durable.",
        author: 'Rayan M.',
      },
    ],
  },
  {
    url: 'emballage-plastique-trop-grand-magasin',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Utilisation d’un emballage plastique trop grand comparé à la quantité de produit emballée dans un magasin',
    seoDescription:
      'Signalez l’utilisation d’un emballage plastique trop grand par rapport à la quantité de produit emballée dans un magasin',
    title: 'Utilisation d’un emballage plastique trop grand par rapport à la quantité de produit emballée dans un magasin',
    targetedCategory: ['AchatMagasin'],
    catchPhrase:
      'Signalez l’utilisation d’un emballage plastique trop grand par rapport à la quantité de produit emballée dans un magasin et contribuez ainsi à préserver notre environnement pour les générations futures.',
    secondaryTitle1:
      'Pourquoi signaler l’utilisation d’un emballage plastique trop grand par rapport à la quantité de produit emballée ?',
    secondaryTitle2:
      'Que devient votre signalement relatif à l’utilisation d’un emballage plastique trop grand par rapport à la quantité de produit emballée ?',
    sampleReports: [
      {
        text: "En me rendant dans votre boucherie, j'ai été consterné de voir que la plupart des produits étaient emballés dans des films plastiques beaucoup trop grands par rapport à la quantité de viande à l'intérieur. Cette utilisation excessive de plastique est non seulement nuisible pour l'environnement, mais aussi inutile pour les consommateurs. En tant que client, je demande que des mesures soient prises pour adopter des emballages plus responsables !",
        author: 'Mohamed L.',
      },
      {
        text: "Lors de ma dernière visite dans votre boulangerie, j'ai été frappé par l'utilisation d'emballages plastiques excessivement grands pour emballer des portions individuelles de pâtisseries ou de viennoiseries. Par exemple, vos croissants individuels étaient emballés dans les mêmes sacs plastiques que vos lots familiaux. C'est beaucoup trop grand ! Quel gaspillage de plastique ! Étant soucieuse de l'environnement, je suis préoccupée par cette pratique et j'aimerais voir des changements dans les semaines à venir ! Merci à vous !",
        author: 'Hassan N.',
      },
    ],
  },
  {
    url: 'erreurs-facturation-restaurants',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Erreurs\n  de facturation dans les restaurants',
    seoDescription: 'Signalez\n  une erreur de facturation dans un restaurant',
    title: 'Erreurs\n  de facturation dans les restaurants',
    targetedCategory: ['CafeRestaurant'],
    catchPhrase: 'Signalez\n  une erreur de facturation dans un restaurant, défendez vos droits !',
    secondaryTitle1: 'Pourquoi\n  signaler une erreur de facturation dans un restaurant ?',
    secondaryTitle2: "Que\n  devient votre signalement d'une erreur de facturation dans un restaurant ?",
    sampleReports: [
      {
        text: "J'ai récemment dîné chez ***** et j'ai remarqué une erreur sur ma facture. Ils ont facturé des articles que je n'avais pas commandés. Quand j'ai essayé de corriger l'erreur, ils ont refusé de modifier la facture. C'est une pratique commerciale déloyale.",
        author: 'Anna E.',
      },
      {
        text: "J'ai dîné chez ***** et après avoir vérifié ma facture, j'ai réalisé qu'ils m'avaient facturé des articles que je n'avais pas commandés. Malgré mes tentatives pour résoudre ce problème, ils ont refusé de corriger l'erreur.",
        author: 'Noémie C.',
      },
      {
        text: "Lors d'un repas chez *****, j'ai constaté qu'ils m'avaient facturé des plats que je n'avais pas commandés. Malgré mes tentatives pour rectifier cela, ils ont refusé de corriger la facture.",
        author: 'Bernard É.',
      },
      {
        text: "Après un dîner chez *****, j'ai découvert qu'ils m'avaient facturé des articles que je n'avais pas commandés. Malgré mes efforts pour résoudre ce problème, ils ont refusé d'ajuster la facture.",
        author: 'Sarah H.',
      },
      {
        text: "Après un repas chez *****, j'ai remarqué des erreurs sur ma facture, avec des frais pour des plats que je n'avais pas commandés. Malgré mes tentatives, ils ont refusé de rectifier la facture.",
        author: 'Jade L.',
      },
      {
        text: "Après un dîner chez *****, j'ai découvert des frais pour des plats que je n'avais pas commandés sur ma note. Malgré mes demandes, le personnel du restaurant a refusé de corriger l'erreur.",
        author: 'Ahmed E.',
      },
    ],
  },
  {
    url: 'fichage-bancaire-injustifie',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Fichage bancaire injustifié',
    seoDescription: "Vous avez été injustement fiché par une banque ? Signalez l'incident pour obtenir de l'aide.",
    title: 'Fichage Bancaire Injustifié',
    targetedCategory: ['BanqueAssuranceMutuelle'],
    catchPhrase:
      'Signalez un problème de fichage bancaire injustifié, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1:
      'Pourquoi signaler un fichage incorrect, un refus de service injuste ou une violation de la confidentialité ?',
    secondaryTitle2: 'Que devient votre signalement relatif à un fichage bancaire injustifié ?',
    sampleReports: [
      {
        text: "Je viens de découvrir que ***** a procédé à un fichage bancaire à mon encontre sans aucune justification. J'ai toujours été à jour dans mes remboursements et je n'ai jamais été informé d'une telle action.",
        author: 'Juliette A.',
      },
      {
        text: "J'ai été informé par ma banque que ***** a injustement procédé à mon fichage bancaire. Je n'ai jamais été en retard sur mes paiements et je suis choqué par ce manque de professionnalisme.",
        author: 'Leila É.',
      },
      {
        text: "J'ai récemment découvert que j'ai été inscrit sur le fichier bancaire par ***** sans raison valable. C'est inacceptable, car j'ai toujours respecté mes engagements financiers.",
        author: 'Émile R.',
      },
      {
        text: "Malgré une gestion rigoureuse de mes finances, j'ai été surpris d'apprendre que ***** m'a fiché auprès de la banque de France. Je suis actuellement dans l'incapacité d'obtenir un crédit à cause de ce fichage abusif.",
        author: 'Hassan F.',
      },
      {
        text: "J'ai découvert récemment que j'étais fiché par ***** malgré une bonne conduite bancaire. C'est très déconcertant et inacceptable.",
        author: 'Amira I.',
      },
    ],
  },
  {
    url: 'fourniture-systematique-echantillons-internet',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: "Fourniture systématique d’échantillons lors d'achat sur internet",
    seoDescription: "Signalez la fourniture d’échantillons systématique sans demande de votre part lors d'un achat sur internet",
    title: "Fourniture systématique d’échantillons sans demande de votre part lors d'un achat sur internet",
    targetedCategory: ['AchatInternet'],
    catchPhrase:
      "Signalez la fourniture systématique d’échantillons, même sans demande de la part des consommateurs, lors d'une commande sur internet et contribuez ainsi à préserver notre environnement pour les générations futures.",
    secondaryTitle1:
      "Pourquoi signaler la fourniture systématique d’échantillons aux consommateurs lors d'une commande internet ?",
    secondaryTitle2:
      "Que devient votre signalement relatif à la fourniture systématique d’échantillons lors d'une commande sur internet ?",
    sampleReports: [
      {
        text: "Lors de ma récente commande sur votre boutique en ligne, j'ai constaté que des échantillons de produits non sollicités ont été inclus dans mon colis. Je vous remercie pour ce qui semble être une délicate attention, néanmoins je suis déçue de recevoir des échantillons que je n'ai pas demandés et qui ne correspondent pas à mes besoins ou à mes préférences en matière de cosmétiques. Cette pratique contribue au gaspillage de produits et à la génération de déchets inutiles. De plus, cela va à l'encontre de mes efforts pour adopter un mode de vie plus minimaliste et plus respectueux de l'environnement.\nMerci de revoir vos pratiques.",
        author: 'Ahmed N.',
      },
      {
        text: "J'ai récemment passé une commande pour des produits sur votre site. Cependant, à ma grande surprise, j'ai découvert que des échantillons de produits de nettoyage ont été inclus dans mon colis. Cette pratique m'a déconcerté, car elle va à l'encontre de mes attentes en tant que consommateur et de mes valeurs environnementales.\nJe suis préoccupé par le fait que ces échantillons non sollicités contribuent au gaspillage de produits et à la génération de déchets inutiles. De plus, ces produits ne correspondent pas à mes besoins ni à mes préférences, ce qui rend leur réception d'autant plus regrettable.\nJe vous encourage à offrir aux clients la possibilité de choisir s'ils souhaitent recevoir des échantillons et à respecter leur volonté.",
        author: 'Léo D.',
      },
    ],
  },
  {
    url: 'fourniture-systematique-echantillons-magasin',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Fourniture systématique d’échantillons même sans demande dans un magasin',
    seoDescription: 'Signalez la fourniture d’échantillons par votre commerçant sans demande de votre part dans un magasin',
    title: 'Fourniture systématique d’échantillons par un commerçant même sans demande de la part des consommateurs ',
    targetedCategory: ['AchatMagasin'],
    catchPhrase:
      'Signalez la fourniture systématique d’échantillons par votre commerçant même sans demande de la part des consommateurs et contribuez ainsi à préserver notre environnement pour les générations futures.',
    secondaryTitle1:
      'Pourquoi signaler la fourniture systématique d’échantillons par votre commerçant même sans sollicitation de la part des consommateurs ?',
    secondaryTitle2:
      'Que devient votre signalement relatif à la fourniture systématique d’échantillons par votre commerçant même sans sollicitation de la part des consommateurs ?',
    sampleReports: [
      {
        text: "En me rendant dans votre salon de beauté, j'ai été surprise de constater qu'on m'avait glissé des échantillons de produits de soins capillaires dans mon sac, sans que je ne les aie demandés. Bien que cela puisse sembler être une attention, je considère que cette pratique engendre un gaspillage de produits. En tant que cliente, je devrais avoir le choix de recevoir ou non des échantillons. Il faudrait éviter que cette pratique soit systématique pour respecter la volonté des consommateurs et lutter contre le gaspillage.",
        author: 'Adam S.',
      },
      {
        text: "Lors de ma dernière visite dans votre magasin, j'ai été surprise de constater qu'à la caisse, on m'avait remis des échantillons de produits de beauté sans que je n'aie rien demandé. Premièrement, ces échantillons ne correspondent pas du tout à mes habitudes de consommation. J'ai l'habitude de choisir méticuleusement les produits de beauté que j'utilise, en tenant compte de leur composition, de leur éthique et de leur adéquation avec mes besoins. Recevoir des échantillons aléatoires ne correspondant pas à mes critères de sélection habituels me laisse donc perplexe quant à leur utilisation potentielle.\nDeuxièmement, distribuer des échantillons non sollicités est une pratique qui génère du gaspillage. Si ces échantillons ne correspondent pas à mes besoins ou à mes préférences, ils risquent fort de finir à la poubelle, ce qui est dommage tant du point de vue économique que environnemental.\nEnfin, cette pratique pourrait être repensée de manière à mieux servir les intérêts des clients. Par exemple, au lieu de distribuer des échantillons de façon indiscriminée, le magasin pourrait mettre en place un système permettant aux clients de choisir les échantillons qui les intéressent, peut-être en les intégrant dans un programme de fidélité ou en les proposant lors de l'achat de produits connexes.\nEn conclusion, bien que je comprenne que cette initiative puisse être perçue comme une gentille attention, je pense qu'il est important de revoir cette pratique pour qu'elle soit plus respectueuse des choix et des besoins des clients, tout en réduisant le gaspillage pour préserver l'environnement. ",
        author: 'Élias H.',
      },
    ],
  },
  {
    url: 'fruits-legumes-emballage-plastique-magasin',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Utilisation d’emballage plastique pour une quantité de fruits ou légume inférieure à 1,5kg dans un magasin',
    seoDescription:
      'Signalez l’utilisation d’emballage plastique pour une quantité de fruits ou légume inférieure à 1,5kg dans un magasin.',
    title: 'Utilisation d’emballage plastique par votre commerçant pour une quantité de fruits ou légume inférieure à 1,5kg.',
    targetedCategory: ['AchatMagasin'],
    catchPhrase:
      'Signalez l’utilisation d’emballage plastique pour une quantité de fruits ou légume inférieure à 1,5kg et contribuez ainsi à préserver notre environnement pour les générations futures ! ',
    secondaryTitle1:
      'Pourquoi signalez l’utilisation d’emballage plastique pour une quantité de fruits ou légume inférieure à 1,5kg ?',
    secondaryTitle2:
      'Que devient votre signalement relatif à l’utilisation d’emballage plastique pour une quantité de fruits ou légume inférieure à 1,5kg ?',
    sampleReports: [
      {
        text: "En visitant le marché de producteurs de la ville, célèbre pour son engagement envers des pratiques agricoles durables, j'ai été surpris de constater que certains stands utilisaient encore des emballages plastiques pour des quantités très réduites de fruits et légumes. Cette contradiction entre les valeurs affichées et les pratiques réelles est inacceptable. En tant que fervent défenseur de l'environnement, je trouve qu'il serait appréciable que des actions concrètes soient prises pour éliminer l'utilisation inutile de plastique dans l'emballage des produits frais sur ce marché.",
        author: 'Zahra S.',
      },
      {
        text: "En effectuant mes achats dans votre épicerie, j'ai remarqué que même de petites quantités de fruits et légumes, comme une seule courgette ou une poignée de tomates cerises, étaient systématiquement enveloppées dans du plastique. Cette pratique me semble non seulement superflue mais également préjudiciable à l'environnement. Il est urgent que des mesures soient prises pour éliminer l'utilisation excessive de plastique dans l'emballage de vos produits frais.",
        author: 'Clara S.',
      },
      {
        text: "En faisant mes courses dans votre supermarché bio, j'ai été consternée de constater que des emballages plastiques étaient encore utilisés pour emballer des quantités minimes de fruits et légumes, même pour des portions inférieures à 1,5 kg. Cette pratique est bien loin des valeurs environnementales que votre magasin prétend promouvoir. Soucieuse de réduire mon empreinte écologique, je souhaiterais que des alternatives respectueuses de l'environnement soient mises en place pour éviter ce gaspillage de plastique inutile. Merci !",
        author: 'Sofia M.',
      },
    ],
  },
  {
    url: 'hygiene-douteuse-personnel-restauration',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Hygiène\n  douteuse du personnel de restauration',
    seoDescription: 'Signalez\n  une hygiène douteuse du personnel de restauration',
    title: 'Hygiène\n  douteuse du personnel de restauration',
    targetedCategory: ['CafeRestaurant'],
    catchPhrase: 'Signalez\n  une hygiène douteuse du personnel de restauration, la propreté est\n  essentielle !',
    secondaryTitle1: 'Pourquoi\n  signaler une hygiène douteuse du personnel de restauration ?',
    secondaryTitle2: "Que\n  devient votre signalement d'une hygiène douteuse du personnel de restauration\n  ?",
    sampleReports: [
      {
        text: "Lors de ma dernière visite chez *****, j'ai été témoin de pratiques d'hygiène douteuses de la part du personnel. Ils ne se lavaient pas les mains régulièrement et utilisaient les mêmes gants pour différentes tâches. C'est inacceptable et très préoccupant.",
        author: 'Fatima A.',
      },
      {
        text: "Lors de mon dernier repas chez *****, j'ai observé que le personnel de cuisine ne portait pas de filets à cheveux et que le serveur n'a pas changé de gants après avoir touché des objets sales. Je suis préoccupé par l'hygiène douteuse du personnel.",
        author: 'Anna M.',
      },
      {
        text: "Lors de ma dernière visite chez *****, j'ai remarqué que le personnel de cuisine ne respectait pas les règles d'hygiène basiques, comme le port de coiffes, et les serveurs ne changeaient pas de gants après avoir touché des surfaces sales. C'est vraiment préoccupant.",
        author: 'Théo H.',
      },
      {
        text: "Le comportement du personnel chez ***** a soulevé des questions sérieuses sur l'hygiène. Les employés ne semblaient pas se laver les mains régulièrement et manipulaient la nourriture sans gants. C'est inquiétant.",
        author: 'Clara M.',
      },
      {
        text: "Le personnel de chez ***** ne semblait pas suivre les règles élémentaires d'hygiène, touchant souvent leur visage et leurs cheveux tout en préparant la nourriture. C'est franchement inquiétant.",
        author: 'Inès M.',
      },
      {
        text: "Le personnel de chez ***** semblait négliger les principes élémentaires de l'hygiène, souvent en touchant leurs visages ou en ajustant leurs vêtements pendant la préparation des repas.",
        author: 'Michel C.',
      },
    ],
  },
  {
    url: 'impression-systematique-ticket-de-caisse',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Impression systématique du ticket de caisse sans demande du consommateur',
    seoDescription: 'Signalez l’impression de votre ticket de caisse par votre commerçant sans demande de votre part',
    title: 'Impression de votre ticket de caisse par votre commerçant sans demande de votre part',
    targetedCategory: ['AchatMagasin'],
    catchPhrase:
      'Signalez l’impression systématique des tickets de caisse par votre commerçant sans demande de la part des consommateurs',
    secondaryTitle1:
      'Pourquoi signaler que votre commerçant imprime systématiquement les tickets de caisse sans demande de la part des consommateurs ?',
    secondaryTitle2:
      'Que devient votre signalement relatif à l’impression de votre ticket de caisse par votre commerçant sans demande de votre part ?',
    sampleReports: [
      {
        text: "Lors de mon dernier passage à votre boulangerie, j'ai été choquée de constater que le personnel imprimait systématiquement des tickets de caisse sans même demander aux clients s'ils en avaient besoin. Cette pratique non sollicitée génère un gaspillage de papier inutile, contribuant ainsi à la dégradation de l'environnement. De plus, elle va à l'encontre des efforts de sensibilisation pour réduire la consommation de papier et promouvoir des pratiques commerciales plus durables. Je demande que des mesures soient prises pour mettre fin à cette pratique irresponsable.",
        author: 'Nour L.',
      },
      {
        text: "En faisant mes courses dans votre supermarché, j'ai été consterné de voir que les caissiers impriment automatiquement des tickets de caisse pour chaque transaction, même lorsque les clients ne le demandent pas expressément. Cette pratique irresponsable contribue à un gaspillage de ressources inutile, en particulier de papier, alors que nous devrions tous œuvrer à réduire notre empreinte écologique. Je trouve cela inacceptable et demande que des mesures soient prises pour mettre fin à cette pratique et encourager une gestion plus responsable des ressources.",
        author: 'Anna F.',
      },
      {
        text: "Lors de ma dernière visite à votre pharmacie, j'ai été surprise de constater que les tickets de caisse étaient imprimés à chaque fois, pour chaque achat, même pour les plus petits articles. Cette pratique me semble non seulement inutile, mais aussi préjudiciable à l'environnement, compte tenu de l’utilisation de papier qu'elle engendre. En tant que cliente, je suis préoccupée par l'impact de cette pratique et je demande instamment que des mesures soient prises pour encourager une utilisation plus responsable de votre imprimante !",
        author: 'Idriss Z.',
      },
    ],
  },
  {
    url: 'incidents-intoxication-alimentaire',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: "Incidents\n  d'intoxication alimentaire",
    seoDescription: "Signalez\n  un incident d'intoxication alimentaire",
    title: "Incidents\n  d'intoxication alimentaire",
    targetedCategory: ['CafeRestaurant'],
    catchPhrase: "Signalez\n  un incident d'intoxication alimentaire, préservez la sécurité alimentaire !",
    secondaryTitle1: "Pourquoi\n  signaler un incident d'intoxication alimentaire ?",
    secondaryTitle2: "Que\n  devient votre signalement d'un incident d'intoxication alimentaire ?",
    sampleReports: [
      {
        text: "Après avoir mangé chez *****, j'ai été victime d'une intoxication alimentaire. Je suis certain que cela est dû à la nourriture que j'ai consommée dans leur restaurant, car je n'ai rien mangé d'autre ce jour-là. C'est un incident grave qui doit être traité avec le sérieux qu'il mérite.",
        author: 'Anna J.',
      },
      {
        text: "Après avoir déjeuné chez *****, j'ai eu des nausées et des vomissements toute la nuit. Il est évident que j'ai été victime d'une intoxication alimentaire à cause de la nourriture de ce restaurant. C'est inacceptable.",
        author: 'Sarah A.',
      },
      {
        text: "Après un repas chez *****, j'ai eu des symptômes graves d'intoxication alimentaire. Il est inacceptable que leur nourriture ait pu causer cela.",
        author: 'Enzo M.',
      },
      {
        text: "Suite à un repas chez *****, j'ai souffert d'une sévère intoxication alimentaire. C'est inadmissible qu'un restaurant puisse servir de la nourriture contaminée.",
        author: 'Salma T.',
      },
      {
        text: "J'ai eu une intoxication alimentaire sévère après avoir mangé chez *****. C'est inacceptable qu'un restaurant serve de la nourriture qui rend malade ses clients.",
        author: 'Théo A.',
      },
      {
        text: "J'ai souffert d'une grave intoxication alimentaire suite à mon déjeuner chez *****. Il est inadmissible qu'un restaurant mette en danger la santé de ses clients.",
        author: 'Théo L.',
      },
    ],
  },
  {
    url: 'infestation-nuisibles-restaurants',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Infestation\n  de nuisibles dans les restaurants',
    seoDescription: 'Signalez\n  une infestation de nuisibles dans un restaurant',
    title: 'Infestation\n  de nuisibles dans les restaurants',
    targetedCategory: ['CafeRestaurant'],
    catchPhrase: "Signalez\n  un problème d'infestation de nuisibles dans un restaurant, contribuez à la\n  propreté publique !",
    secondaryTitle1: 'Pourquoi\n  signaler une infestation de nuisibles dans un restaurant ?',
    secondaryTitle2: "Que\n  devient votre signalement d'une infestation de nuisibles dans un restaurant ?",
    sampleReports: [
      {
        text: "Hier soir, j'ai mangé chez ***** et j'ai été choqué de voir plusieurs nuisibles dans le restaurant. J'ai vu des souris courir sous les tables et j'ai également remarqué des cafards sur les murs. C'est tout simplement inacceptable et je suis très déçu de leur manque de respect pour l'hygiène.",
        author: 'Stella Z.',
      },
      {
        text: "Lors d'un récent repas chez *****, j'ai aperçu des insectes rampants sur les murs et le sol. C'est un signe clair d'infestation de nuisibles et soulève de sérieuses questions sur leur niveau d'hygiène.",
        author: 'Mehdi M.',
      },
      {
        text: "En dînant récemment chez *****, j'ai été dégoûté de voir des insectes se promener dans le restaurant. Cela indique clairement une infestation qui met en question leur niveau d'hygiène.",
        author: 'Mohamed M.',
      },
      {
        text: "J'ai été témoin d'une infestation de nuisibles chez *****. En plein milieu de mon repas, j'ai remarqué des insectes sur les murs. C'est absolument inadmissible pour un restaurant.",
        author: 'Mehdi J.',
      },
      {
        text: "J'ai dîné chez *****, où j'ai remarqué une présence excessive de mouches. C'est clairement le signe d'un problème d'infestation de nuisibles.",
        author: 'Ali G.',
      },
      {
        text: "Mon récent repas chez ***** a été gâché par la présence indéniable d'insectes nuisibles. L'apparition de mouches au milieu du repas était franchement dérangeante.",
        author: 'Louis B.',
      },
    ],
  },
  {
    url: 'informations-environnementales-vehicules-neufs',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: "Absence d'informations environnementales pour les véhicules neufs",
    seoDescription: "Signalez l'absence d'informations environnementales pour les véhicules neufs",
    title: "Absence d'informations environnementales pour les véhicules neufs",
    targetedCategory: ['VoitureVehiculeVelo'],
    catchPhrase:
      "Signalez l'absence d'informations environnementales pour les véhicules neufs, absence ou erreur d'affichage de la classe d'émission de CO2 du véhicule, absence ou erreur d'affichage des messages \"#SeDéplacerMoinsPolluer\" spot publicitaire",
    secondaryTitle1: "Pourquoi signaler l'absence d'informations environnementales pour les véhicules neufs ?",
    secondaryTitle2: 'Que se passera-t-il après le dépôt du signalement ?',
    sampleReports: [
      {
        text: "J'ai acheté une voiture neuve mais la classe d'émission de CO2 de la voiture ne m'a pas été communiquée !\n",
        author: 'Dylan A.',
      },
      {
        text: "J'ai vu une publicité sur la chaine XXX et l'obligation de la mention #SeDéplacerMoinsPolluer# n'était pas visible",
        author: 'Maria L.',
      },
    ],
  },
  {
    url: 'insalubrite-cuisines-restaurants',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Insalubrité\n  dans les cuisines de restaurants',
    seoDescription: "Signalez\n  un problème d'insalubrité dans une cuisine de restaurant",
    title: 'Insalubrité\n  dans les cuisines de restaurants',
    targetedCategory: ['CafeRestaurant'],
    catchPhrase: "Signalez\n  un problème d'insalubrité dans la cuisine d'un établissement de restauration\n  !",
    secondaryTitle1: "Pourquoi\n  signaler un problème d'insalubrité dans une cuisine de restaurant ?",
    secondaryTitle2: "Que\n  devient votre signalement d'un problème d'insalubrité en cuisine de\n  restaurant ?",
    sampleReports: [
      {
        text: "J'ai récemment visité le restaurant ***** et j'ai été horrifié par les conditions insalubres de leur cuisine. De la nourriture était laissée sans protection sur les comptoirs, les sols étaient sales et l'odeur était dégoûtante. Il est clair qu'aucune attention n'est portée à l'hygiène. C'est très dérangeant car ils mettent la santé des clients en danger.",
        author: 'Clara J.',
      },
      {
        text: "Lors de ma visite chez *****, j'ai aperçu l'intérieur de la cuisine et ce que j'ai vu était vraiment inquiétant. Le plan de travail était en désordre avec des restes de nourriture partout, les ustensiles n'étaient pas propres. C'est une question de santé publique et cela doit être pris au sérieux.",
        author: 'Louis C.',
      },
      {
        text: "Chez *****, j'ai eu un aperçu de la cuisine et j'ai été horrifié. Des aliments étaient éparpillés partout, les ustensiles semblaient négligés. Il s'agit là d'un risque évident pour la santé publique.",
        author: 'Louis É.',
      },
      {
        text: "Lors d'une visite au restaurant *****, j'ai eu l'occasion d'apercevoir l'état déplorable de leur cuisine. Les conditions de préparation des aliments étaient clairement insalubres, ce qui m'a incité à quitter les lieux immédiatement.",
        author: 'Noémie A.',
      },
      {
        text: "Durant ma visite chez *****, j'ai pu entrevoir la cuisine. L'hygiène y était catastrophique, avec des détritus partout et une odeur putride qui s'en échappait.",
        author: 'Margaux N.',
      },
      {
        text: "J'ai été témoin d'une scène alarmante lors de ma dernière visite chez ***** : les conditions de salubrité de la cuisine étaient effrayantes, avec des déchets alimentaires accumulés et un sol grasseux.",
        author: 'Soukaina J.',
      },
    ],
  },
  {
    url: 'invasion-rats-cafards-restauration',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Invasion\n  de rats et cafards dans la restauration',
    seoDescription: 'Signalez\n  une invasion de rats ou cafards dans un établissement de restauration',
    title: 'Invasion\n  de rats et cafards dans la restauration',
    targetedCategory: ['CafeRestaurant'],
    catchPhrase:
      "Signalez\n  un problème d'invasion de rats ou cafards dans la restauration, aidez-nous à\n  maintenir un environnement sain !",
    secondaryTitle1: 'Pourquoi\n  signaler une invasion de rats ou cafards dans la restauration ?',
    secondaryTitle2: "Que\n  devient votre signalement d'une invasion de rats ou cafards dans la\n  restauration ?",
    sampleReports: [
      {
        text: "J'ai récemment visité ***** pour un dîner et j'ai été horrifié de voir des rats et des cafards se promener librement dans le restaurant. C'était vraiment dégoûtant et indique clairement un manque d'hygiène et de respect pour la santé des clients.",
        author: 'Léa A.',
      },
      {
        text: "J'étais en train de dîner à *****, lorsque j'ai vu un rat courir le long du mur. Plus tard, j'ai également vu des cafards. C'est vraiment inacceptable dans un établissement qui sert de la nourriture.",
        author: 'Soukaina É.',
      },
      {
        text: "Au cours d'un repas chez *****, j'ai aperçu un rat se faufiler le long du mur. Par la suite, des cafards sont également apparus. C'est absolument répugnant pour un lieu qui sert de la nourriture.",
        author: 'Nour S.',
      },
      {
        text: "Lors d'un dîner chez *****, j'ai aperçu un rat courir le long des plinthes. Plus tard, j'ai également vu des cafards. C'est totalement inacceptable et indigne d'un lieu servant de la nourriture.",
        author: 'Fatima I.',
      },
      {
        text: "Dans le restaurant *****, j'ai été choqué de voir un rat se faufiler entre les tables, et plus tard, j'ai repéré des cafards sur le mur. C'est intolérable.",
        author: 'Amira N.',
      },
      {
        text: "Lors de ma dernière visite chez *****, j'ai été horrifié de voir un rat courir dans la salle à manger, suivi par la découverte de cafards sur le comptoir de la cuisine.",
        author: 'Ahmed S.',
      },
    ],
  },
  {
    url: 'lunettes',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Problème lunettes',
    seoDescription: 'Signalez un problème relatif au remboursement de vos lunettes ou à la qualité de vos lunettes ?',
    title: 'Problème remboursement ou qualité des lunettes',
    targetedCategory: ['BanqueAssuranceMutuelle', 'Sante'],
    catchPhrase: 'Signalez un problème de remboursement ou de qualité de vos lunettes',
    secondaryTitle1: 'Pourquoi signaler un problème de remboursement ou de qualité de vos lunettes ?',
    sampleReports: [
      {
        text: "Bonjour, une mutuelle refuse un équipement de lunettes pour une enfant de 16 ans malgré l'ordonnance d'un médecin. Depuis quand une mutuelle a plus de pouvoir de décision qu'un médecin pour savoir si quelqu'un a besoin de lunettes ou non ? La réponse apportée par le professionnel ne me convient pas du tout. Il y a non respect du contrat de l'adhérent et de plus, non respect de la loi car à aucun moment il n'est noté une correction minimale pour obtenir des lunettes. (sauf en cas de renouvellement mais ce sont ses premières lunettes). ",
        author: 'Enzo A.',
      },
      {
        text: "La mutuelle refuse de rembourser mes frais de lunettes faites en date du 09/03/2022, ils me demandent pleins de documents que j'ai transmis et que j'ai demandé à l'opticien, et malgré cela rien à faire ils refusent toujours de rembourser en prétextant à chaque fois une erreur de document, de date etc...les ordonnances sont valables plus d'un an, soit disant l'ordonnance n'est pas à la même date que l'achat de la monture, bref je suis épuisée. Il m'ont même demandé mes relevés de banque ??? Je paye des cotisations mutuelle et je ne suis pas remboursée !",
        author: 'Leila M.',
      },
      {
        text: "J'ai acheté des lunettes en janvier 2023, une première mutuelle complémentaire m'a remboursé la somme de 420€. Mon autre complémentaire santé xxx refuse de me rembourser le reste à charge alors que j'ai un contrat avec cette mutuelle depuis plus de 10 ans et je n'ai jamais rencontré de problème auparavant prétextant un non cumul des contrats. Or aucune réglementation ne va dans ce sens. ",
        author: 'Hassan M.',
      },
      {
        text: "Je n'ai toujours pas reçu le remboursement de mes lunettes et lentilles de septembre 2021, malgré les nombreuses relances la sécurité sociale me confirme que c'est à ma mutuelle donc à xxx de les prendre en charge. Quant aux lunettes je leur ai fourni le décompte ameli. Je reste dans l'attente de remboursement des 410€ +200 Euros (lunettes + lentilles). Vous trouverez en pj les factures .",
        author: 'Fatima G.',
      },
      {
        text: "Je suis allé chez l'opticien xxx  pour une prescription de lunettes. Sur le devis qui m'a été remis, la vendeuse a rayé l'offre \"100% santé\" en prétextant que c'était réservé aux bénéficiaires de la CMU. Il me semble que c'est faux. Je suis retourné le soir pour rencontrer une autre vendeuse. J'ai indiqué que je souhaitais opter pour l'offre 100% santé. Elle m'a indiqué que je devais préalablement obtenir une attestation de l'assurance maladie indiquant que je suis \"éligible\" à l'offre. Cela me paraît faux également. Je comprends que les opticiens ne souhaitent pas mettre en avant cette offre 100% santé mais l'opticien xxx utilise des méthodes condamnables pour dissuader ses clients de choisir cette offre. J",
        author: 'Louis A.',
      },
      {
        text: "L'opticien fait du chantage pour refaire mes lunettes avec un bon centrage. Il me demande de faire un écrit pour refaire mes lunettes avec le centrage de mes lunettes précédentes faites chez un autre opticien en 2018. Selon lui le mauvais centrage serait de mon ancien opticien et ma vue aurait l'habitude d'un mauvais centrage. C'est un cauchemar, jamais je n'ai eu ce type de chantage chez un opticien. En raison de ce chantage j'exige mon remboursement de 1415 euros, j'ai réglé cette somme et n'ai toujours pas de lunettes de cet opticien. Aujourd'hui je suis dans l'obligation de porter mes vieilles lunettes car je n'ai pas l'argent pour refaire une monture correcte avec un bon centrage et de qualité chez un autre opticien. C'est le 2ème signalement que je fais sur votre site. Ce désagrément dure depuis décembre 2022.",
        author: 'Sarah S.',
      },
      {
        text: "Mauvaise qualité du service, verres ébréchés au montage. Le responsable du magasin n'a pas voulu rembourser voire refaire les lunettes commandées comme stipulé dans la fiche envoyée à ma mutuelle. N'a d'ailleurs pas voulu me remettre la facture détaillée pour que je la retourne à ma mutuelle pour expertise des lunettes remises. Mauvais confort de vision et verres mal adaptés. Finalement j'ai laissé les lunettes au magasin. Je me répète au point d'être peut-être vulgaire j'ai été dépouillée de plus de 300€ avec le sourire. Le responsable du magasin laisse un sentiment de quelqu'un de rapace et qui ne vise que son chiffre d'affaires. J'avais l'impression de déranger de faire remarquer le mauvais montage et les éclats de verres constatés. Achat des lunettes en novembre 2022 reçu fin janvier 2023",
        author: 'Nour J.',
      },
      {
        text: "Verres de mauvaise qualité et vol sur la marchandise. Achat d'une monture de lunettes d'une valeur d'un peu plus de 200€. Verres montés abîmés, découpes avec des traces d'éclats. En échange un verre non polarisé comme en demande initiale et de moindre qualité. Vérifiez la facture si non déjà falsifiée. Beaucoup de soucis et plus de 2 mois d'attente pour cette paire de lunettes que j'ai laissé au magasin. Ne recommande absolument pas cet opticien !",
        author: 'Mehdi L.',
      },
    ],
  },
  {
    url: 'malfacons-non-conformites-batiment',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Malfaçons\n  et non-conformités dans le domaine du bâtiment',
    seoDescription:
      'Signalez\n  un problème de malfaçon, de non-conformité ou de défauts de construction dans\n  le domaine du bâtiment.',
    title: 'Malfaçons\n  et non-conformités dans le domaine du bâtiment',
    targetedCategory: ['TravauxRenovations'],
    catchPhrase:
      'Signalez\n  un problème de malfaçon, de non-conformité ou de défauts de construction dans\n  le bâtiment, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1:
      'Pourquoi\n  signaler des malfaçons, non-conformités ou défauts de construction dans le\n  domaine du bâtiment ?',
    secondaryTitle2:
      'Que\n  devient votre signalement relatif à des malfaçons, non-conformités ou défauts\n  de construction dans le bâtiment ?',
    sampleReports: [
      {
        text: "J'ai engagé l'entreprise ***** pour construire une extension à ma maison. Toutefois, je constate de nombreuses malfaçons et l'extension n'est pas conforme aux plans initiaux. Malgré mes plaintes répétées, l'entreprise ne prend pas ses responsabilités.",
        author: 'Hassan N.',
      },
      {
        text: "J'ai fait appel à ***** pour la construction de ma nouvelle maison. Cependant, la maison présente de nombreuses malfaçons et ne correspond pas aux plans convenus. Je me sens trompé et désemparé face à cette situation.",
        author: 'Enzo M.',
      },
      {
        text: "J'ai contracté les services de ***** pour la rénovation de ma cuisine. Cependant, les travaux effectués présentent de nombreuses malfaçons et ne sont pas conformes à ce que nous avions convenu. Malgré mes tentatives de contact, l'entreprise ne répond pas.",
        author: 'Timothée D.',
      },
      {
        text: "J'ai engagé l'entreprise ***** pour la construction d'un garage attenant à ma maison. Malheureusement, le garage présente de nombreuses malfaçons et n'est pas du tout conforme aux plans. L'entreprise ne semble pas vouloir prendre ses responsabilités.",
        author: 'Nour T.',
      },
      {
        text: "J'ai fait appel à ***** pour des travaux de rénovation dans ma maison. Cependant, les travaux réalisés présentent de nombreuses malfaçons et ne sont pas conformes aux plans initiaux. Je me sens complètement arnaqué.",
        author: 'Hassan M.',
      },
      {
        text: "J'ai contracté les services de ***** pour la construction d'une nouvelle salle de bains. Toutefois, je constate de nombreuses malfaçons et la salle de bains n'est pas conforme aux plans. Malgré mes plaintes, l'entreprise ne semble pas vouloir résoudre le problème.",
        author: 'Nathan L.',
      },
      {
        text: "J'ai engagé l'entreprise ***** pour la rénovation de mon salon. Cependant, je constate de nombreuses malfaçons et le travail réalisé n'est pas conforme aux plans. Je me sens déçu et arnaqué.",
        author: 'Leila R.',
      },
      {
        text: "J'ai fait appel à ***** pour des travaux de rénovation dans ma maison. Les travaux réalisés présentent de nombreuses malfaçons et ne sont pas conformes aux plans initiaux. Malgré mes tentatives de contact, l'entreprise ne semble pas vouloir résoudre le problème.",
        author: 'Stella J.',
      },
    ],
  },
  {
    url: 'pompe-a-chaleur-pac',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Pompe à chaleur - PAC',
    seoDescription: 'Votre problème concerne une arnaque relative à une pompe à chaleur-PAC ?\n',
    title: 'Problème ou arnaque lié à une pompe à chaleur PAC ?',
    targetedCategory: ['TravauxRenovations'],
    catchPhrase: "Signalez une arnaque liée à la livraison, l'installation ou la mise en service d'une pompe à chaleur PAC !",
    secondaryTitle1:
      "Pourquoi faire un signalement si vous rencontrez des problèmes lors de travaux d'installation ou de de mise en service d'une pompe à chaleur (PAC) air-air ou air-eau ?",
    secondaryTitle2: 'Que va devenir votre signalement concernant la pompe à chaleur ?',
    sampleReports: [
      {
        text: "Bonjour , j'ai signalé le 27/04/2023 au professionnel une absence de fonctionnement de la climatisation réversible vendu et monté par ses soins le 07/07/2021. Marque xxx facture au prix de 19 000€. Depuis cette date, et malgré mes appels téléphoniques répétés, mails et courrier recommandé avec accusé de réception rien n'y fait. En tant que vendeur de cette pompe à chaleur, vous êtes tenu de garantir le bien contre ses défauts de conformités. Je suis suivi pour des problèmes respiratoires conséquents et le non fonctionnement de la climatisation va porter préjudice a mon état de santé.",
        author: 'Idriss Z.',
      },
      {
        text: "Bonjour, nous avons fait installer une pompe à chaleur avec la société xxx France au mois de janvier. Depuis fin janvier début février je tente de les contacter par mail, par appel ou encore par courrier mais aucune réponse alors que je tente de les informer que cette PAC ne fonctionne pas !! J'ai envoyé un recommandé expliquant tout mon parcours et ainsi leur disant que je voulais mettre fin a leur prestation et qu'ils viennent récupérer tout leur matériel.. J'ai fais de même à l'entreprise de financement. Mais toujours aucune nouvelle.\n",
        author: 'Emma I.',
      },
      {
        text: "La pompe à chaleur qu'on ma installé n'est pas aux normes électriques (disjoncteurs et section de câble sous dimensionnés). L'entreprise que j'ai contacté pour la révision annuelle de ma PAC refuse de faire l'entretien de ma pompe à chaleur. J'ai voulu joindre l'entreprise qui me l'a installé. Entreprise est en liquidation judiciaire depuis 2020. Par contre le crédit lui est prélevé tout les mois sur mon compte.",
        author: 'Dylan Z.',
      },
      {
        text: "Panne de pompe à chaleur air eau haute température depuis 25 novembre 2022. Travaux réalisés en mai 2022 cette entreprise ne respecte pas la garantie panne, une grosse partie de l'hiver nous avons souffert du froid. SAV incompétent pompe à chaleur inutilisable pour le moment.\n",
        author: 'Dylan M.',
      },
      {
        text: "Bonjour, j'ai commandé et payé une installation de panneaux solaires avec une pompe à chaleur. L'installation des panneaux a été faite mais je n'ai toujours pas reçu la pompe à chaleur promise pour le 14/03/2022. Malgré mes nombreuses relances la société xxx ne répond pas. Pouvez vous m'aider à me faire rembourser la pompe à chaleur. La commande est de 22500 € tout compris que j'ai payée à l'aide d'un financement.  Je compte sur votre compréhension et action.",
        author: 'Émile M.',
      },
      {
        text: "Bonjour, suite à mon courrier -à l'attention du professionnel- du 03/05/2023 resté sans réponse, je vous demande à nouveau de bien vouloir prendre en compte ma réclamation : Le 15/03/2023, dans le cadre du contrat de maintenance me liant à xxx, vous avez réalisé l’entretien de ma pompe à chaleur. Moins de 48h après, la pompe à chaleur est tombée en panne suite à la vidange totale du gaz frigorigène. Il s'avère que votre technicien a oublié de resserrer (ou desserré) un raccord de compresseur, provoquant la perte du gaz. Le devis fourni par xxx pour la remise en état dans le cadre du contrat de maintenance était anormalement élevé du fait d'une surestimation du volume de gaz et de son prix au kg (973€HT). J'ai donc fait réaliser la réparation par une autre société (facturation 310.5€ HT de gaz R407C). Cette intervention a confirmé que la panne faisait bien suite à votre intervention. Je vous demande le remboursement de l'intervention d'entretien et des frais de remise en état. ",
        author: 'Dylan M.',
      },
      {
        text: "Bonjour, j'ai eu un démarchage téléphonique pour une pose de pompe à chaleur, puis les personnes de l'organisme conseil sont venues chez moi avec un QR code. Elles m'ont donné des explications sur les aides de l'Etat, ils m'ont fait remplir un bon de commande en attente du retour d'acceptation du dossier savoir si je suis éligible et m'ont dit que rien ne serait fait sans cet accord. Alors j'ai signé les formulaires et communiqué mon avis d'imposition; bulletins de salaire; carte d'identité; une facture de domicile et RIB. t le soir même j'ai bien relu tout cela, et j'ai constaté sur le internet que c'était (je pense) une arnaque. J'ai contacté xxx au par téléphone qui ont cherché des infos et ils m'ont suggéré d'envoyer de suite le formulaire de rétractation avec accusé de réception. Et cette démarche va être fait demain matin 26/05/2023 en espérant que je vais me sortir de cette arnaque à la PAC !\n",
        author: 'Théo N.',
      },
      {
        text: "Copie du message adressé à l'artisan : Vous nous avez arnaqué en nous faisant souscrire un prêt à la consommation pour une pompe à chaleur soit disant entièrement remboursée par les aides de l'Etat. La pompe à chaleur ne fonctionne pas et toujours aucune nouvelles. Nous n aurions jamais installé cette PAC sans les aides de l'Etat 21 000€, nous n avons pas les moyens c'est honteux, nous nous retrouvons dans une situation plus que délicate !!!\n",
        author: 'Noémie N.',
      },
    ],
  },
  {
    url: 'prelevements-non-autorises-entreprise',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Prélèvements non autorisés avec une entreprise',
    seoDescription:
      'Avez-vous été victime de prélèvements non autorisés par une entreprise ? Signalez-le pour aider à stopper ces pratiques.',
    title: 'Prélèvements Non Autorisés avec une Entreprise',
    targetedCategory: ['BanqueAssuranceMutuelle'],
    catchPhrase:
      'Signalez un problème de prélèvements non autorisés avec une entreprise, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi signaler un débit injustifié, un montant incorrect ou un abonnement non désiré ?',
    secondaryTitle2: 'Que devient votre signalement relatif à un prélèvement non autorisé par une entreprise ?',
    sampleReports: [
      {
        text: "En mai 2023, j'ai constaté des prélèvements récurrents sur mon compte bancaire de la part d'une entreprise, *****. Je n'ai jamais autorisé ces prélèvements et malgré plusieurs tentatives pour contacter l'entreprise et ma banque, les prélèvements continuent chaque mois. J'ai besoin d'aide pour résoudre cette situation frustrante.",
        author: 'Timothée A.',
      },
      {
        text: "En juin 2023, j'ai réalisé que ***** avait commencé à débiter de mon compte un montant important chaque mois. En vérifiant mes relevés bancaires, j'ai constaté que ces prélèvements avaient commencé trois mois auparavant sans mon consentement. J'ai contacté à plusieurs reprises l'entreprise et ma banque, mais sans succès.",
        author: 'Idriss H.',
      },
      {
        text: "J'ai récemment découvert que ***** prélève de mon compte bancaire un montant mensuel depuis avril 2023. Je n'ai jamais souscrit à leurs services ni donné mon accord pour ces prélèvements. Malgré mes tentatives pour les contacter et résoudre le problème, ces prélèvements continuent.",
        author: 'Clara Y.',
      },
      {
        text: "Depuis janvier 2023, je constate un prélèvement mensuel effectué par ***** sur mon compte bancaire. Je n'ai jamais signé d'accord de prélèvement avec cette entreprise et mes tentatives pour arrêter ces prélèvements sont restées sans réponse.",
        author: 'Zahra A.',
      },
      {
        text: "J'ai remarqué que depuis mars 2023, ***** prélève un montant fixe de mon compte bancaire chaque mois. Je n'ai jamais autorisé ce prélèvement et malgré mes efforts pour les contacter et résoudre cette situation, les prélèvements continuent.",
        author: 'Juliette S.',
      },
    ],
  },
  {
    url: 'problemes-adhesion-salle-sport',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: "Problèmes d'adhésion à la salle de sport",
    seoDescription: "Signalez un problème d'adhésion à la salle de sport",
    title: "Problèmes d'adhésion à la salle de sport",
    targetedCategory: ['VoyageLoisirs'],
    catchPhrase:
      "Signalez un problème d'adhésion à la salle de sport, renseignez-vous sur vos droits avec la répression des fraudes !",
    secondaryTitle1: "Pourquoi signaler un problème d'adhésion, de résiliation ou de frais cachés dans une salle de sport ?",
    secondaryTitle2: "Que devient votre signalement relatif à un problème d'adhésion à la salle de sport ?",
    sampleReports: [
      {
        text: "Mon désespoir est au plus haut. J'ai souscrit un abonnement à la salle de sport ***** avec l'espoir d'améliorer ma santé et mon bien-être. Mais depuis mon adhésion, je n'ai rencontré que des problèmes. Non seulement les équipements sont souvent hors service, mais le personnel est également très désagréable. De plus, ils ont récemment augmenté les frais d'adhésion sans en informer leurs membres.",
        author: 'Nathan Z.',
      },
      {
        text: "Je suis à bout de nerfs avec cette salle de sport *****. Depuis mon inscription, c'est un défilé incessant de problèmes. Entre les machines défectueuses et l'attitude déplorable du personnel, je ne sais plus où donner de la tête. Pour couronner le tout, ils ont osé augmenter les frais d'adhésion sans prévenir.",
        author: 'Ayoub L.',
      },
      {
        text: "Je suis exaspéré par la salle de sport *****. Les équipements sont souvent hors service et l'attitude du personnel laisse à désirer. Et comme si cela ne suffisait pas, ils ont augmenté les frais d'adhésion sans préavis. C'est une honte.",
        author: 'Zahra N.',
      },
      {
        text: "Je suis consterné par le comportement de la salle de sport *****. L'état des équipements est lamentable et le personnel est d'une rudesse inouïe. Pour couronner le tout, les frais d'adhésion ont été augmentés sans préavis. C'est vraiment une situation déplorable.",
        author: 'Rachid I.',
      },
      {
        text: "Je suis désemparé par mon expérience avec la salle de sport *****. Les équipements sont constamment en panne, l'atmosphère est désagréable et pour couronner le tout, ils ont augmenté les frais d'adhésion sans aucune annonce préalable. C'est honteux.",
        author: 'Émile N.',
      },
      {
        text: "Je suis perplexe face à la gestion de la salle de sport *****. Les équipements sont régulièrement hors d'usage, l'ambiance est loin d'être conviviale et, comble de l'ironie, ils ont augmenté les frais d'adhésion sans préavis. C'est scandaleux.",
        author: 'Fatima Z.',
      },
    ],
  },
  {
    url: 'problemes-annulation',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: "Problèmes d'annulation",
    seoDescription: "Signalez un problème d'annulation",
    title: "Problèmes d'annulation",
    targetedCategory: ['VoyageLoisirs'],
    catchPhrase: "Signalez un problème d'annulation, renseignez-vous sur vos droits avec la répression des fraudes !",
    secondaryTitle1: "Pourquoi signaler un problème d'annulation de voyage, de réservation ou de commande en ligne ?",
    secondaryTitle2: "Que devient votre signalement relatif à un problème d'annulation ?",
    sampleReports: [
      {
        text: "J'ai récemment vécu une situation désespérée avec *****. J'avais réservé un événement avec eux mais à cause d'un imprévu, j'ai dû l'annuler. Je leur ai envoyé une demande d'annulation bien avant leur délai de préavis, mais ils ont refusé de me rembourser. C'est incroyablement frustrant et décevant.",
        author: 'Margaux S.',
      },
      {
        text: "Quelle déconvenue avec *****! Suite à un imprévu, j'ai dû annuler une réservation. Malgré le respect des délais d'annulation, ils ont refusé de me rembourser. C'est révoltant et profondément décevant.",
        author: 'Leila S.',
      },
      {
        text: "Je suis dévasté par l'expérience que j'ai eue avec *****. J'ai dû annuler une réservation à cause d'un imprévu, et malgré le respect des délais, ils ont refusé de me rembourser. Leur indifférence est révoltante et extrêmement décevante.",
        author: 'Myriam A.',
      },
      {
        text: 'Je suis bouleversé par la manière dont ***** a géré mon annulation. Malgré le respect des délais, ils ont refusé mon remboursement, rendant cette situation encore plus difficile. Leur manque de compassion est sidérant.',
        author: 'Nadia Z.',
      },
      {
        text: "Je suis consterné par la façon dont ***** a géré mon annulation. J'ai dû annuler une réservation en raison d'un imprévu et malgré le respect des délais, ils ont refusé de me rembourser. C'est une véritable déception.",
        author: 'Nathan R.',
      },
      {
        text: "Je suis attristé par la manière dont ***** a géré mon annulation. J'ai dû annuler une réservation pour cause de force majeure et malgré le respect des conditions, ils ont refusé mon remboursement. C'est profondément décevant.",
        author: 'Amira Z.',
      },
    ],
  },
  {
    url: 'problemes-carte-credit-banque',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Problèmes de carte de crédit avec votre banque',
    seoDescription:
      "Vous avez rencontré des problèmes de carte de crédit avec votre banque ? Signalez le problème pour obtenir de l'aide.",
    title: 'Problèmes de carte de crédit avec votre banque',
    targetedCategory: ['BanqueAssuranceMutuelle'],
    catchPhrase:
      'Signalez un problème de carte de crédit avec votre banque, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1:
      'Pourquoi signaler un débit excessif, une commission injustifiée ou un dysfonctionnement de carte de crédit ?',
    secondaryTitle2: 'Que devient votre signalement relatif à un problème de carte de crédit avec votre banque ?',
    sampleReports: [
      {
        text: "En août 2023, j'ai fait une demande pour une nouvelle carte de crédit auprès de *****. Cependant, jusqu'à ce jour, je n'ai pas reçu la carte malgré plusieurs réclamations. En outre, des frais de carte de crédit sont déduits de mon compte chaque mois. Je considère cela comme un prélèvement non autorisé et un manque de service de la part de la banque *****.",
        author: 'Hugo A.',
      },
      {
        text: "J'ai signalé une fraude sur ma carte de crédit à la banque ***** le 1er juin 2023, suite à des transactions non autorisées. J'ai été assuré que l'affaire serait traitée et que je serais remboursé. Malheureusement, plus d'un mois s'est écoulé et je n'ai reçu aucune communication de leur part, ni aucun remboursement.",
        author: 'Hugo S.',
      },
      {
        text: "Le 20 mai 2023, j'ai perdu ma carte de crédit de la banque ***** et j'ai immédiatement signalé la perte à la banque. On m'a assuré qu'une nouvelle carte serait envoyée dans les 7 à 10 jours ouvrables, mais elle n'est jamais arrivée. J'ai contacté la banque à plusieurs reprises et chaque fois, on m'a dit que la carte était en route.",
        author: 'Maria T.',
      },
      {
        text: "J'ai effectué un paiement en ligne avec ma carte de crédit de la banque ***** le 15 juin 2023. Le paiement a été débité deux fois de mon compte. J'ai immédiatement contacté la banque, qui a assuré que le montant serait remboursé dans les 10 jours ouvrables. Cependant, plus d'un mois s'est écoulé et je n'ai pas encore reçu le remboursement.",
        author: 'Idriss L.',
      },
      {
        text: "Le 30 juin 2023, j'ai reçu un avis de la banque ***** concernant une augmentation du taux d'intérêt de ma carte de crédit, sans aucune notification préalable. J'ai contacté la banque pour contester cette augmentation soudaine et inexpliquée, mais ils n'ont pas été en mesure de me fournir une explication satisfaisante.",
        author: 'Emma R.',
      },
    ],
  },
  {
    url: 'problemes-compensation-financiere',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Problèmes de compensation financière',
    seoDescription: 'Signalez un problème de compensation financière',
    title: 'Problèmes de compensation financière',
    targetedCategory: ['VoyageLoisirs'],
    catchPhrase:
      'Signalez un problème de compensation financière, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1:
      'Pourquoi signaler un problème de compensation financière suite à un retard de vol ou une annulation de voyage ?',
    secondaryTitle2: 'Que devient votre signalement relatif à un problème de compensation financière ?',
    sampleReports: [
      {
        text: "Il est inacceptable de voir comment ***** gère les problèmes de compensation financière. Suite à un problème de leur part, ils ont admis leur erreur et ont promis de me dédommager. Cependant, cela fait maintenant plus de deux mois et je n'ai toujours pas reçu le montant promis. J'ai contacté leur service client plusieurs fois, mais ils continuent de me donner des réponses évasives. C'est absolument frustrant.",
        author: 'Yasmina T.',
      },
      {
        text: "Quelle désillusion avec *****! Suite à une mésaventure, ils m'ont promis une compensation financière. Cela fait déjà plusieurs semaines et je n'ai toujours rien reçu. Malgré mes multiples relances, leurs réponses restent vagues et sans solution. C'est inadmissible.",
        author: 'Adam S.',
      },
      {
        text: "Je suis au bord de la crise de nerfs avec *****. Ils ont promis une compensation financière suite à une erreur de leur part, mais je n'ai rien reçu malgré mes nombreuses relances. Leurs réponses évasives ne font qu'ajouter de l'huile sur le feu.",
        author: 'Mehdi M.',
      },
      {
        text: "Je suis profondément déçu par la conduite de *****. Suite à une erreur de leur part, ils m'ont promis une compensation financière qui n'est jamais arrivée. Malgré mes nombreuses demandes, leur réponse a été évasive et sans solution. C'est inacceptable.",
        author: 'Ayoub L.',
      },
      {
        text: "Je suis désillusionné par le comportement de *****. Ils ont promis une compensation financière suite à une erreur, mais je n'ai encore rien reçu. Malgré mes relances, leurs réponses sont toujours évasives et non résolutives.",
        author: 'Anna E.',
      },
      {
        text: "Je suis exaspéré par l'attitude de *****. Suite à une erreur de leur part, ils m'ont promis une compensation financière, mais je n'ai rien reçu à ce jour. En dépit de mes multiples relances, leurs réponses restent floues et insatisfaisantes.",
        author: 'Ali L.',
      },
    ],
  },
  {
    url: 'problemes-conditions-hebergement',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: "Problèmes de conditions d'hébergement",
    seoDescription: "Signalez un problème de conditions d'hébergement",
    title: "Problèmes de conditions d'hébergement",
    targetedCategory: ['VoyageLoisirs'],
    catchPhrase:
      "Signalez un problème de conditions d'hébergement, renseignez-vous sur vos droits avec la répression des fraudes !",
    secondaryTitle1: "Pourquoi signaler un problème de conditions d'hébergement dans un hôtel, une auberge ou un gîte ?",
    secondaryTitle2: "Que devient votre signalement relatif à un problème de conditions d'hébergement ?",
    sampleReports: [
      {
        text: "Mon expérience avec l'hébergement chez ***** a été affligeante. Les conditions étaient bien en-dessous de ce qui était promis dans leur description en ligne. Il y avait des problèmes d'hygiène majeurs et le niveau de bruit était insupportable. Je ne peux pas croire qu'ils puissent offrir de telles conditions à leurs clients.",
        author: 'Salma Y.',
      },
      {
        text: "L'expérience cauchemardesque avec ***** m'a laissé une amertume indélébile. Les conditions d'hébergement étaient effroyables, bien loin de la description alléchante en ligne. Entre problèmes d'hygiène et nuisance sonore incessante, c'est tout bonnement inconcevable.",
        author: 'Élias F.',
      },
      {
        text: "Je suis horrifié par mon expérience avec *****. Les conditions d'hébergement étaient bien en-dessous de ce qui était promis. Entre les problèmes d'hygiène et le bruit constant, je suis scandalisé qu'ils puissent offrir de telles conditions à leurs clients.",
        author: 'Ahmed A.',
      },
      {
        text: "Je suis révolté par les conditions d'hébergement proposées par *****. Les problèmes d'hygiène et de bruit ont rendu mon séjour insupportable. C'est une honte qu'ils puissent offrir de telles conditions à leurs clients.",
        author: 'Amine E.',
      },
      {
        text: "Je suis choqué par les conditions d'hébergement de *****. Entre les problèmes d'hygiène et le bruit constant, mon séjour a été un véritable enfer. C'est inacceptable et je suis déçu qu'ils puissent offrir de telles conditions à leurs clients.",
        author: 'Nathan R.',
      },
      {
        text: "Je suis horrifié par les conditions d'hébergement offertes par *****. Entre les problèmes d'hygiène graves et le bruit incessant, mon séjour a été insupportable. C'est inconcevable qu'ils puissent proposer de telles conditions à leurs clients.",
        author: 'Bernard I.',
      },
    ],
  },
  {
    url: 'problemes-frais-caches',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Problèmes de frais cachés',
    seoDescription: 'Signalez un problème de frais cachés',
    title: 'Problèmes de frais cachés',
    targetedCategory: ['VoyageLoisirs'],
    catchPhrase: 'Signalez un problème de frais cachés, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1:
      "Pourquoi signaler un problème de frais cachés lors d'une réservation, d'une adhésion ou d'un achat en ligne ?",
    secondaryTitle2: 'Que devient votre signalement relatif à un problème de frais cachés ?',
    sampleReports: [
      {
        text: "Je suis complètement outré par la façon dont ***** opère. Après avoir acheté un service chez eux, j'ai découvert qu'il y avait des frais cachés qui n'avaient pas été clairement indiqués au moment de l'achat. Cela a rendu le service beaucoup plus cher que ce que j'avais initialement prévu. C'est un manque total de transparence.",
        author: 'Margaux T.',
      },
      {
        text: "Je suis sidéré par l'opacité de *****. Après l'achat d'un service, j'ai découvert des frais cachés qui n'avaient jamais été mentionnés. Le coût final était bien au-delà de mon budget. Il s'agit là d'un manque de transparence absolu.",
        author: 'Fatima S.',
      },
      {
        text: "Je suis furieux à cause des pratiques de *****. J'ai été choqué de découvrir des frais cachés après l'achat d'un service. Le coût réel était beaucoup plus élevé que prévu. Leur manque de transparence est scandaleux.",
        author: 'Alexandre M.',
      },
      {
        text: "Je suis furieux contre ***** pour leurs pratiques déloyales. J'ai été horrifié de découvrir des frais cachés après l'achat d'un service. Leur manque de transparence est choquant et inacceptable.",
        author: 'Théo T.',
      },
      {
        text: "Je suis furieux contre *****. J'ai découvert des frais cachés après l'achat d'un service. C'est une véritable surprise et un manque de transparence de leur part.",
        author: 'Fatima L.',
      },
      {
        text: "Je suis en colère contre *****. J'ai été choqué de découvrir des frais supplémentaires après l'achat d'un service. Leur manque de transparence est sidérant et inacceptable.",
        author: 'Mehdi M.',
      },
    ],
  },
  {
    url: 'problemes-informations-trompeuses',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: "Problèmes d'informations trompeuses",
    seoDescription: "Signalez un problème d'informations trompeuses",
    title: "Problèmes d'informations trompeuses",
    targetedCategory: ['VoyageLoisirs'],
    catchPhrase:
      "Signalez un problème d'informations trompeuses, renseignez-vous sur vos droits avec la répression des fraudes !",
    secondaryTitle1: "Pourquoi signaler un problème d'informations trompeuses sur un produit, un service ou une offre ?",
    secondaryTitle2: "Que devient votre signalement relatif à un problème d'informations trompeuses ?",
    sampleReports: [
      {
        text: "Je suis indigné par les informations trompeuses fournies par *****. Leur site Web indiquait clairement que le produit que j'ai acheté avait certaines caractéristiques. Cependant, une fois que j'ai reçu le produit, j'ai découvert qu'il ne correspondait pas du tout à la description. C'est une pratique déloyale et décevante.",
        author: 'Sofia J.',
      },
      {
        text: "L'indignation me gagne lorsque je pense à *****. Leurs informations sont purement et simplement trompeuses. Le produit que j'ai reçu n'a rien à voir avec la description faite sur leur site. C'est un manque d'honnêteté flagrant.",
        author: 'Dylan F.',
      },
      {
        text: "Mon indignation est à son comble à cause de *****. Les informations sur leur site sont complètement trompeuses. Le produit que j'ai reçu était loin de la description en ligne. C'est une trahison totale de leur part.",
        author: 'Enzo S.',
      },
      {
        text: "Je suis atterré par la manière dont ***** présente ses produits. Les informations sur leur site sont délibérément trompeuses. Le produit que j'ai reçu était radicalement différent de la description en ligne. C'est un manque d'intégrité évident.",
        author: 'Yasmina A.',
      },
      {
        text: "Je suis indigné par les pratiques de *****. Les informations qu'ils fournissent sur leurs produits sont trompeuses. Le produit que j'ai reçu était radicalement différent de ce qui était décrit sur leur site. C'est une véritable tromperie.",
        author: 'Fatima N.',
      },
      {
        text: "Je suis outré par les pratiques déloyales de *****. Les informations fournies sur leurs produits sont délibérément trompeuses. Le produit que j'ai reçu était loin de la description en ligne. C'est une véritable duperie.",
        author: 'Nadia N.',
      },
    ],
  },
  {
    url: 'problemes-prelevements-banque-ligne',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Problèmes de prélèvements avec votre banque en ligne',
    seoDescription:
      'Rencontrez-vous des problèmes de prélèvements avec votre banque en ligne ? Signalez le problème pour obtenir une résolution rapide.',
    title: 'Problèmes de Prélèvements avec Votre Banque en Ligne',
    targetedCategory: ['BanqueAssuranceMutuelle'],
    catchPhrase:
      'Signalez un problème de prélèvements avec votre banque en ligne, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi signaler un prélèvement incorrect, un débit excessif ou une transaction non autorisée ?',
    secondaryTitle2: 'Que devient votre signalement relatif à des problèmes de prélèvements avec votre banque en ligne ?',
    sampleReports: [
      {
        text: "J'ai récemment remarqué des prélèvements inexpliqués sur mon compte chez *****. Malgré plusieurs tentatives de contact, le service client ne répond pas à mes préoccupations. Cette situation est très stressante, car ces prélèvements non autorisés perturbent mes finances.",
        author: 'Margaux A.',
      },
      {
        text: "Depuis trois mois, j'observe des prélèvements répétés non autorisés sur mon compte en ligne avec *****. Malgré mes multiples requêtes, la banque n'a fourni aucune explication ni solution.",
        author: 'Zahra M.',
      },
      {
        text: "J'ai des problèmes de prélèvements non autorisés sur mon compte avec *****. J'ai signalé le problème à plusieurs reprises, mais aucune action n'a été entreprise pour résoudre la situation.",
        author: 'Rachid A.',
      },
      {
        text: "Des prélèvements inconnus ont été effectués sur mon compte en ligne chez ***** sans mon consentement. J'ai contacté la banque à plusieurs reprises, mais sans réponse satisfaisante.",
        author: 'Sofia Z.',
      },
      {
        text: "Mon compte en ligne avec ***** a subi plusieurs prélèvements non autorisés au cours des derniers mois. Malgré mes plaintes répétées, la banque n'a pas réussi à résoudre le problème.",
        author: 'Gérard D.',
      },
    ],
  },
  {
    url: 'problemes-punaises-de-lit',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Problèmes relatifs à des punaises de lit',
    seoDescription: 'Avez-vous des problèmes relatifs à des punaises de lit ? Chiens renifleurs ?',
    title: 'Problèmes de punaises de lit',
    targetedCategory: ['ServicesAuxParticuliers'],
    catchPhrase: 'Signalez un problème relatif à des punaises de lit !',
    secondaryTitle1:
      'Pourquoi signaler un problème relatif à des punaises de lit ? Chiens renifleurs ? Prestataires lutte contre les nuisibles ?',
    secondaryTitle2: 'Que devient votre signalement relatif à un problème de punaises de lit ?',
    sampleReports: [
      {
        text: "Je voulais vérifier que je n'avais pas de punaise de lit chez moi. J'ai téléphoné à la société ***** le 27 septembre 2023. J'ai obtenu un rendez-vous le jour même. Deux personnes sont venues avec 2 chiens : à la fin de l'inspection, ils nous ont affirmé que nous avions bien des punaises dans les chambres. Aucun compte-rendu écrit. Suite à ce constat, ils m'ont donné le numéro de téléphone d'une personne pouvant effectuer le traitement. Avant d'appeler cette personne et ayant quelques soupçons sur leur honnêteté, nous avons fait une inspection de fond en comble des chambres (matelas, sommier etc.). Nous avons mis des pièges à punaises de lit. Résultat : aucune punaise de lit n'a été trouvée, aucune trace noire ni tâche de sang sur la literie ni aucune piqure. Nous sommes convaincus que cette société est malhonnête et est en cheville avec le particulier qu'ils conseillent d'appeler pour des punaises de lit imaginaires",
        author: 'Timothée A.',
      },
      {
        text: "L'entreprise est venue quatre fois avec ses chiens renifleurs pour un problème de punaises de lit, le travail a été effectué, mais le problème, je n'arrive plus à joindre cette société pour récupérer ma facture, envoyé des messages, des mails pas de réponse de leur part.",
        author: 'Théo T.',
      },
      {
        text: 'La société ***** où nous avons demandé pour l’intervention le 17 août 2023. Même jour Ils sont venus dans mon appartement et ils sont déjà équipés des produits à appliquer pour tuer des punaises de lit. Ils m’ont fait presser la décision pour réaliser et payer . Et Ils m’ont rassurés que ça a suffit juste une fois de passage . Ils m’ont dit le prix qui était 420€. Quand je paye , ils ont ajouté Tax donc je devais payé 526€ TTC au final. Ce qui m’ont indiqué le prix d’abord (420€) ,c’était le prix HT. C’était déjà le façon malhonnête pour le client. Après nous avons enfin reçu la facture (3 semaines de relance) , et c’est indiqué le prix pour 2 passages alors qu’ils sont venus juste une passage. Nous sommes arnaqués par cette société, nous avons tous essayé pour joindre à eux et sans succès jusqu’à ce jour . Nous souhaitons d’être remboursé par eux , et souhaitons qu’ils n’existent plus ce genre de établissement malhonnête et arnaque qui profite aux gens qui sont en difficulté.',
        author: 'Nadia D.',
      },
      {
        text: "Le 19 août en rentrant de vacances je trouve des punaises de lits chez moi. J'appelle donc la première société que je trouve sur internet. La personne me dit qu'elle arrive et que ça fera 249 euros. J'accepte. A sa venue elle m'explique brièvement ce qui sera fait sans trop de détails. Nous patientons en bas de chez nous et 10 minutes après le début de l'intervention c'est déjà terminé. Je demande pourquoi c'était aussi rapide, il me dit que ça suffit et qu'il faudra qu'il revienne pour une autre intervention. Étant enceinte de 7 mois je demande au bout de combien de temps je peux revenir chez moi, il nous dit 3 heures et bien aérer (c'est normalement 1 semaine). Le lendemain nous découvrons avec mon mari qu'il reste plusieurs punaises de lits vivantes. Je les contacte et je n'ai pas de retours ni de réponses. Je me sens arnaquée, j'ai payé 249 euros pour un service mal fait, et j'ai un doute si ce sont de véritables professionnels. J'espère un geste de leur part",
        author: 'Amine L.',
      },
      {
        text: "J'ai fait appel à ***** pour régler une infection de punaises de lit. Un 1er technicien est venu le 21/05/2023 avec un chien renifleur en me promettant qu'1 seule intervention sera suffisante, il me fait un devis à 430€ alors que sur leur site il est écrit que le traitement coute 240€, il me demande de payer en espèces, ce que je fais. Son intervention dans mon appartement n'a duré que 5min. 3 jours après, j'avais encore des punaises, donc je paie 80€ de frais de déplacement, et le 27/05/2023 un 2ème technicien vient et traite l'appartement en 3min chrono ! le lendemain, j'ai encore des punaises ! Sur leur site est écrit \"Nous incluons dans notre offre un service de contrôle. Le but est de vérifier si le traitement a été efficace et si votre habitat est entièrement désinfecté. Nos experts peuvent aussi vous proposer des conseils de prévention qui permettent d’empêcher une nouvelle infestation.\" C'est faux. Ils me raccrochent au nez et ne veulent rien entendre",
        author: 'Amine D.',
      },
    ],
  },
  {
    url: 'problemes-remboursement',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Problèmes de remboursement',
    seoDescription: 'Signalez un problème de remboursement',
    title: 'Problèmes de remboursement',
    targetedCategory: ['VoyageLoisirs'],
    catchPhrase: 'Signalez un problème de remboursement, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi signaler un problème de remboursement suite à une annulation de voyage ou un achat en ligne ?',
    secondaryTitle2: 'Que devient votre signalement relatif à un problème de remboursement ?',
    sampleReports: [
      {
        text: "Mon niveau de frustration est à son comble. J'ai demandé un remboursement pour un produit défectueux que j'ai acheté sur *****. Malgré de nombreuses relances, ils n'ont toujours pas traité ma demande. C'est inacceptable de leur part de traiter ainsi leurs clients.",
        author: 'Mohamed E.',
      },
      {
        text: "J'ai le sentiment d'être pris pour un imbécile avec *****. J'ai demandé un remboursement pour un article défectueux. Malgré mes relances incessantes, ils n'ont toujours pas daigné traiter ma demande. C'est un manque de respect total envers leurs clients.",
        author: 'Yasmina A.',
      },
      {
        text: "Mon sang bouillonne d'indignation à cause de *****. J'ai demandé un remboursement pour un article défectueux, mais malgré mes multiples relances, ils n'ont pas daigné traiter ma demande. Leur insouciance est inacceptable.",
        author: 'Emma H.',
      },
      {
        text: "Je suis outré par l'attitude de *****. J'ai retourné un article défectueux, demandant un remboursement, mais ils semblent complètement indifférents à ma situation. Leurs actions démontrent un manque de respect flagrant pour leurs clients.",
        author: 'Maria M.',
      },
      {
        text: "Je suis outragé par l'attitude de *****. J'ai retourné un article défectueux en demandant un remboursement, mais ils semblent complètement indifférents à ma situation. C'est une manque de respect flagrant envers leurs clients.",
        author: 'Axel S.',
      },
      {
        text: "Je suis scandalisé par le comportement de *****. J'ai renvoyé un produit défectueux, espérant un remboursement, mais ils semblent ignorer complètement ma situation. C'est un manque de respect flagrant envers leurs clients.",
        author: 'Maria C.',
      },
    ],
  },
  {
    url: 'problemes-remboursement-mutuelle',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Problèmes de remboursement avec votre mutuelle',
    seoDescription:
      "Avez-vous des problèmes de remboursement avec votre mutuelle ? Signalez le problème pour obtenir l'aide nécessaire.",
    title: 'Problèmes de Remboursement avec Votre Mutuelle',
    targetedCategory: ['BanqueAssuranceMutuelle'],
    catchPhrase:
      'Signalez un problème de remboursement avec votre mutuelle, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi signaler un remboursement tardif, un montant incorrect ou un refus non justifié ?',
    secondaryTitle2: 'Que devient votre signalement relatif à un problème de remboursement avec votre mutuelle ?',
    sampleReports: [
      {
        text: "J'ai souscrit une mutuelle chez ***** qui promettait un remboursement rapide et complet pour tous les frais médicaux. Cependant, malgré mes nombreuses demandes de remboursement, je n'ai reçu aucun paiement de leur part.",
        author: 'Maria M.',
      },
      {
        text: "Depuis janvier 2023, ***** refuse de me rembourser les frais médicaux que j'ai engagés, malgré le fait que ces dépenses soient clairement couvertes par ma police d'assurance. J'ai contacté à plusieurs reprises ma mutuelle mais mes demandes de remboursement ont été ignorées.",
        author: 'Jade G.',
      },
      {
        text: "J'ai des problèmes avec ***** concernant le remboursement des frais médicaux. Malgré plusieurs tentatives de contact et la fourniture de tous les documents nécessaires, je n'ai reçu aucun remboursement pour les frais médicaux que j'ai engagés.",
        author: 'Amine Z.',
      },
      {
        text: "Depuis février 2023, j'ai du mal à me faire rembourser par ***** pour des frais médicaux. J'ai fourni tous les documents nécessaires mais mes demandes sont constamment rejetées ou ignorées.",
        author: 'Nathan M.',
      },
      {
        text: "Ma mutuelle, *****, a refusé de me rembourser pour des frais médicaux qui sont supposés être couverts par mon contrat. Malgré de multiples appels et courriels, ils n'ont pas traité ma demande de remboursement et je me retrouve à payer de ma poche des frais médicaux importants.",
        author: 'Alexandre G.',
      },
    ],
  },
  {
    url: 'problemes-reservation-voyage',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Problèmes de réservation de voyage',
    seoDescription: 'Signalez un problème de réservation de voyage',
    title: 'Problèmes de réservation de voyage',
    targetedCategory: ['VoyageLoisirs'],
    catchPhrase: 'Signalez un problème de réservation de voyage, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi signaler un problème de réservation de vol, de train ou de croisière ?',
    secondaryTitle2: 'Que devient votre signalement relatif à un problème de réservation de voyage ?',
    sampleReports: [
      {
        text: "Je suis dans une colère noire suite à mon expérience avec *****. J'ai essayé de réserver un voyage pour ma famille via leur plateforme, seulement pour découvrir qu'il y avait un bug dans leur système. J'ai passé des heures à choisir la destination parfaite, à sélectionner les dates et à entrer tous les détails nécessaires, mais lorsque j'ai cliqué sur \"réserver\", j'ai été redirigé vers une page d'erreur. J'ai essayé à plusieurs reprises, mais le même problème persistait. C'était incroyablement frustrant et a complètement gâché mon expérience.",
        author: 'Sarah C.',
      },
      {
        text: "Une vraie déception que ce site de réservation *****. J'ai tenté d'organiser un voyage surprise pour mon conjoint, passant des heures à peaufiner chaque détail. Lors de la confirmation finale, je suis redirigé vers une page d'erreur. Inutile de dire que j'étais hors de moi après tant d'efforts. Ce genre d'incident est inacceptable de la part d'un tel service.",
        author: 'Jacqueline C.',
      },
      {
        text: "Je suis abasourdi par la débâcle que j'ai vécue avec *****. J'essayais d'organiser un voyage en famille, mais à chaque tentative de réservation, je tombais sur une page d'erreur. C'était comme un coup de poignard dans le dos après toutes les heures que j'avais passées à planifier.",
        author: 'Nathan L.',
      },
      {
        text: "J'ai été dévasté par l'expérience que j'ai vécue avec *****. J'essayais d'organiser un voyage romantique pour ma partenaire, mais chaque tentative se soldait par un échec. Après tant d'efforts et d'excitation, le sentiment de désespoir était accablant.",
        author: 'Mathis F.',
      },
      {
        text: "Je suis déconcerté par l'expérience que j'ai eue avec *****. J'ai passé des heures à essayer de réserver un voyage en ligne, mais à chaque fois, je me suis heurté à des erreurs techniques. C'est décevant et frustrant après tous les efforts que j'ai déployés.",
        author: 'Amira M.',
      },
      {
        text: "Mon expérience avec ***** a été épouvantable. J'ai passé de nombreuses heures à essayer de réserver un voyage, mais je me suis heurté à des bugs et des erreurs. Après tous les efforts et l'enthousiasme que j'ai investis dans ce voyage, c'est dévastateur.",
        author: 'Anna É.',
      },
    ],
  },
  {
    url: 'problemes-sejours-hotel',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: "Problèmes liés aux séjours à l'hôtel",
    seoDescription: "Signalez un problème lié à votre séjour à l'hôtel",
    title: "Problèmes liés aux séjours à l'hôtel",
    targetedCategory: ['VoyageLoisirs'],
    catchPhrase:
      "Signalez un problème lié à votre séjour à l'hôtel, renseignez-vous sur vos droits avec la répression des fraudes !",
    secondaryTitle1: "Pourquoi signaler un problème de réservation, de surbooking ou de conditions d'hébergement à l'hôtel ?",
    secondaryTitle2: "Que devient votre signalement relatif à un problème lors de votre séjour à l'hôtel ?",
    sampleReports: [
      {
        text: "J'ai récemment séjourné dans un hôtel ***** et mon expérience a été bien loin de ce que j'avais espéré. La chambre était sale à notre arrivée et malgré plusieurs demandes, personne n'est venu la nettoyer. Le service était lent et le personnel semblait indifférent à nos préoccupations. C'était censé être des vacances reposantes, mais cela s'est avéré être un véritable cauchemar.",
        author: 'Axel S.',
      },
      {
        text: "C'est avec une grande tristesse que je raconte mon séjour à l'hôtel *****. La chambre était dans un état lamentable, avec une propreté plus que douteuse. Malgré nos multiples requêtes, le personnel a fait preuve d'une indifférence déconcertante. Les vacances tant attendues se sont transformées en un calvaire inoubliable.",
        author: 'Mathis E.',
      },
      {
        text: "Je suis profondément troublé par mon récent séjour à l'hôtel *****. L'état de la chambre était pitoyable et les demandes de nettoyage sont restées sans réponse. Les vacances tant attendues sont devenues une véritable épreuve, c'est affligeant.",
        author: 'Hugo L.',
      },
      {
        text: "Mon séjour à l'hôtel ***** a été une véritable descente aux enfers. L'état de la chambre était déplorable, entre la poussière et les draps tachés. Malgré nos demandes répétées, rien n'a changé. Ce qui devait être une escapade reposante s'est transformé en un véritable cauchemar.",
        author: 'Sarah T.',
      },
      {
        text: "Mon séjour à l'hôtel ***** a été tout simplement désastreux. La chambre était sale, le service était médiocre, et le personnel semblait indifférent à nos plaintes. Ce qui devait être une escapade relaxante s'est avéré être un véritable cauchemar.",
        author: 'Leila Y.',
      },
      {
        text: "Mon séjour à l'hôtel ***** a été une véritable tragédie. La chambre était insalubre, le service était lamentable et le personnel ne tenait pas compte de nos réclamations. Une expérience qui devait être un havre de paix s'est transformée en un enfer.",
        author: 'Théo L.',
      },
    ],
  },
  {
    url: 'problemes-service-client',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Problèmes de service client',
    seoDescription: 'Signalez un problème de service client',
    title: 'Problèmes de service client',
    targetedCategory: ['VoyageLoisirs'],
    catchPhrase: 'Signalez un problème de service client, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: "Pourquoi signaler un problème de service client, de réponse tardive ou d'informations trompeuses ?",
    secondaryTitle2: 'Que devient votre signalement relatif à un problème de service client ?',
    sampleReports: [
      {
        text: "Il est triste de voir à quel point le service client de ***** est incompétent. Quand j'ai rencontré un problème avec mon achat, je les ai contactés pour obtenir de l'aide. Cependant, leur réponse a été lente et inutile. Ils semblaient ne pas comprendre le problème et n'ont pas réussi à fournir une solution. Cette expérience a complètement ruiné ma confiance en eux.",
        author: 'Simone N.',
      },
      {
        text: "C'est désespérant de constater l'inefficacité du service client de *****. Face à un souci d'achat, j'ai tenté de les contacter. Leur réponse a été d'une lenteur exaspérante et totalement inutile. Cette expérience m'a laissé un goût amer et a sévèrement ébranlé ma confiance en eux.",
        author: 'Gérard T.',
      },
      {
        text: "Je suis déconcerté par le service client de *****. Après avoir rencontré un problème avec un produit, j'ai cherché de l'aide, mais leur réponse était inutile et tardive. Cette expérience a laissé un goût amer et a érodé ma confiance en leur service.",
        author: 'Salma S.',
      },
      {
        text: "Je suis scandalisé par le service client de *****. Lorsque j'ai rencontré un problème avec un de leurs produits, j'ai essayé de les contacter, mais leur réponse a été tardive et insatisfaisante. Cette expérience m'a laissé un sentiment de frustration intense.",
        author: 'Émile T.',
      },
      {
        text: "Je suis sidéré par le service client de *****. Lorsque j'ai rencontré un problème avec leur produit, j'ai contacté leur service client, mais leur réponse était non seulement tardive, mais également inutile. C'est une expérience vraiment désagréable.",
        author: 'Zahra J.',
      },
      {
        text: "Je suis abasourdi par le manque de compétence du service client de *****. Lorsque j'ai rencontré un problème avec l'un de leurs produits, j'ai contacté leur service client, mais leur réaction a été à la fois tardive et inadéquate. C'est une expérience déplorable.",
        author: 'Louis S.',
      },
    ],
  },
  {
    url: 'publicite-impact-environnemental',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: "Site internet publicité sur l'impact environnemental",
    seoDescription: "Signalez une publicité sur l'impact environnemental d'un site internet",
    title: "Site internet publicité sur l'impact environnemental",
    targetedCategory: ['AchatInternet'],
    catchPhrase: "Signalez une publicité sur l'impact environnemental sur un site internet",
    secondaryTitle1:
      "Pourquoi signaler une publicité sur l'impact environnemental sur un site internet ? Ecolabel, neutralité en carbone, un produit acheté = arbre planté etc.",
    secondaryTitle2: 'Que se passera-t-il après le dépôt du signalement ?',
    sampleReports: [
      {
        text: 'J’ai commandé une lessive mettant en avant dans le titre et sur la photo de présentation un label écologique Ecolabel européen. Sauf que lorsque j’ai reçu ma commande j’avais la meme lessive mais SANS aucun label. C’est donc complètement mensonger, il est écrit partout lessive écologique, label écologique et finalement rien du tout. Il a tromperie sur la marchandise !',
        author: 'Ali A.',
      },
      {
        text: "J'ai commandé sur un site de grande distribution des packs d'eau de la marque xxx. Sur le pack de 6 bouteilles d'eau plate il est indiqué en énorme : 1 pack acheté, 1 arbre planté. La promesse est énorme ! Mais en regardant les petits caractères, on se rend compte que la procédure pour que son achat permette effectivement la plantation \nd'un arbre est extrêmement compliquée et qu'il y a une restriction sur le nombre d'arbres plantés (4200). Par ailleurs l'offre n'est plus d'actualité, pourtant elle est encore affichée. On est loin de la promesse, c'est clairement mensonger et induit le consommateur en erreur. J'estimais utile de vous le signaler. Bien cordialement",
        author: 'Chloé Z.',
      },
    ],
  },
  {
    url: 'publicite-trompeuse-restaurants',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Publicité\n  trompeuse dans les restaurants',
    seoDescription: 'Signalez\n  une publicité trompeuse dans un restaurant',
    title: 'Publicité\n  trompeuse dans les restaurants',
    targetedCategory: ['CafeRestaurant'],
    catchPhrase: 'Signalez\n  une publicité trompeuse dans un restaurant, ne laissez pas les consommateurs\n  être dupés !',
    secondaryTitle1: 'Pourquoi\n  signaler une publicité trompeuse dans un restaurant ?',
    secondaryTitle2: "Que\n  devient votre signalement d'une publicité trompeuse dans un restaurant ?",
    sampleReports: [
      {
        text: "J'ai visité le restaurant ***** après avoir vu leur publicité pour un menu spécifique. Cependant, une fois sur place, j'ai découvert qu'ils ne proposaient pas ce menu. C'est de la publicité mensongère et c'est absolument inacceptable.",
        author: 'Sarah L.',
      },
      {
        text: "J'ai visité *****, attiré par leur publicité promettant un buffet de fruits de mer frais. Cependant, à mon arrivée, il n'y avait pas de buffet de fruits de mer. Il s'agit clairement d'une publicité trompeuse.",
        author: 'Noémie L.',
      },
      {
        text: "J'ai visité ***** attiré par une publicité vantant un buffet de fruits de mer frais. Cependant, une fois sur place, il n'y avait pas de trace de ce buffet. C'est de la publicité mensongère.",
        author: 'Fatima E.',
      },
      {
        text: "J'ai été déçu par ***** qui avait annoncé une offre spéciale sur leur menu, qui s'est avérée inexistante à mon arrivée. C'est un cas flagrant de publicité trompeuse.",
        author: 'Clara A.',
      },
      {
        text: "Le restaurant ***** a annoncé une promotion qui s'est avérée inexistante lors de ma visite. Il s'agit d'une publicité manifestement trompeuse.",
        author: 'Adam N.',
      },
      {
        text: 'Le restaurant ***** avait un panneau promotionnel qui promettait des réductions spéciales, qui se sont avérées non disponibles lors de ma commande. Cela semble être une publicité trompeuse.',
        author: 'Michel J.',
      },
    ],
  },
  {
    url: 'refus-eau-potable',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: "Refus de donner accès à de l'eau potable dans un café ou un restaurant",
    seoDescription: 'Signalez un problème de refus de donner un accès à une eau potable\n',
    title: "Problèmes d'accès à une eau potable",
    targetedCategory: ['CafeRestaurant'],
    catchPhrase: 'Signalez un problème relatif à un refus de donner accès à une eau potable',
    secondaryTitle1: "Pourquoi signaler un problème relatif à un refus de donner accès à de l'eau potable ?",
    secondaryTitle2: 'Que devient votre signalement relatif à un refus de donner accès à une eau potable ?',
    sampleReports: [
      {
        text: "Ce restaurant refuse de servir de l'eau potable !",
        author: 'Fatima J.',
      },
      {
        text: "Le serveur a refusé de nous servir une carafe d'eau au prétexte que l'établissement n'avait pas d'eau potable. Malgré les remarques de plusieurs tables pour leur rappeler leur obligation, le serveur nous a répondu qu'il travaillait dans la restauration depuis plus de 25 ans et qu'il connaissait bien cette loi.\nEt de rajouter que son patron préférait payer une amende plutôt que de fournir de l'eau gratuitement. Nous avons été forcé d'acheter une bouteille d'eau plate de 75cl !!!",
        author: 'Jules I.',
      },
      {
        text: "Après avoir réglé deux repas au self, j'ai demandé une carafe d'eau au bar, avec le ticket de caisse en main. Le barman m'a répondu qu'il n'avait pas de carafe, puis m'a dit qu'il n'y avait pas d'eau potable. Impossible d'avoir de l'eau, hormis l'acheter en bouteille. Je n'ai pas vu d'affichage à l'entrée du \nrestaurant, ou près du bar ou du self, mentionnant la non existence d'eau potable. Je doute de la véracité des propos du barman concernant une eau non potable dans son établissement, vu l'emplacement !\n",
        author: 'Soukaina S.',
      },
    ],
  },
  {
    url: 'renouvellement-abonnement-non-desire',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: "Problèmes de renouvellement d'abonnement non désiré",
    seoDescription:
      "Vous rencontrez des problèmes de renouvellement d'abonnement non désiré ? Signalez-le pour arrêter les frais inutiles.",
    title: "Renouvellement d'Abonnement Non Désiré",
    targetedCategory: ['BanqueAssuranceMutuelle'],
    catchPhrase:
      "Signalez un problème de renouvellement d'abonnement non désiré, renseignez-vous sur vos droits avec la répression des fraudes !",
    secondaryTitle1: 'Pourquoi signaler un renouvellement non souhaité, une résiliation ignorée ou une facturation continue ?',
    secondaryTitle2: "Que devient votre signalement relatif à un renouvellement d'abonnement non désiré ?",
    sampleReports: [
      {
        text: "J'ai essayé d'annuler mon abonnement chez ***** avant le renouvellement automatique, mais ils l'ont quand même renouvelé et ont prélevé les frais d'abonnement de mon compte.",
        author: 'Yasmina M.',
      },
      {
        text: "J'ai annulé mon abonnement chez *****, mais ils ont continué à le renouveler automatiquement et à me facturer. J'ai contacté le service client, mais ils n'ont pas réussi à résoudre le problème.",
        author: 'Emma É.',
      },
      {
        text: "J'ai été facturé pour le renouvellement d'un abonnement que j'ai annulé avec *****. Malgré mes tentatives de résolution de ce problème, je n'ai pas reçu de réponse de leur part.",
        author: 'Anna N.',
      },
      {
        text: "J'ai annulé mon abonnement avec *****, mais ils ont continué à me facturer pour un renouvellement automatique. J'ai tenté plusieurs fois de les contacter, sans succès.",
        author: 'Amine M.',
      },
      {
        text: "J'ai résilié mon abonnement chez *****, mais j'ai quand même été facturé pour le renouvellement automatique. Le service client n'a pas été en mesure de résoudre ce problème malgré mes nombreuses plaintes.",
        author: 'Émile N.',
      },
    ],
  },
  {
    url: 'repas-sur-place-couverts-jetables',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Le repas sur place est servi avec des couverts jetables',
    seoDescription: 'Signalez le fait que le repas sur place est servi avec des couverts jetables',
    title: 'Repas sur place servi avec des couverts jetables',
    targetedCategory: ['CafeRestaurant'],
    catchPhrase: 'Signalez le fait que le repas sur place est servi avec des couverts jetables',
    secondaryTitle1: 'Pourquoi signaler le fait que le repas sur place est servi avec des couverts jetables ?',
    secondaryTitle2: "Que devient votre signalement à propos d'un repas sur place servi avec des couverts jetables ?",
    sampleReports: [
      {
        text: "Distribution d'une cuillère en plastique jetable pour chaque achat d'une glace en cornet, même aux clients qui ne le demandent pas. Le patron allègue qu'il a quantité de stock. Je souhaite signaler ce commerçant délinquant. Se poster quelques minutes devant ce commerce devrait permettre de faire le constat \ndu délit. La distribution de couverts jetables est interdite depuis le 1er janvier 2021.",
        author: 'Axel T.',
      },
      {
        text: "Dans ce restaurant, c'est systématique, pour le déjeuner on a le droit à des assiettes en couverts jetables en plastique mais pour le diner c'est autre chose.",
        author: 'Soukaina S.',
      },
      {
        text: "Dans cette brasserie, il est usuel d'utiliser des assiettes et des couverts jetables en plastique pour manger sur place, je croyais que c'était interdit !\n",
        author: 'Enzo C.',
      },
    ],
  },
  {
    url: 'resiliation-non-respectee-contrat-assurance',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: "Résiliation non respectée de votre contrat d'assurance",
    seoDescription:
      "Votre assurance n'a-t-elle pas respecté la résiliation de votre contrat ? Signalez l'incident pour obtenir justice.",
    title: "Résiliation Non Respectée de Votre Contrat d'Assurance",
    targetedCategory: ['BanqueAssuranceMutuelle'],
    catchPhrase:
      "Signalez un problème de résiliation non respectée de votre contrat d'assurance, renseignez-vous sur vos droits avec la répression des fraudes !",
    secondaryTitle1: 'Pourquoi signaler une résiliation ignorée, un prélèvement continu ou une pénalité non justifiée ?',
    secondaryTitle2: "Que devient votre signalement relatif à une résiliation non respectée de votre contrat d'assurance ?",
    sampleReports: [
      {
        text: "J'ai demandé la résiliation de mon contrat d'assurance chez ***** en avril 2023. Cependant, malgré ma demande, ils continuent à prélever les cotisations sur mon compte bancaire. J'ai tenté à plusieurs reprises de les contacter, sans succès.",
        author: 'Clara J.',
      },
      {
        text: 'Malgré ma demande écrite de résiliation de mon contrat avec *****, envoyée en mai 2023, ils continuent à prélever des frais mensuels sur mon compte. Mes tentatives pour résoudre ce problème ont été ignorées.',
        author: 'Myriam M.',
      },
      {
        text: "J'ai demandé à ***** de résilier mon contrat d'assurance en mars 2023. Malgré cela, ils continuent à prélever des primes d'assurance sur mon compte bancaire. J'ai essayé de les contacter à plusieurs reprises mais sans réponse.",
        author: 'Théo A.',
      },
      {
        text: "En février 2023, j'ai envoyé une demande de résiliation de mon contrat d'assurance à *****. Malgré cela, ils continuent à prélever les primes mensuelles sur mon compte. J'ai contacté plusieurs fois le service client, mais mes demandes sont restées sans réponse.",
        author: 'Soukaina C.',
      },
      {
        text: "J'ai résilié mon contrat avec ***** en janvier 2023, mais ils continuent à me facturer des primes mensuelles. J'ai tenté à plusieurs reprises de les contacter pour résoudre ce problème, mais je n'ai reçu aucune réponse.",
        author: 'Margaux F.',
      },
    ],
  },
  {
    url: 'retard-train',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Retard train',
    seoDescription: "Signalez un problème lié au retard d'un train",
    title: 'Retard train',
    targetedCategory: ['VoyageLoisirs'],
    catchPhrase: "Signalez un problème lié au retard d'un train, renseignez-vous sur vos droits avec la répression des fraudes !",
    secondaryTitle1: 'Pourquoi faire un signalement pour un problème de retard, annulation ou remboursement de train ?',
    secondaryTitle2: 'Que devient votre signalement relatif à un retard, une annulation ou un remboursement de train ?',
    sampleReports: [
      {
        text: "Bonjour,  nous n'avons pas reçu le remboursement de 75% du billet suite à un train retardé ! La compagnie xxx nous a répondu qu'on avait pas répondu à leur proposition de dédommagement forfaitaire de 40€. Nous n'avons pas vu votre mail à temps donc nous n'avons pas de remboursement ?  Nous voulions le remboursement dû. Nous sommes arrivés ce jour là à 1h30 du matin à la gare de xxx, heureusement qu'un ami a pu venir nous chercher sinon nous aurions dormi à la gare !! Bref  4 mois après, aucune trace de réponse de la compagnie xxx, c'est inadmissible d'autant qu'il y a toujours autant de trains en retard !",
        author: 'Ali J.',
      },
      {
        text: "Madame, Monsieur, \nSuite à 2 trajets TGV annulés, les 7 et 8 février 2023, j'ai demandé le remboursement de mes billets correspondants. 12/02/2023: 1ère demande. 14/02/2023, je refais une demande suivant les instructions de mes interlocuteurs. 21/03/2023 : après relance, nouvelle demande... qui m'est refusée par le système car les trajets en question font déjà l'objet d'une demande de remboursement. 22/03/2023: relance par mail. 15/04/2023: demande refusée au titre que \"dans le cadre du traitement des remboursements de billets et des indemnisations sur le réseau la compagnie xxx en accord avec l’autorité organisatrice, la région xxx, ne rembourse pas les montants inférieurs à 4 € conformément aux tarifs voyageurs nationaux et à la convention blablabla.\" 17/04/2023: demande est faite d'obtenir copie des textes dont il est fait référence pour justifier le refus de remboursement suite à l'annulation de ces trains.",
        author: 'Hugo N.',
      },
      {
        text: "Bonjour, Voici mon mail adressé à l'entreprise xxx qui ne souhaite pas me rembourser du supplément de 60€ que j'ai dû verser de par leur faute la veille du départ pour au début retard puis annulation du train faute de grève car aucune solution n'était proposée sur leur plateforme et également du billet initial de 25€ que je n'ai jamais utilisé chez eux puisque j'ai pris un autre train via une autre compagnie. En vous remerciant.",
        author: 'Émile M.',
      },
      {
        text: "La compagnie xxx s'est engagée à effectuer un remboursement sous 1 mois suite à un retard plus que conséquent d'un train dû à un mouvement de grève. Après 1 mois je n'ai eu aucun retour, j'ai à nouveau effectué une demande par email après avoir eu la compagnie au téléphone. Le trajet de train retardé date du 6 avril 2023, la compagnie devait donc effectuer le remboursement au plus tard le 6 mai 2023. Aucun retour depuis.",
        author: 'Mohamed D.',
      },
      {
        text: "J'avais réservé 4 billets TGV pour le 25 mars 2023 à 13h31. Nous avons été informés que notre train était supprimé compte tenu des mouvements sociaux. Nous avons donc du procéder à l'annulation des 4 billets (nous devons donc nous-mêmes faire la démarche d'annuler nos billets alors que l'annulation provient de la compagnie) et en racheter 4 à un autre horaire. En raison des (multiples et désormais habituels problèmes techniques tant de l'application que du site de la compagnie (outre l'impossibilité de joindre quelqu'un) je n'ai pu matériellement annuler que 2 billets sur les 4. Cela fait désormais deux mois et malgré mes nombreuses réclamations, je n'ai ni nouvelle ni remboursement des 146 euros qui me sont dus au titre des deux billets restants.",
        author: 'Yasmina N.',
      },
      {
        text: "Le vendredi 24 mars 2023 le train de mon fils et de sa copine a subi un retard de plus de quatre heures. Je n'ai toujours pas été remboursée des 30€ que j'ai payé alors qu'il a fallut aller les chercher à plus de 300 kilomètres !!",
        author: 'Nour Z.',
      },
    ],
  },
  {
    url: 'sinistre-non-resolu-assurance',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Sinistre non résolu avec votre assurance',
    seoDescription:
      "Vous avez un sinistre non résolu avec votre assurance ? Signalez l'incident pour résoudre le problème rapidement.",
    title: 'Sinistre Non Résolu avec Votre Assurance',
    targetedCategory: ['BanqueAssuranceMutuelle'],
    catchPhrase:
      'Signalez un problème de sinistre non résolu avec votre assurance, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi signaler un remboursement tardif, un montant non conforme ou une réclamation ignorée ?',
    secondaryTitle2: 'Que devient votre signalement relatif à un sinistre non résolu avec votre assurance ?',
    sampleReports: [
      {
        text: "En mars 2023, j'ai eu un accident de voiture pour lequel j'étais clairement la partie non fautive, comme l'a confirmé le rapport de police. J'ai immédiatement informé mon assurance, *****. Ils m'ont assuré qu'ils traiteraient le sinistre et organiseraient la réparation de mon véhicule. Malgré de multiples relances et des promesses de règlement, je suis toujours sans voiture plus de quatre mois après l'accident et mon assurance semble faire la sourde oreille.",
        author: 'Ayoub N.',
      },
      {
        text: "En janvier 2023, j'ai déclaré un dégât des eaux à mon assurance habitation, *****, causé par une fuite provenant de l'appartement du dessus. J'ai rempli tous les documents nécessaires et fourni toutes les preuves demandées. Malgré cela, il m'a été répété à plusieurs reprises que mon dossier était en cours de traitement. Cela fait maintenant plus de six mois et je n'ai reçu aucune compensation pour les dommages subis.",
        author: 'Jacqueline A.',
      },
      {
        text: "J'ai souscrit à une assurance santé chez *****. En avril 2023, j'ai eu une intervention chirurgicale pour laquelle j'ai payé les frais et ensuite demandé un remboursement à mon assurance. Malgré plusieurs relances, et après avoir fourni toutes les factures et les documents médicaux demandés, je n'ai pas reçu de remboursement et mon assurance semble ignorer mes demandes.",
        author: 'Zoé A.',
      },
      {
        text: "En février 2023, un incendie a causé des dommages importants à ma maison. J'ai immédiatement informé mon assurance habitation, *****. Après une longue période d'attente et de nombreux appels, ils ont finalement envoyé un expert pour évaluer les dommages. Cependant, ils ont minimisé l'étendue des dommages et m'ont offert un montant de dédommagement largement inférieur aux coûts de réparation. Malgré mes objections et les preuves fournies par des professionnels de la construction, ils refusent d'augmenter le montant du dédommagement.",
        author: 'Adam C.',
      },
      {
        text: "J'ai souscrit à une assurance vie chez *****. Selon mon contrat, j'étais censé recevoir un paiement à l'âge de 65 ans. Quand j'ai atteint l'âge en question en juin 2023 et contacté mon assurance pour demander le paiement, ils m'ont informé que je ne remplissais pas les conditions pour recevoir le paiement, sans expliquer clairement quelles étaient ces conditions. Malgré plusieurs tentatives pour clarifier la situation, je suis laissé dans l'incertitude.",
        author: 'Emma A.',
      },
    ],
  },
  {
    url: 'soucis-hygiene-restauration',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: "Soucis\n  d'hygiène dans le secteur de la restauration",
    seoDescription: "Signalez\n  un problème d'hygiène généralisé en restauration",
    title: "Soucis\n  d'hygiène dans le secteur de la restauration",
    targetedCategory: ['CafeRestaurant'],
    catchPhrase: "Signalez\n  un problème d'hygiène dans le secteur de la restauration, nous sommes là pour\n  vous aider !",
    secondaryTitle1: "Pourquoi\n  signaler un souci d'hygiène en restauration ?",
    secondaryTitle2: "Que\n  devient votre signalement d'un souci d'hygiène en restauration ?",
    sampleReports: [
      {
        text: "Je suis allé dîner au restaurant ***** et j'ai remarqué plusieurs problèmes d'hygiène. Les serveurs ne portaient pas de gants lorsqu'ils manipulaient les aliments, et j'ai vu un serveur éternuer sans se couvrir la bouche puis continuer à servir la nourriture. Je suis vraiment dégoûté et je ne compte plus jamais y retourner.",
        author: 'Chloé F.',
      },
      {
        text: "Hier, j'ai été témoin de plusieurs manquements aux règles d'hygiène de base au restaurant *****. Le personnel ne portait pas de masques ou de gants, continuait de toucher leur visage et leurs cheveux tout en manipulant la nourriture. C'est inacceptable et un risque pour la santé.",
        author: 'Nadia A.',
      },
      {
        text: "J'ai observé de nombreuses pratiques hygiéniques douteuses lors de ma dernière visite au restaurant *****. Le personnel manipulait la nourriture sans protections, touchant régulièrement leur visage. Cela est inadmissible et potentiellement dangereux.",
        author: 'Sarah A.',
      },
      {
        text: "Le manque d'hygiène chez ***** est sidérant. J'ai été témoin de pratiques inacceptables, notamment du personnel touchant leur visage puis manipulant la nourriture sans se laver les mains. C'est un véritable risque pour la santé des clients.",
        author: 'Dylan H.',
      },
      {
        text: "Chez *****, j'ai vu le personnel manipuler de la nourriture après avoir manipulé de l'argent, sans se laver les mains entre les deux. De telles pratiques hygiéniques sont inexcusables.",
        author: 'Axel S.',
      },
      {
        text: "J'ai remarqué, avec consternation, que les employés de chez ***** semblaient ignorer les protocoles sanitaires de base, manipulant des aliments sans gants ni lavage des mains régulier.",
        author: 'Dylan É.',
      },
    ],
  },
  {
    url: 'substitution-assurance-emprunteur-non-respectee',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: "Substitution d'assurance emprunteur non respectée",
    seoDescription:
      "Votre droit à la substitution d'assurance emprunteur a-t-il été non respecté ? Signalez-le pour obtenir de l'aide.",
    title: "Substitution d'Assurance Emprunteur Non Respectée",
    targetedCategory: ['BanqueAssuranceMutuelle'],
    catchPhrase:
      "Signalez un problème de substitution d'assurance emprunteur non respectée, renseignez-vous sur vos droits avec la répression des fraudes !",
    secondaryTitle1:
      'Pourquoi signaler un refus injuste de substitution, une modification non autorisée ou un manque de transparence ?',
    secondaryTitle2: "Que devient votre signalement relatif à une substitution d'assurance emprunteur non respectée ?",
    sampleReports: [
      {
        text: "J'ai souhaité substituer mon assurance emprunteur chez ***** avec une autre compagnie qui offre de meilleures conditions, mais ils ont refusé ma demande sans motif valable.",
        author: 'Anna S.',
      },
      {
        text: "J'ai demandé à ***** de substituer mon assurance emprunteur. Malgré les dispositions légales, ils ont refusé ma demande sans fournir de raisons valables.",
        author: 'Nadia N.',
      },
      {
        text: "J'ai tenté de changer mon assurance emprunteur chez *****, mais ma demande a été rejetée sans explication. C'est très frustrant car j'ai trouvé une meilleure offre ailleurs.",
        author: 'Enzo C.',
      },
      {
        text: "Malgré mon droit à la substitution d'assurance emprunteur, ***** a rejeté ma demande sans fournir une justification claire.",
        author: 'Margaux L.',
      },
      {
        text: "J'ai demandé la substitution de mon assurance emprunteur chez *****, mais ils ont refusé ma demande sans me fournir de motif valable. C'est non seulement frustrant, mais aussi contraire à mes droits en tant que consommateur.",
        author: 'Nour R.',
      },
    ],
  },
  {
    url: 'tromperie-allegation-label-environnement-internet',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Utilisation fausse ou trompeuse d’allégations ou labels environnementaux sur un site internet',
    seoDescription: 'Signalez l’utilisation frauduleuse d’allégations ou labels environnementaux sur un site internet',
    title: 'Utilisation frauduleuse d’allégations ou labels environnementaux sur un site internet',
    targetedCategory: ['AchatInternet'],
    catchPhrase:
      'Signalez l’utilisation frauduleuse d’allégations ou labels environnementaux sur un site internet, pour éviter qu’il y ait d’autres victimes !',
    secondaryTitle1:
      'Pourquoi signaler l’utilisation frauduleuse d’allégations ou labels environnementaux sur un site internet ?',
    secondaryTitle2:
      'Que devient votre signalement relatif à l’utilisation frauduleuse d’allégations ou labels environnementaux sur un site internet ?',
    sampleReports: [
      {
        text: "Lors de ma dernière visite sur votre site, j'ai été troublée de constater que plusieurs produits étaient mmis en avant comme étant \"respectueux de l'environnement\" sans preuves tangibles pour étayer ces allégations. Par exemple, des produits de nettoyage étaient présentés comme étant \"100% naturels\" alors qu'ils contenaient encore des ingrédients potentiellement nocifs pour l'environnement.\nEn tant que consommatrice soucieuse de l'environnement, je trouve cette pratique trompeuse et préjudiciable, car elle induit en erreur les clients qui cherchent à faire des choix respectueux de l'environnement. De fausses allégations environnementales non seulement trompent les consommateurs, mais elles sapent également les efforts collectifs visant à promouvoir des modes de vie plus durables et à protéger notre planète.\nMerci de revoir vos pratiques.",
        author: 'Adam L.',
      },
      {
        text: 'Je suis client régulier de votre boutique en ligne et je suis déçu par la présence de fausses allégations environnementales sur certains produits que vous vendez. Lors de ma dernière commande, j\'ai remarqué que plusieurs articles étaient étiquetés comme étant "entièrement durables" ou "respectueux de l\'environnement", mais après avoir examiné de plus près les informations sur ces produits, j\'ai découvert que ces allégations étaient trompeuses et fausses !\nPar exemple, un sac à dos était promu comme étant fabriqué à partir de "matériaux écologiques", mais en réalité, il contenait des composants plastiques non recyclables. Cette utilisation de fausses allégations environnementales est non seulement trompeuse pour les consommateurs, mais elle est également néfaste pour l\'environnement, car elle induit en erreur les clients qui cherchent à prendre des décisions d\'achat plus durables.\nMerci de revoir vos pratiques de marketing douteuses et de vous assurer que toutes les allégations environnementales sont précises et vérifiables. ',
        author: 'Michel A.',
      },
      {
        text: 'Lors de ma dernière commande, j\'ai remarqué que certains articles étaient présentés comme étant "biodégradables" ou "fabriqués à partir de matériaux recyclés", mais après recherches, j\'ai découvert que ces allégations étaient fausses.\nJ\'ai acheté un ensemble de vaisselle qui était décrit comme étant "100% compostable", mais après avoir examiné les spécifications du produit, j\'ai découvert qu\'il contenait en réalité des additifs plastiques non biodégradables. Merci de veiller à adopter une approche transparente et honnête en matière de communication sur l\'impact environnemental de vos produits ainsi vous renforcerez la confiance des consommateurs et contribuerez à promouvoir une consommation plus responsable. ',
        author: 'Salma N.',
      },
    ],
  },
  {
    url: 'tromperie-allegation-label-environnement-magasin',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Utilisation fausse ou trompeuse d’allégations ou labels environnementaux dans un magasin',
    seoDescription: 'Signalez l’utilisation frauduleuse d’allégations ou labels environnementaux dans un magasin',
    title: 'Utilisation frauduleuse d’allégations ou labels environnementaux dans un magasin',
    targetedCategory: ['AchatMagasin'],
    catchPhrase:
      'Signalez l’utilisation frauduleuse d’allégations ou labels environnementaux dans un magasin, pour éviter qu’il y ait d’autres victimes !',
    secondaryTitle1: 'Pourquoi signaler l’utilisation frauduleuse d’allégations ou labels environnementaux dans un magasin ?',
    secondaryTitle2:
      'Que devient votre signalement relatif à l’utilisation frauduleuse d’allégations ou labels environnementaux dans un magasin ?',
    sampleReports: [
      {
        text: "En visitant le site web de votre société de cosmétiques, j'ai été attirée par leurs affirmations selon lesquelles leurs produits étaient entièrement naturels et respectueux de l'environnement. Cependant, après avoir examiné de près la liste des ingrédients de plusieurs produits, j'ai découvert la présence de nombreux produits chimiques nocifs et de substances synthétiques potentiellement dangereuses pour la santé humaine et pour l'environnement. Cette utilisation frauduleuse de l'étiquetage environnemental induit les consommateurs en erreur et les expose à des risques pour leur santé et pour l'environnement. Il serait souhaitable que la mise en avant de ces allégations cesse rapidement.",
        author: 'Michel S.',
      },
      {
        text: "En tant que cliente régulière de votre café, j'ai été surprise de constater que l'établissement affichait fièrement des labels environnementaux sur ses emballages de café, affirmant qu'ils étaient issus de plantations durables et respectueuses de l'environnement. Cependant, après avoir effectué des recherches , j'ai découvert que ces allégations étaient fallacieuses. Les plantations de café associées à ce café ne respectaient pas les normes environnementales requises et étaient même accusées de déforestation et de pratiques agricoles destructrices. Cette utilisation frauduleuse de labels environnementaux est trompeuse pour les consommateurs et nuit à la réputation des produits authentiquement durables.",
        author: 'Juliette R.',
      },
      {
        text: "J'ai acheté des vêtements dans votre magasin spécialisé dans les vêtements écologiques et équitables. J'ai remarqué plusieurs étiquettes affirmant que les produits étaient fabriqués à partir de matériaux recyclés et respectueux de l'environnement. Cependant, j'ai bien peur que ces allégations soient trompeuses. De nombreux articles semblent être fabriqués à partir de matériaux synthétiques nocifs pour l'environnement. Cette utilisation frauduleuse d'étiquettes environnementales nuit à la confiance des consommateurs.",
        author: 'Ali A.',
      },
    ],
  },
  {
    url: 'vehicule-neuf',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Véhicule neuf',
    seoDescription: 'Votre problème concerne un véhicule neuf ?',
    title: 'Véhicule neuf',
    targetedCategory: ['VoitureVehiculeVelo'],
    catchPhrase:
      'Signalez un problème relatif à un véhicule neuf, renseignez-vous sur vos droits avec la répression des fraudes !',
    secondaryTitle1: 'Pourquoi faire un signalement pour un problème lié à un véhicule neuf ? Voiture neuve ?',
    sampleReports: [
      {
        text: "Mon véhicule, XXX, livré le 16/12/2019, présente depuis de nombreux mois ces défauts : ‐ A‐coups lorsque le moteur est chaud ‐ Moteur qui broute et repart ensuite doucement ‐ Aiguille de compte‐tours qui vacille / oscille Ma concession XXX en a localisé l'origine: défauts de la boite de vitesse robotisée. Le constructeur, informé de ces éléments le 05/05/2023, juge nécessaire de réparer de la sorte: - double vidange de boite - correction de la programmation électronique de la boite de vitesse. Ceci pour la somme de 896,28€. Ces défauts provenant de la boite de vitesse, avant même 70 000km, ils s’apparenteraient à un vice caché (d'après mes assistances juridiques : article 1641 du code civil). C'est pourquoi, j'ai sollicité le constructeur pour procéder à la correction de ces défauts, à leurs frais, sur mon véhicule. Il\n a refusé ma demande de prendre, partiellement ou totalement à leurs frais.",
        author: 'Timothée S.',
      },
      {
        text: 'Bonjour, Je voudrais signaler mon litige avec XXX concernant un véhicule neuf en attende de livraison depuis le 15/10/2021. Puis pour non conformité contractuelle du véhicule depuis le 16/06/2023. (voir courrier en PJ). Cordialement',
        author: 'Simone S.',
      },
      {
        text: "J'ai acheté un véhicule neuf via le site internet XXX. Malgré plusieurs demandes et relances, il refuse de me remettre le bon de commande. Hors, ayant mis mon ancien véhicule à la destruction, ma demande de prime à la conversion se voit refusée car le \"bon\" de réservation communiqué est irrecevable. Notons également que la facture m'a été communniquée suite à mes nombreuses demandes 2 mois après la livraison de mon véhicule. Outre le fait que ce refus est probablement contraire à leurs obligations, il me prive de mes droits à l'aide à l'acquisition d'un véhicule propre.\n",
        author: 'Zahra D.',
      },
      {
        text: 'Bonjour, j’ai acheté un véhicule neuf début janvier en LOA et revendu mon autre véhicule en contrepartie par la même occasion dans la même entreprise. J’ai acheté ce véhicule début janvier pour une livraison (contrat signé) fin mars. Cependant je n’ai reçu le véhicule que fin janvier sans nouvelles de leur part. J’ai donc du garder et payer mon autre véhicule que j’avais en LOA pendant tout ce temps. Lors de la livraison de mon véhicule neuf le 27/06/23 il me devait 722€ que je n’ai toujours pas et je n’ai en plus aucunes nouvelles de leur part malgré mes appels et mails. De plus je viens de voir que je vais être prélevé dans quelque jour pour mon ancienne voiture que je n’ai plus et que je leur ai vendu. Je souhaite\n donc être remboursée rapidement car la situation devient critique et je risque de porter plainte. Cela fait des mois que ça dure…',
        author: 'Jade M.',
      },
      {
        text: 'J’ai acheté un véhicule neuf au concessionnaire XXX et celle ci vient d’avoir un problème d’AdBlue, la voiture a a peine 2 ans et demi et même pas 20000kms. Nous l’avons mis en diagnostic au concessionnaire où nous l’avons acheté et après diagnostic ils disent que la pièce de réservoir d’adblue a un problème et qu’il faut la changer. Ils prennent en charge à 100% la pièce mais pas la main d’œuvre, mais j’ai lu qu’il y avait eu des problèmes de défaut de fabrication de cette pièce admis par Peugeot, qu’il y a eu un rappel de voiture (pas sur ma série) donc comment savoir si ma voiture n’a pas eu un défaut de fabrication sachant qu’elle a a peine 2 ans et demi et aussi peu de kms. Puisque si le défaut de fabrication n’avait pas eu lieu je n’aurai pas eu besoin de changer cette pièce et donc de payer cet extra. D’autre part ils ne veulent pas me donner d’attestation indiquant que la pièce d’origine n’avait pas de défaut de fabrication. Merci.\n',
        author: 'Zoé J.',
      },
    ],
  },
  {
    url: 'vente-de-produit-plastique-a-usage-unique-internet',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'vente de produits en plastique à usage unique sur internet',
    seoDescription: 'Signalez la vente de produits en plastique à usage unique sur internet.',
    title: 'Vente de produits en plastique à usage unique sur internet',
    targetedCategory: ['AchatInternet'],
    catchPhrase:
      'Signalez la commercialisation sur internet de produits en plastique à usage unique et contribuez ainsi à préserver notre environnement pour les générations futures.',
    secondaryTitle1: 'Pourquoi signaler la commercialisation sur internet de produits en plastique à usage unique ?',
    secondaryTitle2:
      'Que devient votre signalement relatif à la commercialisation sur internet de produits en plastique à usage unique ?',
    sampleReports: [
      {
        text: "Lors de ma récente commande sur votre boutique en ligne, j'ai été consterné de constater que certains articles étaient livrés avec un emballage excessif en plastique à usage unique. Cette pratique est extrêmement préoccupante, car elle va à l'encontre des valeurs environnementales que votre entreprise prétend défendre. Non seulement l'utilisation excessive de plastique contribue à la pollution environnementale, mais elle témoigne également d'un manque de considération pour l'impact écologique de vos opérations. En tant que client engagé dans la réduction des déchets plastiques, je suis profondément déçu de constater que votre boutique en ligne ne prend pas les mesures nécessaires pour limiter son empreinte plastique.\nJe vous encourage donc vivement à revoir vos pratiques d'emballage et à adopter des alternatives plus durables, telles que l'utilisation de matériaux recyclés, compostables ou réutilisables. En prenant des mesures concrètes pour réduire l'utilisation de plastique à usage unique, vous pourriez non seulement contribuer à la préservation de l'environnement, mais également renforcer la confiance des consommateurs dans votre engagement en faveur du développement durable.",
        author: 'Sofia N.',
      },
      {
        text: "Cliente de votre boutique en ligne, j'ai récemment passé une commande pour des produits écologiques. Cependant, à ma grande déception, que ces articles étaient livrés dans des conditionnements en plastique à usage unique. Cette pratique est contraire à mes principes écologiques et elle est également inacceptable dans le contexte actuel de sensibilisation croissante à la pollution plastique.\nCette utilisation excessive de plastique contribue de manière significative à la crise environnementale que nous traversons et représente une menace sérieuse pour la biodiversité et la santé de notre planète. \nJe vous demande donc instamment de faire une effort sur vos pratiques d'emballage en prenant des mesures concrètes pour réduire votre utilisation de plastique à usage unique. Vous avez un rôle important à jouer et vous pourriez inspirer d'autres entreprises. Merci.",
        author: 'Dylan N.',
      },
    ],
  },
  {
    url: 'vente-de-produit-plastique-a-usage-unique-magasin',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'vente de produits en plastique à usage unique dans un magasin',
    seoDescription: 'Signalez la vente de produits en plastique à usage unique dans un magasin.',
    title: 'Vente de produits en plastique à usage unique dans un magasin',
    targetedCategory: [],
    catchPhrase:
      'Signalez la commercialisation de produits en plastique à usage unique dans un magasin et contribuez ainsi à préserver notre environnement pour les générations futures.',
    secondaryTitle1: 'Pourquoi signaler la commercialisation de produits en plastique à usage unique dans un magasin ?',
    secondaryTitle2:
      'Que devient votre signalement relatif à la commercialisation de produits en plastique à usage unique dans un magasin ?',
    sampleReports: [
      {
        text: "Lors de ma dernière visite dans votre café, j'ai été consterné de constater que vous continuez à commercialiser des gobelets en plastique à usage unique pour servir vos boissons chaudes à emporter. En tant que client engagé dans la réduction des déchets plastiques, cette pratique va à l'encontre de mes valeurs et de mes efforts personnels.\nNon seulement les gobelets en plastique à usage unique sont néfastes pour l'environnement en raison de leur impact sur les déchets plastiques, mais ils représentent également une source de pollution majeure pour les écosystèmes, notamment marins. Leur fabrication, leur utilisation et leur élimination contribuent à la crise mondiale de la pollution plastique, qui menace la biodiversité et la santé humaine.\nJe trouve particulièrement regrettable que votre café continue de promouvoir une pratique aussi dommageable pour l'environnement. Je vous encourage vivement à reconsidérer votre approche et à envisager des alternatives durables telles que des gobelets réutilisables ou compostables. Je suis prêt à soutenir ces initiatives et à continuer à fréquenter votre établissement si des mesures concrètes sont prises pour réduire l'utilisation de plastique à usage unique.",
        author: 'Simone Y.',
      },
      {
        text: "En faisant mes courses dans votre supermarché, j'ai été désolée de constater que vous continuez à vendre des fruits et légumes emballés dans des barquettes en plastique à usage unique. Cette pratique est inacceptable à mes yeux, car elle contribue de manière significative à la pollution plastique et va à l'encontre de mes valeurs environnementales. Les emballages plastiques à usage unique sont une source majeure de déchets qui représentent une menace pour la faune et la flore. Je suis profondément préoccupée par l'impact de ces emballages sur notre planète et les générations futures.\nMerci d'adopter une politique d'emballage plus responsable et des alternatives plus durables, telles que des emballages compostables ou des solutions sans emballage pour les fruits et légumes. En prenant des mesures concrètes pour réduire l'utilisation de plastique à usage unique, vous pourriez véritablement contribuer à la préservation de l'environnement et renforcer la confiance des consommateurs dans votre engagement écologique.",
        author: 'Salma A.',
      },
    ],
  },
  {
    url: 'violation-normes-conservation-aliments',
    lang: 'fr',
    isSemiAutomatic: false,
    seoTitle: 'Violation\n  des normes de conservation des aliments',
    seoDescription: 'Signalez\n  une violation des normes de conservation des aliments',
    title: 'Violation\n  des normes de conservation des aliments',
    targetedCategory: ['CafeRestaurant'],
    catchPhrase: 'Signalez\n  une violation des normes de conservation des aliments, protégez les autres\n  consommateurs !',
    secondaryTitle1: 'Pourquoi\n  signaler une violation des normes de conservation des aliments ?',
    secondaryTitle2: "Que\n  devient votre signalement d'une violation des normes de conservation des\n  aliments ?",
    sampleReports: [
      {
        text: "Chez *****, j'ai remarqué que les produits alimentaires n'étaient pas correctement stockés ou conservés. Les aliments périssables étaient laissés à l'air libre, exposés à des températures élevées. Cela représente une violation grave des normes de conservation des aliments.",
        author: 'Stella T.',
      },
      {
        text: "J'ai été choqué de voir que ***** laisse des produits laitiers à température ambiante pendant des heures. Cela va à l'encontre de toutes les normes de conservation des aliments que je connais et c'est un risque pour la santé publique.",
        author: 'Maria A.',
      },
      {
        text: "J'ai été consterné de voir que ***** laisse des produits laitiers dehors pendant des heures. Cela contredit toutes les normes de conservation des aliments, et représente un risque pour la santé publique.",
        author: 'Nathan A.',
      },
      {
        text: "J'ai remarqué que ***** laissait les produits laitiers hors des réfrigérateurs pendant des périodes prolongées. C'est une violation évidente des normes de conservation des aliments et cela peut être dangereux.",
        author: 'Bernard I.',
      },
      {
        text: "J'ai constaté que chez *****, les produits laitiers étaient laissés sur le comptoir à température ambiante pendant des heures. C'est une violation claire des normes de conservation des aliments.",
        author: 'Dylan A.',
      },
      {
        text: "Au restaurant *****, j'ai remarqué que les produits laitiers étaient laissés sans surveillance à la température ambiante pendant de longues périodes, une violation flagrante des normes de conservation des aliments.",
        author: 'Simone B.',
      },
    ],
  },
]
